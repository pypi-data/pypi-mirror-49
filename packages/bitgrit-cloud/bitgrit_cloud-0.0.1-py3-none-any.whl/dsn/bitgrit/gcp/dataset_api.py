...
def get_dataset_bucket(dataset_type):
    from dsn.bitgrit.datasets.dataset import DatasetType
    switcher = {
        DatasetType.TEXTS: 'dataset-text',
        DatasetType.IMAGES: 'dataset-images',
        DatasetType.VIDEOS: 'dataset-videos'
    }
    return switcher.get(dataset_type)
...
class DatasetAPI:
    ...
    def __init__(self, dataset_type):
        ...
        self._bucket_name = get_dataset_bucket(dataset_type)
    ...
    def _convert_blob_to_dataset(self, blob):
        ...
        return Dataset(self.dataset_type, blob.name)
    ...
    async def upload_file(self, source_file_name, destination_file_name):
        ...
        Parameters :
        1. dataset_type : i.e. (image/text/video)
        2. source_file_name : Source file name with full path (Local)
        3. destination_file_name : i.e. filename or dirname/filename
        """
        try:

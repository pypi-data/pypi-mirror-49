    ...
    try:
        ...
        with logging_utilities.log_activity(logger,
            ...
            with automl_run_context.get_run() as run:
                ...
                try:
                    automl_run_context.save_str_output(
                        inference.inference._create_conda_env_file(), constants.CONDA_ENV_FILE_PATH)
                except Exception as e:
                    logger.warning("Failed to create conda environment file. Error seen is: " + str(e))

                # save scoring file into artifacts
                try:
                    automl_run_context.save_str_output(inference.inference._get_scoring_file(
                        if_pandas_type=(transformed_data_context._get_raw_data_type() ==
                                        inference.PandasParameterType),
                        input_sample_str=transformed_data_context._get_raw_data_snapshot_str(),
                        automl_run_id="{}".format(run.id),
                        is_forecasting=automl_settings.is_timeseries),
                        constants.SCORING_FILE_PATH)
                except Exception as e:
                    logger.warning("Failed to scoring inference file. Error seen is: " + str(e))
...
def _explain_model_in_fit(child_run, pipeline, transformed_data_context, class_labels, max_cores_per_iteration,
    ...
    explainer_data_X = training_utilities._upgrade_sparse_matrix_type(transformed_data_context.X)
    explainer_data_X_valid = training_utilities._upgrade_sparse_matrix_type(transformed_data_context.X_valid)

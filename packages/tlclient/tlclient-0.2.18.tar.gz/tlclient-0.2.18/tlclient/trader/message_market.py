...
from tlclient.trader.constant import (MODULE_ERR_MSG_LENGTH, MODULE_TICKER_LENGTH,
                             ...
                             AssetType, BarType, ExchangeID, MsgType)
...
class MktBasic(ctypes.Structure):
    ...
    def get_real_vol(self, vol):
        return vol * (10 ** -self.vol_precision)
...
class MktSnapPlus(MktSnap):
    ...
    def __str__(self):
        return '<snap-{} last_px={} vol={} ask1={}@{} bid1={}@{}>'.format(
            MktBasic.__str__(self),
            self.last_price, self.get_real_vol(self.volume),
            self.get_real_vol(self.ask_volume[0]), self.ask_price[0],
            self.get_real_vol(self.bid_volume[0]), self.bid_price[0],
        )
...
class MktOrder(MktBasic):
    ...
    def __str__(self):
        return '<md_order-{} group_id={} index={} price={} volume={} order_type={}>'.format(
            ...
            self.price, self.get_real_vol(self.volume),
            self.order_type
        )
...
class MktBar(MktBasic):
    ...
    def __str__(self):
        return '<bar/{}-{} O={} C={} L={} H={} V={}>'.format(
            ...
            self.open, self.close, self.low, self.high, self.get_real_vol(self.volume),
        )

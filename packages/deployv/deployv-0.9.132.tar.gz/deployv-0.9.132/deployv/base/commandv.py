...
class CommandV(Extensible):
    ...
    def update_production(self):
        ...
        env = self.instance_manager.docker_env
        original_db_config = self.instance_manager.db_config
        ...
        backup_params = {'data_dir': original_db_config.get('data_dir'),
                         'extra_files': [branches_json], 'prefix': prefix}
        ...
        start_result = self._production_update_and_start(
            image, container_name, env, self.__config.odoo_version, switch_res.get('result'))
    ...
    def _production_backup(self, container_name, backup_params):
        ...
        self.sender.send_message('Temporary backup generated in "{}"'.format(backup))
    ...
    def _production_update_and_start(self, stable_image, container_name,
                                     env, odoo_version, maintenance_container=False):
        ...
        for module in modules:
            for attempt in range(2):
                ...
                pg_shell.terminate_sessions(
                    db_config['db_name'], config.instance_config.get('config').get('db_owner'),
                    config.instance_config.get('config').get('db_owner_passwd'))
                try:

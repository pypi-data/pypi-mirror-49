...
class ESVClient(Client):
    ...
    def get(self, indicators=None, periods=None):
        ...
        names = df[['expenditure', 'name', 'allocation', 'allocation_name']]
        df = df.drop(['name', 'allocation_name'], axis=1)

        expenditures = (names
                        .drop_duplicates(subset=['expenditure'])
                        .dropna(subset=['expenditure']))

        expenditures = expenditures.filter(items=['expenditure', 'name'])

        allocations = (names
                       .drop_duplicates(subset=['allocation'])
                       .dropna(subset=['allocation']))

        allocations = allocations.filter(items=['allocation', 'allocation_name'])
        allocations = allocations.rename(columns={'allocation_name': 'name'})
    ...
    def save(self, path, **kwargs):
        ...
        f2p.update_entity('expenditure', self.data['expenditures'])
        f2p.update_entity('allocation', self.data['allocations'])
...
#    },
    ...
    {
        'concept': 'ingoing_transfer_amount',

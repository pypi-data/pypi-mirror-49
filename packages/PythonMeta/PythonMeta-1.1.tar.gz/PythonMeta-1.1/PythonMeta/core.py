...
def Fig_Forest (size,dpi,es_w_ci, titletxt="Meta-analysis Results",no_ttl=False):
    ...
    myfig = plt.figure(num="Forest plot by PythonMeta", linewidth=1, figsize=size, dpi=dpi)  #Frameon=False
...
def Fig_Funnel (size,dpi,effs):
    myfig = plt.figure(num="Funnel plot",linewidth=1,figsize=size,dpi=dpi)
    myfig.set_size_inches(size)
    x=[];y=[]

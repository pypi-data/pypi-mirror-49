...
def ks_compute(df,feature_name, ks_plot=True,file_path='',savefig = True):
    ...
    scorelst = sorted(set(df[(df[feature_name]<abnormalval_big)&(df[feature_name]>abnormalval_litter)][feature_name].values))
    kslst = []
    goodcum = []
    badcum = []
...
def score_interval_describe(df,feature_name ,file_path, bins=None):
    ...
    for ind, cutoff in enumerate(bins):
        ...
        overdue_num = len(df[(df[feature_name] <= interval_right) & (df[feature_name] > interval_left) & (df['y'] == 1)])

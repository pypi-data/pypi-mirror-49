...
def time_interval(
        ...
        # NOTE: gradient_amplitude and positions could be vectors
        # This works for phase accumulation through dot product, what about 
        # diffusion ?
    ...
    delta_omega = (
        ...
        # + sycomore.gamma*numpy.dot(gradient_amplitude, position)
    )
    F = phase_accumulation(duration * 2*numpy.pi*rad * delta_omega)
    
    A = numpy.identity(4) #diffusion(species, gradient_amplitude, duration, gamma)
...
def diffusion(species, gradient_amplitude, duration, gamma=42.57747892*MHz/T):
    A = numpy.exp(
        -1/3 
        * (species.D * gamma**2 * gradient_amplitude**2 * duration**3).magnitude)
    return numpy.diag((A, A, 1, 1))

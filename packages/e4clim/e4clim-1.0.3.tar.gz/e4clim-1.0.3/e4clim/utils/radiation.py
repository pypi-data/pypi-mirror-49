...
class HorizontalRadiationComputer():
    ...
    def _add_attributes(self, var_name):
        ...
        try:
            var_name = var_name[1:] if var_name[0] == '_' else var_name
            var.attrs['long_name'] = self.long_names[var_name]
            var.attrs['units'] = (self.power_units
                                  if UNITS[var_name] == 'power'
                                  else UNITS[var_name])
...
class TiltedRadiationComputer(HorizontalRadiationComputer):
    ...
    def diffused_trans_factor(self):
        ...
        .. note::
          The diffuse transposition factor is calculated using the HDKR.actuator.

...
class Bilitime:
    ...
    def draw(self):
        ...
        x1 = np.array([i+1 for i in range(len(self.playlist))])
        y1 = np.array(self.playlist)
        plt.plot(x1,y1,color = 'r',label = "播放人数")# 折线 1 x 2 y 3 color

        x2 = np.array([i+1 for i in range(len(self.weblist))])
        y2 = np.array(self.weblist)
        plt.plot(x2,y2,color = 'g',label = "在线人数")# 折线 1 x 2 y 3 color

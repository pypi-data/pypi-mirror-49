...
from ndlapi.api.recognition.EmotionRecognition import EmotionRecognition
...
def parse_args():
    ...
    parser.add_argument('-er', action='store_true', default=False, help='Use EmotionRecognition (default=False)')
...
def initialize_connection(args):
    # Check arguments
    assert args.fd or args.er, 'Choose face detector (-fd) or emotion recognition (-er) to process'
    ...
    auth = Auth.AuthCredential('emotionsdemo.com:50051', osp.join(cert_home, root_json_filepath), ssl_auth_info)
...
def process_emotion_recognition_image(er, image_path, out_path):
    # Prepare image data from file
    image = Image.from_file(image_path)
...
# Process image with emotion recognition
def process_emotion_recognition_video(er, video_path, out_path):
    # Prepare video data
    video = Video.from_file(video_path)
    ...
    if query.ok():
        ...
        with open(osp.join(out_path, 'er_video_result.json'), 'w') as f:
            json.dump(result, f, indent=2)
    else:
        print('Query error:', query.error())
...
if __name__ == '__main__':
    ...
    if arguments.er:
        emotion_recognition = EmotionRecognition(ssl_auth)

        if arguments.image_path is not None:
            print("Start processing image with Emotion Recognition " + arguments.image_path)
            process_emotion_recognition_image(emotion_recognition, arguments.image_path, arguments.out_path)
            print("Processing image done")

        if arguments.video_path is not None:
            print("Start processing video with Emotion Recognition " + arguments.video_path)
            process_emotion_recognition_video(emotion_recognition, arguments.video_path, arguments.out_path)
            print("Processing video done")

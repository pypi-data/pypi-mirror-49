...
class Gamertag(object):
    ...
    def __init__(self, name):
        ...
        self.total_games_played = int(re.search(r": (\d+)", topRC[0].text_content()).group(1))
        self.games_completed = int(re.search(r": (\d+)", topRC[1].text_content()).group(1))
        self.average_completion = float(re.search(r": (\d+)", topRC[2].text_content()).group(1))

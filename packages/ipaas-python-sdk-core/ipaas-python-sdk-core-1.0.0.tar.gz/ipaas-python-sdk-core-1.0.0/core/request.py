...
# coding=utf-8
import abc
from core.vendored.six import iterkeys
from core.vendored.six import iteritems
from core.vendored.six import add_metaclass

from core.http import protocol_type
from core.http import method_type as mt
from core.http import format_type as ft
from core.auth.composer import rpc_signature_composer as rpc_signer
from core.auth.composer import roa_signature_composer as roa_signer
from core.utils.parameter_helper import md5_sum
from core.auth.algorithm import sha_hmac1
...
class ClickRequest:
    """
    Click request base class. This class wraps up common parameters for a request.
    """
...
class RpcRequest(ClickRequest):
    ...
    def get_signed_header(self, region_id=None, ak=None, secret=None):
        ...
        for headerKey, headerValue in iteritems(self.get_headers()):
            if headerKey.startswith("x-click-") or headerKey.startswith("x-sdk-"):
                headers[headerKey] = headerValue
        return headers


class RoaRequest(ClickRequest):
    """
    ...
    def _get_sign_params(self):
        ...
        if req_params is None:
            req_params = {}
        self.add_header("x-click-version", self.get_version())
        # req_params['Version'] = self.get_version()
        # req_params['Action'] = self.get_action_name()
        # req_params['Format'] = self.get_accept_format()
        return req_params
    ...
    def get_signed_header(self, region_id, ak, secret):
        ...
        if 'RegionId' not in sign_params.keys():
            sign_params['RegionId'] = region_id
            self.add_header('x-click-region-id', str(region_id))
...
class CommonRequest(ClickRequest):
    ...
    def trans_to_click_request(self):

...
class sasImage(SubArray, np.ma.MaskedArray):
    ...
    def __new__(cls, file, detector_distance=None, center=None,
        ...
        center : None, list 2xfloat, sasImage
            Center is [height, width] position of closest point to sample or primary beam in standard SAS geometry
    ...
    def setDetectorDistance(self, detector_distance):
        ...
        if isinstance(detector_distance, (float, int)):
            ...
            self.detector_distance = [v if isinstance(v, (float, int)) else v for v in detector_distance]
        elif isinstance(detector_distance, sasImage):
            self.detector_distance = [v if isinstance(v, (float, int)) else v
    ...
    def _pXYZ(self):
        ...
        xyz[:, :, 2] = self.detector_distance  # in  units m
    ...
    def radialAverage(self, center=None, number=300, kind='log', calcError=False, units=None):
        ...
        center : list 2x float
            ...
            If not given the attribut center in the data is used.
        ...
        for attri in ['DateTime', 'ImageLength', 'ImageWidth', 'XResolution', 'YResolution', 'center',
                      'description', 'detector_distance', 'exposure_time', 'filename', 'imageDescription',
                      'pixel_size', 'wavelength', 'alpha', 'beta', 'gamma']:
            try:
    ...
    def _showPolar_old(self, center=None, scaleR=1, offset=0):
        """
        ...
        ndimage.geometric_transform(self, mapping=transform, output=newimage,
                                    extra_keywords={'bc': self.center, 'shape': self.shape, 'scale': scaleR,
                                                    'offset': offset})
    ...
    def asdataArray(self, masked=0):
        ...
        -------
         dataArray with [qx,qy,I(qx,qy) ]
            ...
            - .sasImageshape as shap of original sasImage
        ...
        if isinstance(masked, (float, int)):
            ...
            out = dA(np.vstack([qxzw[mask, :].T, self.flatten()[mask]]), XYeYeX=[0, 2, None, None, 1, None])
        out.qx = qxzw[:, 0]
        out.qz = qxzw[:, 1]

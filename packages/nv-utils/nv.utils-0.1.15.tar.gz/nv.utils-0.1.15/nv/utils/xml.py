        ...
        for element in parent_element:
            if element:
                ...
                # in a series are the same, then the rest are the same.
                    ...
                    dct = {element[0].tag: XmlListConfig(element)}
                # if the tag has attributes, add those to the dict
                if element.items():
                    dct.update(dict(element.items()))

                if children_names.count(element.tag) > 1:
                    try:
                        current_value = self[element.tag]
                        current_value.append(dct)
                        self.update({element.tag: current_value})
                    except:  # the first of its kind, an empty list must be created
                        self.update({element.tag: [dct]})  # aDict is written in [], i.e. it will be a list

                else:
                    self.update({element.tag: dct})
...
def loads(s):
    return XmlDictConfig(ElementTree.XML(s))

...
import os.path
def rtAlign(tables, mapTables=None, refTable=None, destination=None, nPeaks=-1,
...
#    import pyopenms
    ...
    for fm, table in fms:
        ...
        if fm is refMap:
            # we map the original table to keep all peaks detected:
            t_align=get_map_table(table, mapTables)
            results.append(t_align)
            continue
        ...
        # we map the original table to keep all peaks detected prior to _transformTable:
        t_align=get_map_table(table, mapTables)
        _transformTable(t_align, transformation)
        path=_add_transformation(t_align, transformation, destination)
        t_align.meta['trafo_path']=path
        results.append(t_align)
    for t in results:
...
def get_map_table(t, tables):
    import copy
    from emzed.core.data_types import PeakMap
    ids=[_source_id(table) for table in tables]
    id_=_source_id(t)
    try:
        i=ids.index(id_)
        table=copy.deepcopy(tables[i])
        table.replaceColumn('peakmap', t.peakmap.uniqueValue(), type_=PeakMap)
        return table
    except:
        return t
...
def _add_transformation(t, trafo, destination):
    filename=_get_filename(t)
    path=os.path.join(destination, filename)
    save_transformation(trafo, path)
    return path
    
    

def  _get_filename(t):
    filename=t.peakmap.uniqueValue().meta['source']
    fields=filename.split('.')
    __=fields.pop(-1) # remove ending
    fields.append('xml')
    return '.'.join(fields)

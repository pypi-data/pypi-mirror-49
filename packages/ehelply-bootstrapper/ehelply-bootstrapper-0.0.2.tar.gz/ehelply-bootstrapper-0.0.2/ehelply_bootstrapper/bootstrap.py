...
from ehelply_bootstrapper.drivers.rabbitmq import Rabbitmq
...
class Bootstrap:
    ...
    def __init__(self, service_name: str, service_key: str, service_version: str, loadables: List[str],
        ...
        self.rabbitmq_driver: Rabbitmq = None
        self.aws_driver: Rabbitmq = None

        if len(sys.argv) > 1 and "--dev" in sys.argv:
            self.dev_mode = True

        if self.config_path is None:
            if True or self.dev_mode:
                self.config_path = str(Path(__file__).parents[1]) + "/config"
            else:
                self.config_path = "/config/" + self.service_key

        self.logger: Logger = Logger(debug_mode=self.dev_mode)

        self.logger.info("Starting loading procedure for `" + self.service_name + "` with key `" + self.service_key + "`")
        ...
        self.logger.info("Confliction test passed")
        ...
        Config(self.service_key, self.config_path, configs, dev_mode=self.dev_mode).init()
        ...
        if LOADABLE_FASTAPI in self.loadables:
            ...
            self.fastapi_init()
            self.logger.info("Registering middleware to fast api...")
            self.fastapi_middleware()
            self.logger.info("Registering routers to fast api...")
        ...
        if LOADABLE_RABBITMQ in self.loadables:
            self.logger.info("Loading rabbitmq...")
            self.rabbitmq_init()
        ...
        if LOADABLE_FASTAPI in self.loadables:
            self.logger.info("Registering additional endpoints to fast api...")
            self.fastapi_register_endpoints()

        if LOADABLE_SOCKET in self.loadables:
            self.logger.info("Registering additional events to socket io...")
            self.socket_register_events()

        self.logger.info("Setting up service state...")
        ...
        State.rabbitmq = self.rabbitmq_driver
        State.aws = self.aws_driver

        self.logger.info("Ready")

        self.logger.info("Running post load...")
        self.post_load()
    ...
    def check_loadables_improvements(self):
        if LOADABLE_SENTRY not in self.loadables:
            self.logger.info("You're not using sentry. Consider using sentry to improve error catching and debugging.")
    ...
    def rabbitmq_init(self):
        self.rabbitmq_driver = Rabbitmq(dev_mode=self.dev_mode,
                                        connection_details=ConnectionDetails(host=State.config.bootstrap.rabbitmq.host,
                                                                             port=State.config.bootstrap.rabbitmq.port,
                                                                             username=State.config.bootstrap.rabbitmq.username,
                                                                             password=State.config.bootstrap.rabbitmq.password)).init()

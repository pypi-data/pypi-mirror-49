...
class TestBasicUse(TestBase):
    ...
    def test_set_field(self):
        ...
        self.assertEqual(item['two_1_1']['one_x_x']['two_x_1__f_string'],
                         'three')
    ...
    def test_getting_fields_reverse_autocomplete(self):
        ...
        item_two = item_one['two_1_1']  # `__getitem__` item is called
        self.assertTrue(item_two.is_single_item())
        self.assertIs(item_two['one_1_1'], item_one)
           
        item_one = self.ItemGeneralOne()
        item_one['two_1_1__f_integer'] = 10  # `__setitem__` item is called
        ...
        # many-to-one
        item_one = self.ItemGeneralOne()
        item_two = item_one['two_x_1']  # `__getitem__`
        self.assertTrue(item_two.is_single_item())
        ...
        item_one = self.ItemGeneralOne()
        item_one['two_x_1__f_integer'] = 10  # `__setitem__`
        ...
        # --- moving through multiple items ---
        item_one_1 = self.ItemGeneralOne()
        item_one_2 = item_one_1['two_x_1__one_x_1']
        
        # relation item_one_1 <-> two_x_1
        self.assertIn('two_x_1', item_one_1)
        two_x_1 = item_one_1['two_x_1']
        self.assertTrue(two_x_1.is_single_item())
        # reverse must be bulk
        bulk_one = two_x_1['one_1_x']
        self.assertTrue(bulk_one.is_bulk_item())
        self.assertEqual(len(bulk_one), 1)
        self.assertEqual(bulk_one[0], item_one_1)
        
        # relation two_x_1 <-> one_x_1
        self.assertIn('one_x_1', two_x_1)
        one_x_1 = two_x_1['one_x_1']
        self.assertTrue(one_x_1.is_single_item())
        self.assertEqual(two_x_1['one_x_1'], item_one_2)
        # reverse must be bulk
        bulk_two = item_one_2['two_1_x']
        self.assertTrue(bulk_two.is_bulk_item())
        self.assertEqual(len(bulk_two), 1)
        self.assertEqual(bulk_two[0], two_x_1)
        ...
        item_two = bulk_one['two_x_1']  # `__getitem__`
        self.assertTrue(item_two.is_single_item())
        self.assertIs(item_two['one_1_x'], bulk_one)
         
        bulk_one = self.ItemGeneralOne.Bulk()
        bulk_one['two_x_1__f_integer'] = 10  # `__setitem__`
        self.assertNotIn('two_x_1', bulk_one)  # was not generated
        
        # many-to-many
        bulk_one = self.ItemGeneralOne.Bulk()
        bulk_two = bulk_one['two_x_x']  # `__getitem__`
        self.assertTrue(bulk_two.is_bulk_item())
        self.assertTrue(bulk_two['one_x_x'].is_bulk_item())
        self.assertIs(bulk_two['one_x_x'], bulk_one)
        
        # --- moving through multiple items ---
        bulk_one = self.ItemGeneralOne.Bulk()
        self.assertNotIn('two_x_x__one_x_1', bulk_one)
        item_one = bulk_one['two_x_x__one_x_1']  # `__getitem__`
        self.assertTrue(item_one.is_single_item())
        self.assertNotIn('two_x_1', item_one)  # nothing to reverse to
        self.assertNotIn('two_x_x', item_one)  # nothing to reverse to
        
        bulk_one = self.ItemGeneralOne.Bulk()
        bulk_one['two_x_x__one_x_1__f_integer'] = 10  # `__setitem__`
        self.assertNotIn('two_x_x__one_x_1', bulk_one)  # same as before
        item_one = bulk_one['two_x_x__one_x_1']
        self.assertTrue(item_one.is_single_item())
        self.assertNotIn('two_x_1', item_one)  # nothing to reverse to
        self.assertNotIn('two_x_x', item_one)  # nothing to reverse to

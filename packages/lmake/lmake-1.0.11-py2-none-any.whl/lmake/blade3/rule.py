...
class Rule:
  ...
  def expandRule(self):
    ...
    for dep in self.depsList:
      ...
      if packageName is None and ruleName is None:
      ...
      if packageName == "":
        ...
        if packageName in package.globalPackages:
          ...
          pkg = package.Package.createPackage(
              packageName, self.package.dirPrefix, self.package.platform)
      if pkg is None:
  ...
  def parseRule(self, dep):
    if not dep.startswith("//") and not dep.startswith(":") and not dep.startswith('#//'):
      print 'invalid deps: "%s"' % dep
      print 'must be started with "//" (means in the codebase root dir), ":" (means in the current package)'
      print 'or "#//" (means in the builtin dir)'
      sys.exit(-1)

    if dep.startswith('#//'):
      builtin_package = True
    else:
      builtin_package = False

    ruleMatcher = self.ruleRegex.match(dep)
    if ruleMatcher is None:
      return [None, None]
    ...
    if builtin_package:
      packageName = os.path.join(os.path.dirname(config.current_file_dir), packageName)

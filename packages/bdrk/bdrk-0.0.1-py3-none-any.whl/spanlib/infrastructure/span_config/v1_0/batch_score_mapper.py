...
from spanlib.domain.exceptions import ConfigInvalidError
from spanlib.infrastructure.span_config.base_mapper.base_mapper import BaseMapper
from spanlib.infrastructure.span_config.config_objects import BatchScoreConfig
...
class V1BatchScoreConfigMapper(BaseMapper):
    @staticmethod
    def create_config_object(config_dict) -> BatchScoreConfig:
        mapper = V1BatchScoreConfigMapper(config_dict)
        return BatchScoreConfig(
            install=mapper.install_commands.content,
            image=mapper.image,
            script_commands=mapper.script_commands,
            parameters=mapper.parameters,
            secret_names=mapper.secret_names,
        )
    ...
    # TODO(BDRK-552): Use Mapping instead of Dict
    @property

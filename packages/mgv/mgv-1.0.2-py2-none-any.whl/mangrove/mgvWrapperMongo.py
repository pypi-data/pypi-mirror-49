...
def graphExists(pattern, graphpath):
    ...
    """
    g = db[pattern.project.name].find_one({'code': 'Graph', 'path': graphpath,
                                           'parent': pattern.uuid})
    if g:
        return True
    return False
...
def setNodeAttr(node, **kwargs):
    ...
    Parameters:
        ...
        file_path (str): the path of the file containing the data,
                         in case of a file managed database (optional).
    ...
    if not db[project_name].find_one({'code': code, '_id': uuid}):
        print >> sys.__stderr__, "no node type %s with uuid" % code, uuid
        return False
...
def getNodeAttr(node, attr):
    ...
    return db[project_name].find_one({'code': code, '_id': uuid}, {'_id': 0, attr: 1})[attr]
...
def setDictionary(father, typeDic, name, value, file_path=None):
    ...
    db[father.getProject().name].update_one({'code': typeDic, 'parent': father.uuid, 'name': name},
                                            {"$set": {"value": toHtml(value)}})


def delDictionary(father, typeDic, name, file_path=None):

...
class PTBaseModel(ABC):
    ...
    def fit(self):
        ...
        while epoch <= self.num_training_epochs:
            ...
            if epoch % self.log_interval == 0:
                validation_loss, val_evaluation_score = self._valid(valid_data_loader, epoch, restarts)
            ...
            if validation_gap_loss > 0 or validation_gap_metric > 0:
                ...
                self.logging(
                    " * Save model at Epoch {}\t | Improved loss: {:.3f}\t |\
                         Improved accuracy: {:.3f}".format(
                        epoch, validation_gap_loss, validation_gap_metric
                    )
                )
    ...
    def _train(self, data_loader, epoch):
        ...
        self.logging("Data Load Time : batch=>{:.2f} | average => {:.2f}[s]".format(data_time.value, data_time.avg))
        self.logging(
            "Batch Run Time : batch=>{:.2f} | average => {:.2f}[s] | sum  ==> {:.2f}[s]".format(
    ...
    def _valid(self, data_loader, epoch, restarts):
        ...
        # ==============================================================================================================
        losses = AverageMeter()  # loss tracker
        accuracy = AverageMeter()  # TODO accuracy tracker, 需要改成更加通用的表示，有些评估指标并不是accuracy
        batch_time = AverageMeter()  # forward prop + back prop time tracker
        ...
        with torch.no_grad():  # Prohibit gradient computation explicitly.
            for i, (inputs, labels) in enumerate(data_loader):
                ...
                evaluate_score = self.evaluate(predicted_scores, labels)
                ...
                if evaluate_score is None:
                    ...
                    accuracy.update(evaluate_score, data_loader.batch_size)
        ...
        self.logging(
                "\n * Validation Loss: {:.3f}\t | Validation Score: {:.2f}\t\t | Restart times: {}".format(

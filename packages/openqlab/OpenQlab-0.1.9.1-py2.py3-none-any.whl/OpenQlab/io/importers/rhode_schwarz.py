...
class RhodeSchwarz(BaseImporter):
    NAME = 'RhodeSchwarz'
    AUTOIMPORTER = True
    STARTING_LINES = [
        '^Type',
    ...
    def _parse(self):
        ...
        while line:
            ...
            if line.startswith('Values'):
                ...
                ylabel = utils.get_file_basename(self._stream.name) + '_' + current_trace
                data.append(pd.read_csv(self._stream, sep=';', decimal=dec_sep, index_col=0,
                                        usecols=[0, 1], names=[self._get_xlabel(), ylabel],
                                        header=None, nrows=self._header['Values']))

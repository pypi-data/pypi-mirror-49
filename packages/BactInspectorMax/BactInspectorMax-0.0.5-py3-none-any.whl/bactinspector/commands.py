...
def sample_and_refseq_species_matches(args, num_best_matches):
    ...
    if 'local_mash_and_info_file_prefix' in args:
        refseq_species_match_file = args.local_mash_and_info_file_prefix + '.species.tsv'
    else:
...
def run_create_species_info(args):
    ...
    merged = info_df.merge(
        refseq_assembly_details[['GCF_accession_without_version', 'bioproject', 'biosample', 'refseq_category', 'taxid', 'species_taxid', 'organism_name', 'infraspecific_name', 'assembly_level', 'asm_name', 'submitter', 'ftp_path']],
    ...
    # reorder columns
    merged = merged[['accession', 'GCF_accession', 'GCF_accession_without_version', 'filename', 'length', 'num_contigs', 'info', 'organism_name', 'full_organism_name', 'taxid', 'species_taxid', 'bioproject', 'biosample','refseq_category', 'infraspecific_name', 'assembly_level', 'asm_name', 'submitter', 'ftp_path']]
    ...
    merged_with_bacsort['curated_organism_name'] = np.where(merged_with_bacsort['bacsort_organism_name'].notna(), merged_with_bacsort['bacsort_organism_name'], np.nan)
    merged_with_bacsort['curated_organism_name'] = np.where((merged_with_bacsort['curated_organism_name'].isna() & merged_with_bacsort['excluded_GCF_accession'].notna()), 'Excluded', merged_with_bacsort['curated_organism_name'])
    merged_with_bacsort['curated_organism_name'] = np.where(merged_with_bacsort['curated_organism_name'].isna(), merged_with_bacsort['organism_name'], merged_with_bacsort['curated_organism_name'])

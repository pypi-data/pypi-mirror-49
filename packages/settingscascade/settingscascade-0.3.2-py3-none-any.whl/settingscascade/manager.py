...
class SettingsManager:
    ...
    :param els: a List of ElemeentSchema objects, If not specified, no elements will be created
    """
    ...
    def __getattr__(self, item):
        if item in self.els:
            if not self.els.get(item):
                raise ValueError(f"No schema defined for {item}")
    ...
    def lookup_value(self, selector: str, key: str):
        ...
        raise ValueError(
            f"Could not find value: {key} using selector: {selector}. "
            f"Current context is {self.current_context}"
        )

...
import os
import pyacryl.crypto as crypto
import time
import struct
import json
import base58
...
class Address(object):
    def __init__(self, address='', publicKey='', privateKey='', seed='', alias='', nonce=0):
        ...
        if seed:
            ...
            if not pyacryl.validateAddress(address):
                ...
                self.seed = seed
                self.nonce = nonce
        elif alias and not pyacryl.OFFLINE:
            self.address = pyacryl.wrapper('/alias/by-alias/%s' % alias).get("address", "")
            self.publicKey = ''
            self.privateKey = ''
            self.seed = ''
            self.nonce = 0
    ...
    def __str__(self):
        if self.address:
            ab = []
            try:
                assets_balances = pyacryl.wrapper('/assets/balance/%s' % self.address)['balances']
                for a in assets_balances:
                    if a['balance'] > 0:
                        ab.append("  %s (%s) = %d" % (a['assetId'], a['issueTransaction']['name'].encode('ascii', 'ignore'), a['balance']))
            except:
                pass
            return 'address = %s\npublicKey = %s\nprivateKey = %s\nseed = %s\nnonce = %d\nbalances:\n  Acryl = %d%s' % (self.address, self.publicKey, self.privateKey, self.seed, self.nonce, self.balance(), '\n'+'\n'.join(ab) if ab else '')
    ...
    def balance(self, assetId='', confirmations=0):
        try:
            if assetId:
                return pyacryl.wrapper('/assets/balance/%s/%s' % (self.address, assetId))['balance']
            else:
                return pyacryl.wrapper('/addresses/balance/%s%s' % (self.address, '' if confirmations == 0 else '/%d' % confirmations))['balance']
        except:
            return 0

    def assets(self):
        req = pyacryl.wrapper('/assets/balance/%s' % self.address)['balances']
        return [r['assetId'] for r in req]

    def aliases(self):
        a = pyacryl.wrapper('/alias/by-address/%s' % self.address)
        if type(a)==list:
    ...
    def _generate(self, publicKey='', privateKey='', seed='', nonce=0):
        ...
        unhashedAddress = chr(1) + str(pyacryl.CHAIN_ID) + crypto.hashChain(pubKey)[0:20]
    ...
    def issueAsset(self, name, description, quantity, decimals=0, reissuable=False, txFee=pyacryl.DEFAULT_ASSET_FEE):
        if not self.privateKey:
            msg = 'Private key required'
            logging.error(msg)
            pyacryl.throw_error(msg)
            ...
            req = pyacryl.wrapper('/assets/broadcast/issue', data)
            if pyacryl.OFFLINE:
                return req
            else:
                return pyacryl.Asset(req['assetId'])

    def reissueAsset(self, Asset, quantity, reissuable=False, txFee=pyacryl.DEFAULT_TX_FEE):
        timestamp = int(time.time() * 1000)
        sData = b'\5' + \
                base58.b58decode(self.publicKey) + \
        ...
        req = pyacryl.wrapper('/assets/broadcast/reissue', data)
        if pyacryl.OFFLINE:
            return req
        else:
            return req.get('id', 'ERROR')

    def burnAsset(self, Asset, quantity, txFee=pyacryl.DEFAULT_TX_FEE):
        timestamp = int(time.time() * 1000)
        ...
        req = pyacryl.wrapper('/assets/broadcast/burn', data)
        if pyacryl.OFFLINE:
            return req
        else:
            return req.get('id', 'ERROR')

    def sendAcryl(self, recipient, amount, attachment='', txFee=pyacryl.DEFAULT_TX_FEE, timestamp=0):
        if not self.privateKey:
            msg = 'Private key required'
            logging.error(msg)
            pyacryl.throw_error(msg)
            ...
            logging.error(msg)
            pyacryl.throw_error(msg)
        elif not pyacryl.OFFLINE and self.balance() < amount + txFee:
            msg = 'Insufficient Acryl balance'
            logging.error(msg)
            pyacryl.throw_error(msg)
            ...
            return pyacryl.wrapper('/assets/broadcast/transfer', data)

    def massTransferAcryl(self, transfers, attachment='', timestamp=0):
        ...
        if not self.privateKey:
            ...
            logging.error(msg)
            pyacryl.throw_error(msg)
        elif not pyacryl.OFFLINE and self.balance() < totalAmount + txFee:
            msg = 'Insufficient Acryl balance'
            logging.error(msg)
            pyacryl.throw_error(msg)
        else:
            ...
            return pyacryl.wrapper('/transactions/broadcast', data)

    def sendAsset(self, recipient, asset, amount, attachment='', feeAsset='', txFee=pyacryl.DEFAULT_TX_FEE, timestamp=0):
        if not self.privateKey:
            msg = 'Asset not issued'
            logging.error(msg)
            pyacryl.throw_error(msg)
        elif not pyacryl.OFFLINE and asset and not asset.status():
            msg = 'Asset not issued'
            logging.error(msg)
            pyacryl.throw_error(msg)
        elif amount <= 0:
            ...
            logging.error(msg)
            pyacryl.throw_error(msg)
        elif not pyacryl.OFFLINE and asset and self.balance(asset.assetId) < amount:
            msg = 'Insufficient asset balance'
            logging.error(msg)
            pyacryl.throw_error(msg)
        elif not pyacryl.OFFLINE and not asset and self.balance() < amount:
            msg = 'Insufficient Acryl balance'
            logging.error(msg)
            pyacryl.throw_error(msg)
        elif not pyacryl.OFFLINE and not feeAsset and self.balance() < txFee:
            msg = 'Insufficient Acryl balance'
            logging.error(msg)
            pyacryl.throw_error(msg)
        elif not pyacryl.OFFLINE and feeAsset and self.balance(feeAsset.assetId) < txFee:
            msg = 'Insufficient asset balance'
            logging.error(msg)
            pyacryl.throw_error(msg)
        else:
            if feeAsset:
                feeInfos = pyacryl.wrapper('/assets/details/' + feeAsset.assetId)
    ...
    def massTransferAssets(self, transfers, asset, attachment='', timestamp=0):
        ...
        elif not pyacryl.OFFLINE and self.balance() < txFee:
            logging.error('Insufficient Acryl balance')
        elif not pyacryl.OFFLINE and self.balance(assetId=asset.assetId) < totalAmount:
    ...
    def dataTransaction(self, data, timestamp=0):
        if not self.privateKey:
            ...
            return pyacryl.wrapper('/transactions/broadcast', dataObjectJSON)

    def _postOrder(self, amountAsset, priceAsset, orderType, amount, price, maxLifetime=30*86400, matcherFee=pyacryl.DEFAULT_MATCHER_FEE, timestamp=0):
        if timestamp == 0:
            timestamp = int(time.time() * 1000)
        ...
        sData = base58.b58decode(self.publicKey) + \
                base58.b58decode(pyacryl.MATCHER_PUBLICKEY) + \
                asset1 + \
                asset2 + \
                orderType + \
                struct.pack(">Q", price) + \
        ...
        data = json.dumps({
            "senderPublicKey": self.publicKey,
            "matcherPublicKey": pyacryl.MATCHER_PUBLICKEY,
            "assetPair": {
        ...
        req = pyacryl.wrapper('/matcher/orderbook', data, host=pyacryl.MATCHER)
        id = -1
        if 'status' in req:
        ...
        elif not pyacryl.OFFLINE:
            logging.error(req)
            pyacryl.throw_error(req)
        else:
            return req
        return id
    ...
    def cancelOrder(self, assetPair, order):
        ...
        req = pyacryl.wrapper('/matcher/orderbook/%s/%s/cancel' % ('WAVES' if assetPair.asset1.assetId == '' else assetPair.asset1.assetId, 'WAVES' if assetPair.asset2.assetId == '' else assetPair.asset2.assetId), data, host=pyacryl.MATCHER)
        if pyacryl.OFFLINE:
            return req
        else:
            id = -1
    ...
    def buy(self, assetPair, amount, price, maxLifetime=30 * 86400, matcherFee=pyacryl.DEFAULT_MATCHER_FEE, timestamp=0):
        assetPair.refresh()
        ...
        if pyacryl.OFFLINE:
            ...
            return pyacryl.Order(id, assetPair, self)

    def sell(self, assetPair, amount, price, maxLifetime=30 * 86400, matcherFee=pyacryl.DEFAULT_MATCHER_FEE, timestamp=0):
        assetPair.refresh()
        normPrice = int(pow(10, assetPair.asset2.decimals - assetPair.asset1.decimals) * price)
    ...
    def tradableBalance(self, assetPair):
        try:
            req = pyacryl.wrapper('/matcher/orderbook/%s/%s/tradableBalance/%s' % ('WAVES' if assetPair.asset1.assetId == '' else assetPair.asset1.assetId, 'WAVES' if assetPair.asset2.assetId == '' else assetPair.asset2.assetId, self.address), host=pyacryl.MATCHER)
            if pyacryl.OFFLINE:
                    return req
    ...
    def lease(self, recipient, amount, txFee=pyacryl.DEFAULT_LEASE_FEE, timestamp=0):
        if not self.privateKey:
            msg = 'Private key required'
            logging.error(msg)
            pyacryl.throw_error(msg)
            ...
            req = pyacryl.wrapper('/leasing/broadcast/lease', data)
            return req

    def leaseCancel(self, leaseId, txFee=pyacryl.DEFAULT_LEASE_FEE, timestamp=0):
        if not self.privateKey:
            msg = 'Private key required'
            logging.error(msg)
            pyacryl.throw_error(msg)
            ...
            req = pyacryl.wrapper('/leasing/broadcast/cancel', data)
            if pyacryl.OFFLINE:
                return req
            elif 'leaseId' in req:
                return req['leaseId']
    ...
    def getOrderHistory(self, assetPair, timestamp=0):
        ...
        req = pyacryl.wrapper('/matcher/orderbook/%s/%s/publicKey/%s' % ('WAVES' if assetPair.asset1.assetId == '' else assetPair.asset1.assetId, 'WAVES' if assetPair.asset2.assetId == '' else assetPair.asset2.assetId, self.publicKey), headers=data, host=pyacryl.MATCHER)
        return req
    ...
    def cancelOpenOrders(self, assetPair):
        ...
        for order in orders:
            ...
            if status=='Accepted' or status=='PartiallyFilled':
                ...
                pyacryl.wrapper('/matcher/orderbook/%s/%s/cancel' % ('WAVES' if assetPair.asset1.assetId == '' else assetPair.asset1.assetId, 'WAVES' if assetPair.asset2.assetId == '' else assetPair.asset2.assetId), data, host=pyacryl.MATCHER)
    ...
    def deleteOrderHistory(self, assetPair):
        ...
        for order in orders:
            ...
            pyacryl.wrapper('/matcher/orderbook/%s/%s/delete' % ('WAVES' if assetPair.asset1.assetId == '' else assetPair.asset1.assetId, 'WAVES' if assetPair.asset2.assetId == '' else assetPair.asset2.assetId), data, host=pyacryl.MATCHER)

    def createAlias(self, alias, txFee=pyacryl.DEFAULT_ALIAS_FEE, timestamp=0):
        aliasWithNetwork = b'\x02' + crypto.str2bytes(str(pyacryl.CHAIN_ID)) + struct.pack(">H", len(alias)) + crypto.str2bytes(alias)
        if not self.privateKey:
            msg = 'Private key required'
            logging.error(msg)
            pyacryl.throw_error(msg)
            ...
            return pyacryl.wrapper('/alias/broadcast/create', data)

    def sponsorAsset(self, assetId, minimalFeeInAssets, txFee=pyacryl.DEFAULT_SPONSOR_FEE, timestamp=0):
        if not self.privateKey:
            logging.error('Private key required')
        else:
            if timestamp == 0:
    ...
    def setScript(self, scriptSource, txFee=pyacryl.DEFAULT_SCRIPT_FEE, timestamp=0):
        script = pyacryl.wrapper('/utils/script/compile', scriptSource)['script'][7:]
        if not self.privateKey:
            logging.error('Private key required')
        else:
            ...
            sData = b'\x0d' + \
                b'\1' + \
                    crypto.str2bytes(str(pyacryl.CHAIN_ID)) + \
                    base58.b58decode(self.publicKey) + \
                b'\1' + \
    ...
    def setAssetScript(self, asset, scriptSource, txFee=pyacryl.DEFAULT_ASSET_SCRIPT_FEE, timestamp=0):
        script = pyacryl.wrapper('/utils/script/compile', scriptSource)['script'][7:]
        if not self.privateKey:
            logging.error('Private key required')
        else:
    ...
    def issueSmartAsset(self, name, description, quantity, scriptSource, decimals=0, reissuable=False, txFee=pyacryl.DEFAULT_ASSET_FEE):
        script = pyacryl.wrapper('/utils/script/compile', scriptSource)['script'][7:]
        if not self.privateKey:
            msg = 'Private key required'
            logging.error(msg)
            ...
            print(data)
            req = pyacryl.wrapper('/transactions/broadcast', data)
            if pyacryl.OFFLINE:
                return req
            else:
                return req

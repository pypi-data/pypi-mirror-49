...
URL_BASE = "https://www.lua.org/ftp/lua-{}.tar.gz"
FILENAME_BASE = "lua-{}.tar.gz"
VERSION = "5.3.5"
...
def check_installed():
    lib_path = os.path.join(get_install_dir(), "lib", "liblua.a")
...
def install():
    ...
    print "Download lua from url: ", url
    ...
    lua_folder = os.path.join(cache_dir, "lua-{}".format(VERSION))
    platform = get_platform()

    run_command(["make", "posix", "install",
                 "MYCFLAGS=-DLUA_USE_DLOPEN -m{}".format(platform),
                 "MYLDFLAGS=-Wl,--no-as-needed -ldl -m{}".format(platform),
                 "INSTALL_TOP={}".format(install_dir)
                ], cwd=lua_folder)

def main():
    if check_installed():
        print "Lua has been installed"
        return

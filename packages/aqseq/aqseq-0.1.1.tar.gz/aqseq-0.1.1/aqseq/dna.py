...
class AquariumDNA(MyLoggable):
    ...
    def __init__(self, aquarium_sample, connector):
        self.sample = aquarium_sample
        if not isinstance(self.sample, pydent.models.Sample):
            raise Exception("Expects a Sample, not a {}".format(type(self.sample)))
        self.__sequence = None
        self.connector = connector
        self.init_logger("AquariumDNA")
    ...
    def from_registry(self):
        ...
        :return:
        """
        self._info("Attempting to find '{}' in registry".format(self.sample.name))
        try:
    ...
    def register(self, sequence=None):
        ...
        if sequence is None:
            raise Exception("Cannot find a sequence for '{}'".format(self.sample.name))
        self._update_sequence_info(sequence)
    ...
    def pcr_products(self):
        ...
        :return:
        """
        self._info("Building sequence from template and primers")
        self._info(self._pprint_data(self.sample.properties))
        template, p1, p2 = [
        ...
        self._info("Finding primer sequences")
        p1_sequence = self.new(p1).sequence()
        p2_sequence = self.new(p2).sequence()
        self._info("Forward primer: '{}'".format(p1_sequence))
        self._info("Reverse primer: '{}'".format(p2_sequence))

        self._info("Finding template sequence")
        template_sequence = self.new(template).sequence()
        self._info("Template sequence:: {}".format(template_sequence.__repr__()))
        ...
        if self.sample.sample_type.name == "Primer":
            ...
            if seq is None:
                if self.sample.sample_type.name == "Fragment":
                    self._info("Attempting pcr product...")
                    seq = self.pcr_product()
                    if seq is not None:
                        return seq

                    self._info("Attempting linear product...")
...
class AquariumDNAFactory(MyLoggable):
    """AquariumDNAFactory that produces AquariumDNA instances."""

    def __init__(self, connector):
        self.connector = connector
        self.init_logger("AquariumDNAFactory")

    def new(self, sample):
        dna = AquariumDNA(sample, self.connector)

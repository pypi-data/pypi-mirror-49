...
def gini_coeff(vals, nvals, min_bins=10):
    vals = sorted(list(vals))
    nbins = min(len(vals), min_bins)
...
def _clump_score(samfile, binsize=10000, min_bins=10):
    bins, nreads = {}, 0
    for read in samfile:
        if read.is_unmapped:
            continue
...
def entropy_and_clump_score(bam_filename, binsize=10000, min_bins=10):
    samfile = pysam.AlignmentFile(bam_filename, "rb")

...
class fsm_bt():
    ...
    def __init__(self, blackboard_model={}, middleware="nanomsg"):
        ...
        self.lump = Matter()
        self.states = ['idle', 'goal', 'reaction',
                       'cancel_goal', 'continue_goal']
        ...
        self.machine = Machine(self.lump, states=self.states,
    ...
    def runActionModule(self, folder, name, type_):
        ...
        try:
            ...
            action = nep.json2dict(action)
            result = "running"
            print(action["bt"])
            while result == "running":
                result = self.doStep(action["bt"], "reaction")
            ...
            if self.lump.state == "idle":
                ...
                if len(goals) > 0:
                    ...
                    if len(reactions) > 0:
                        ...
                        self.current_reaction = copy.deepcopy(
                            self.dict_reactions[reaction])
            ...
            if self.lump.state == "reaction":
                ...
                if response == "success" or response == "failure":
                    if not len(self.current_goal) == 0:
                        self.return_bt = copy.deepcopy(
                            self.current_goal["return_bt"])
            ...
            if self.lump.state == "cancel_goal":
                if "stop_bt" in self.current_goal:
                    if len(self.current_goal["stop_bt"]) > 0:
                        response = self.doStep(
                            self.current_goal["stop_bt"], "goal")
                        if response == "success" or response == "failure":
                            self.current_goal = {}
                            self.lump.return_idle()
                    else:

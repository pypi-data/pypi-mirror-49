...
def map_arrays_1d_to_scaled_arrays(arrays_1d, map_to_scaled_arrays):
    ...
    return list(
        map(
            lambda array_1d, map_to_scaled_array: map_to_scaled_array(
                array_1d=array_1d
            ),
            arrays_1d,
...
def blurred_images_of_images_and_planes_from_1d_images_and_convolver(
    total_planes,
    ...
    for image_index in range(len(convolvers)):
        ...
        blurred_image_of_planes = lens_fit_util.blurred_image_of_planes_from_1d_images_and_convolver(
            ...
            image_plane_blurring_image_1d_of_planes=image_plane_blurring_images_1d_of_planes[
                image_index
            ],
            convolver=convolvers[image_index],
            map_to_scaled_array=map_to_scaled_arrays[image_index],
        )
...
def unmasked_blurred_image_of_datas_from_padded_grid_stacks_psfs_and_unmasked_images(
    padded_grid_stacks, psfs, unmasked_images_1d
):
    ...
    for image_index in range(len(padded_grid_stacks)):
        ...
        unmasked_blurred_image_of_data = lens_fit_util.unmasked_blurred_image_from_padded_grid_stack_psf_and_unmasked_image(
            padded_grid_stack=padded_grid_stacks[image_index],
            psf=psfs[image_index],
...
def unmasked_blurred_image_of_datas_and_planes_from_padded_grid_stacks_and_psf(
    planes, padded_grid_stacks, psfs
):
    ...
    for image_index in range(len(padded_grid_stacks)):
        ...
        unmasked_blurred_image_of_planes = unmasked_blurred_image_of_planes_from_padded_grid_stack_and_psf(
            planes=planes,
            padded_grid_stack=padded_grid_stacks[image_index],
            psf=psfs[image_index],
        )

        unmasked_blurred_image_of_datas_and_planes.append(
...
def unmasked_blurred_image_of_planes_from_padded_grid_stack_and_psf(
    ...
    for plane_index, plane in enumerate(planes):
        ...
        if plane.has_pixelization:
            ...
            unmasked_blurred_image_of_plane = lens_fit_util.unmasked_blurred_image_from_padded_grid_stack_psf_and_unmasked_image(
...
def unmasked_blurred_image_of_datas_planes_and_galaxies_from_padded_grid_stacks_and_psf(
    planes, padded_grid_stacks, psfs
):
    """For each image in the lens data-set, compute the unmasked blurred image of every unmasked unblurred image of \
    ...
    for image_index in range(len(padded_grid_stacks)):
        ...
        unmasked_blurred_image_of_planes_and_galaxies = unmasked_blurred_image_of_planes_and_galaxies_from_padded_grid_stacks_and_psfs(
            planes=planes,
            padded_grid_stack=padded_grid_stacks[image_index],
            psf=psfs[image_index],
        )

        unmasked_blurred_image_of_datas_planes_and_galaxies.append(
...
def unmasked_blurred_image_of_planes_and_galaxies_from_padded_grid_stacks_and_psfs(
    planes, padded_grid_stack, psf
):
    """This is a utility function for the function above, which performs the iteration over each plane and \
    ...
    for plane_index, plane in enumerate(planes):
        ...
        unmasked_blurred_image_of_galaxies = lens_fit_util.unmasked_blurred_image_of_galaxies_from_psf_and_unmasked_1d_galaxy_images(
            galaxies=plane.galaxies,
            image_plane_image_1d_of_galaxies=plane.image_plane_images_1d_of_galaxies[
                plane_index
            ],
            padded_grid_stack=padded_grid_stack,
            psf=psf,
        ...
        unmasked_blurred_image_of_planes_and_galaxies.append(

...
class DataSource(DataSourceLoaderBase):
    ...
    def download(self, *args, **kwargs):
        ...
        log.warning('{} data download not implemented'.format(self.name))
    ...
    def load(self, *args, **kwargs):
        ...
        with xr.open_dataset(mask_filepath) as ds:
        ...
        with resource_stream(__name__, resource_name) as f:
            if not self.cfg.get('no_verbose'):
                log.info('Reading land use to roughness map from {}'.format(
                    resource_name))
        ...
        if not self.cfg.get('no_verbose'):
            log.info('Reading land cover from {}'.format(filepath))
        ...
        if not self.cfg.get('no_verbose'):
            log.info('Averaging the land cover over the {} grid'.format(
                self.name))
        ...
        # Save the dataset
        fmt = ('clc' + self.cfg['year'], self.med.cfg['area'], self.name)
        postfix = '_{}_{}_{}_{}'.format(*fmt)
        filename = 'roughness_length' + postfix + '.nc'
        filepath = os.path.join(self.med.cfg['data_dir'], 'climate', filename)
        if not self.cfg.get('no_verbose'):
            log.info('Saving roughness length to '.format(filepath))

...
class EnvService(Servicer):
    ...
    def Start(self, request, context):
        try:
            ...
            session = Session(trial_id)
            if self._env_start_pb is not None:
                config = self._env_start_pb()
                config.ParseFromString(request.user_request.content)
                session.env_config = config

            session.env_instance = self._env_class(session)

            if not session.env_state:
                raise Exception("Environment did not create a state")

            self._envs[trial_id] = session

            # Send the initial state of the environment back to the client (orchestrator, normally.)
            reply = EnvStartReply(trial_id=trial_id)
            reply.env_state.tick_id = 0
            reply.env_state.time_stamp.GetCurrentTime()
            reply.env_state.content = session.env_state.SerializeToString()
    ...
    def Update(self, request, context):
        ...
        try:
            ...
            session = self._envs[trial_id]
            env = session.env_instance
            ...
            for i in [MACHINE_AGENT_ID, HUMAN_AGENT_ID]:

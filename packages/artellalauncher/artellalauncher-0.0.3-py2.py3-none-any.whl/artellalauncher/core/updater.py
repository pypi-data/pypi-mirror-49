...
class ArtellaUpdater(base.BaseWidget, object):
    ...
    def get_latest_github_released_version(self, sniff=True, validate=True, format='version', pre=False):
        ...
        if sniff:
            ...
            html = response.text
            artellalauncher.logger.debug('Parsing HTML of {} GitHub release page ...'.format(self.launcher.name.title()))
            ...
            while r:
                ...
                if 'release-timeline-tags' in r['class']:
                    for release in r.find_all(class_='release-entry', recursive=False):
                        ...
                        if validate:
                            try:
                                artellalauncher.logger.debug("Trying version {}.".format(the_version))
                                v = Version(the_version)
                                if not v.is_prerelease or pre:
                                    artellalauncher.logger.debug("Good version {}.".format(the_version))
                                    version = the_version
                                    break_out = True
                                    break
                            except InvalidVersion:
                                # move on to next thing to parse it
                                artellalauncher.logger.error("Encountered invalid version {}.".format(the_version))
                                QApplication.instance().processEvents()
                        else:
                            version = the_version
                            break
                    ...
                    artellalauncher.logger.debug("Inside formal release")
                    # formal release
                    if pre:
                        ...
                        if validate:
                            try:
                                ...
                                if not v.is_prerelease or pre:
                                    ...
                                    artellalauncher.logger.debug("Found a pre-release version: {}. Trying next.".format(the_version))
                            except InvalidVersion:
                                # move on to next thing to parse it
                                artellalauncher.logger.error("Encountered invalid version {}.".format(the_version))
                        else:
        ...
        if validate:
            try:
                ...
                artellalauncher.logger.error('Got invalid version: {}'.format(version))
                return None
    ...
    def update_tools(self):
        ...
        try:
            ...
            if need_to_update:
                ...
                tools_zip = '{}.zip'.format(tools_file)
                tools_url = '{}/archive/{}.zip'.format(repository, last_version)
                ...
                repo_folder = os.path.join(temp_path, tools_file, self._repository_folder)

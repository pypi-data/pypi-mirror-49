...
class Item(ItemBase, metaclass=ItemMetaclass, no_setup=True):
    ...
    **To configure an item class use next class variables:**
    ...
    def _genitem(self, key):
        """ Generates an item for the given relation key and sets a reference
        for reverse relationship.
        
        :param key: Relation key for which to create related item instance.
        :returns: Generated item.
        """
        item = self._cls_genitem(key)
        ...
        if reverse_key:
            if relation['relation_type'].is_many_to_x():
                self_bulk = self.Bulk()
                self_bulk.add(self)
                item[reverse_key] = self_bulk
            else:

...
def version_checker(currentversion):
    # TODO: Handle offline situations gracefully
    jsonfile = os.path.dirname(__file__) + "\\norgatedata_release_version.json" # this file is part of the package distribution
    file1 = open(jsonfile,"r")
    newversion = file1.readline()
    file1.close()
    if (newversion == '0' or time() - os.path.getmtime(jsonfile) > 60 * 60): # Check once an hour
        newversion = get_latest_pypi_version('norgatedata')
        file1 = open(jsonfile,"w")
        file1.write('"' + newversion + '"')
        file1.close()
    if (newversion > currentversion):
        warnings.warn("NorgateData:  This version (" + currentversion + ") of the norgatedata package is out-of-date.  " + newversion + " is available and is a recommended upgraded.")

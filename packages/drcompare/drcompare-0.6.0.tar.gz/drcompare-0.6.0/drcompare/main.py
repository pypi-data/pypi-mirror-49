...
def drcompare_main(inputCsvFile, dirPath):
	...
	try:	
		...
		logger.error("Failed to write row in output file: " + str(err))
	...
	for row in freader:
		...
		if row[0] == "" or row[1] == "":
			try:
				...
				logger.error("Failed to write row in output file: " + str(err))
			logger.error("Found missing data in input csv row :" + str(row))
			continue
		...
		if (not os.path.exists(imagefile1)) or (not os.path.exists(imagefile2)):
			try:
				...
				logger.error("Failed to write row in output file: " + str(err))
			logger.error("Found invalid file path(s) in input csv row :" + str(row))
			continue
		...
		try:
			...
			logger.error("Failed to open path as image: " + str(err))
			try:
				row_writer(str(row[0]), str(row[1]),str(-1),str(-1),writer)
			except Exception as err:
				logger.error("Failed to write row in output file: " + str(err))

...
def GetImageInfo(path:str, img:Image):
  """
  Get image information from image Object.
  
  :param path: image path in azure blob
  :type img: str
  :param img: image passed in
  :type img: Image object
  :return: image information with path in azure blob, mode, width, height, lightness
  :rtype: list
  """
  lightnessMean = 0
  lightnessStd = 0
  ...
  lightnessMean = int(round(lightnessMean))
  lightnessStd = int(round(lightnessStd))
  return [path, img.mode, str(img.width), str(img.height), str(lightnessMean), str(lightnessStd)]
...
def SaveImageInfo(imageRdds:list, saveDirectory:str):
  ...
  for imageRdd in imageRdds:
    imageRdd.map(lambda x : GetImageInfo(x[0],x[1])).map(lambda x: ",".join(x)).saveAsTextFile(saveDirectory + str(num) + "/")
    num += 1

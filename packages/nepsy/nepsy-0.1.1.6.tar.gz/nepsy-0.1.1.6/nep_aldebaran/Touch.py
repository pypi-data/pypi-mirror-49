            ...
            try:
                ...
                if self.Lhand_touched > 0.4:
                    ...
                    if body["left_hand"] != value:
                        data = {"node":"perception", "primitive":"touched", "input":"add", "robot":self.robot, "parameters":{"left_hand":value}}
                ...
                if self.Rhand_touched > 0.4:
                    ...
                    if body["right_hand"] != value:
                        data = {"node":"perception", "primitive":"touched", "input":"add", "robot":self.robot, "parameters":{"right_hand":value}}
                        self.sharo.send_json(data)
                ...
                if self.Head_touched > 0.4:
                    value = 1
                    if body["head"] != value:
                        data = {"node":"perception", "primitive":"touched", "input":"add", "robot":self.robot, "parameters":{"head":value}}
                        self.sharo.send_json(data)
                        body["head"] = value
                        print data
                else:

...
class Extractor(ExtractorBase):
    ...
    * :py:attr:`overrides` (optional, :py:attr:`extractor.ExtractorBase.overrides`)
    ...
    def handle_yara(self, p, match):
        ...
        :type match: List[:class:`malduck.extractor.extractor.YaraMatch`]
        """
        # Call string-based extractors
        for identifier, method_name in self.extractor_methods.items():
            for mkey, mparams in match.offsets.items():
                # Handle grouped Yara strings ($keyex1, $keyex2 will call keyex extractor)
                mmatch = re.match(r"^({})\d*$".format(identifier), mkey)
                if not mmatch:
                    # If not matched - go to next method
                    continue
                # If matched: try to unpack offset parameters
                method = getattr(self, method_name)
                for off, idx in mparams:
                    try:
                        if hasattr(method, "ext_needs_pe"):
                            # If extractor explicitly needs this and p is raw procmem: find PE for specified offset
                            p_pe = ProcessMemoryPE.from_memory(p, base=p.findmz(off)) \
                                   if not isinstance(p, ProcessMemoryPE) else p
                            method(p_pe, p.p2v(off))
                        else:
                            method(p, p.p2v(off))
    ...
    def extractor(string_or_method=None, final=False):
        """
        Decorator for extractor methods
...
class YaraMatch(object):
    ...
    def __init__(self, match):
        ...
        self.offsets = {}
        for idx, s in enumerate(self.match.strings):
            identifier = s[1].lstrip("$")
            offset = s[0]
            if identifier not in self.offsets:
                self.offsets[identifier] = []
            self.offsets[identifier].append((offset, idx))

        self.name = self.rule = self.match.rule

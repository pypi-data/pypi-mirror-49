...
class Blockchain:
    ...
    def add_block(self, block_dict):
        ...
        self._put(f'block_{key}', block_dict)  # height, block_hash -> block_dict
        self._put(f'height_{height}', block_dict['hash'])  # height -> block_hash
        self._put(f"hash_{block_dict['hash']}", height)  # block_hash -> height
        self.commit_write_batch()

    def get_block_by_height(self, height):
        block_hash = utils.bytes_to_str(self._db.get(f'height_{height}'))
        return self._get_block(height, block_hash)

    def get_block_by_hash(self, block_hash):
        height = utils.bytes_to_int(self._db.get(f'hash_{block_hash}'))
        return self._get_block(height, block_hash)

    def get_block_hash_by_height(self, height):
        return utils.bytes_to_str(self._db.get(f'height_{height}'))

    def get_block_height_by_hash(self, block_hash):
        return utils.bytes_to_int(self._db.get(f'hash_{block_hash}'))
    ...
    def print_chain(self):
        for key, value in self._db.scan(prefix='block_'):
            ...
            block_dict = utils.dump_pretty_dict(utils.load_dict(value))
    ...
    def _get_block(self, height, block_hash):
        ...
        return utils.load_dict(self._db.get(block_key))

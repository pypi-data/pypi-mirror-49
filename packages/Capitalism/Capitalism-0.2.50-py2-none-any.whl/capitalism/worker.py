...
import sys, Queue, time, threading, gc
import util
...
class Worker(threading.Thread):
    def __init__(self, target, task_queue=None, result_queue=None,
        delay=1, debug=False, name=None, exit_on_done=False,
        timeout=2, split_array=True, traceback=False, stop_flag=None):
        '''
            ...
            @param traceback:
                Whether to print traceback when print information.
            @param stop_flag:
                The thread will exit when receive stop_flag.
        '''
        ...
        if (task_queue
            ...
            self.t_q = task_queue
        # If the task queue and stop flag are valid
        if (isinstance(self.t_q, Queue.Queue)
            and stop_flag != None):
            # Try to append stop flag to task queue
            try:
                self.t_q.put(stop_flag, block=False, timeout=timeout)
            except Queue.Full:
                # Well, the task queue is currently full
                self.err('Task Queue Is Currently Full, Failed To Put Stop Flag Into It.')
    ...
    def run(self):
        ...
        while not self.should_stop:
            try:
                ...
                # When get the stop_flag
                if self.stop_flag != None and param == self.stop_flag:
                    # Try the pass the flag to result queue if result queue is valid
                    if isinstance(self.r_q, Queue.Queue):
                        try:
                            self.r_q.put(param, block=False, timeout=self.timeout)
                        except Queue.Full:
                            self.err('Result Queue Is Currently Full, Failed To Put Stop Flag Into It.')
                    # OK, as we received stop flag, we should exit
                    self.info('Received Stop Flag, Now Exiting.')
                    self.t_q.task_done()
                    break
            except Queue.Empty:
            ...
            if self.result != None and isinstance(self.r_q, Queue.Queue):
    ...
    def info(self, i):
        ...
        if not self.info_out:
            return
        util.info('[{0}]: {1}'.format(self.name, i), tb=self.tb)
    ...
    def err(self, e):
        ...
        '''
        util.err('[{0}]: {1}'.format(self.name, e), tb=self.tb)

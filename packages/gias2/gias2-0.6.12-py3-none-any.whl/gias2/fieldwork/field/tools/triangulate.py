...
class mesh_triangulator( object ):
    ...
    def draw_surface_simple( self, d, colour='jet', scalar=None, figure=None, name=None, limits=(-0.2,0.2) ):
        ...
        if figure==None:
            if name:
                return mlab.triangular_mesh( self.P[0], self.P[1], self.P[2], T, colormap=colour, scalars=scalar, name=name, vmin=limits[0], vmax=limits[1] )
            else:
                return mlab.triangular_mesh( self.P[0], self.P[1], self.P[2], T, colormap=colour, scalars=scalar, vmin=limits[0], vmax=limits[1] )
        else:
            if name:
                return mlab.triangular_mesh( self.P[0], self.P[1], self.P[2], T, colormap=colour, scalars=scalar, figure=figure, name=name, vmin=limits[0], vmax=limits[1] )
            else:
                return mlab.triangular_mesh( self.P[0], self.P[1], self.P[2], T, colormap=colour, scalars=scalar, figure=figure, vmin=limits[0], vmax=limits[1] )
    ...
    def draw_surface( self, d, plot_nodes=True, node_scale=1.0, plot_ep=False, number_ep=False ):
        ...
        # plot nodes
        if plot_nodes:
            mlab.points3d( array(self.params[0])[:,0], array(self.params[1])[:,0], array(self.params[2])[:,0], mode='sphere', scale_factor=node_scale, color=(1,0,0) )
            for i in range(len(self.params[0])):
                mlab.text( array(self.params[0])[i,0], array(self.params[1])[i,0], str(i), z=array(self.params[2])[i,0], line_width=0.001, width=0.01 )

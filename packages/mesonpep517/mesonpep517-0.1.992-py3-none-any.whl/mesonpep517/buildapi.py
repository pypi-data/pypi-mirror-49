...
def get_metadata(project, config):
    ...
    try:
        pytomlmeta = config['tool']['mesonpep517']['metadata']
    except KeyError:
        pytomlmeta = {}
...
def get_config():
    with open('pyproject.toml') as f:
        return pytoml.load(open('pyproject.toml'))
...
ABI_CHECK = """
from wheel import pep425tags 
print("{0}{1}-{2}".format(pep425tags.get_abbr_impl(), pep425tags.get_impl_ver(), pep425tags.get_abi_tag()))
"""

def get_abi(python):
    return subprocess.check_output([python, '-c', ABI_CHECK]).decode('utf-8').strip('\n')
...
class WheelBuilder:
    ...
    def build(self, wheel_directory, config_settings, metadata_dir):
        ...
        platform_tag = config.get('platforms',

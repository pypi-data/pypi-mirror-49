...
def get_key2help():
    ...
    d['mzdiff']='allowed maximal m/z difference within an isotopoologue pattern int the same sample'\
    ' to be accepted as C13-C12 isotopologue distance.'
    ...
    d['rtalign']='perform retention time alignment. This is necessary if important rt variations'\
                  ' (delta rt > peak width) between the same metabolites in different samples '\
                  'are observed. In case of most UPLC methods this is not required'  
    d['det_s2n']='True or False. If True (default), the minimal required peak intensity is based'\
                ' on spectral signal to noise. If False, the ff_metabolo common_noise_threshold_int'\
                ' parameter is applied'
    d['rm_shoulders']='Default: True. Removes artefact peaks from Orbitrap FTMS data'
    d['max_mz_diff']='parameter of the OPENMS based emzed rt align module: max allowed difference'\
                       ' in mz values for super imposer.'
                       
    d['max_mz_diff_pair']='parameter of the OPENMS based emzed rt align module: max allowed difference'\
                       ' in mz values for pair finding.'
    d['max_rt_diff']='parameter of the OPENMS based emzed rt align module: max allowed difference'\
                       ' in rt values for searching matching features.'
    d['destination']='The folder in which retention time alignment plots will be saved.'
    return d
...
PRINCIPLE: The application allows untargeted extraction of metabolites based on isotope dilution 
mass spectrometry (IDMS) method. For each metabolite a pair of an unlabeled and an 
uniformly 13C labeled [U-13C] isotopologue is present in the same sample, since the sample is a 
mixture of a natural labeled (n. l.) and [U-13C] labeled cell extract. 
...
mz.c12 is the m/z of 12C isotope, mz.C13 equals the m/z of the 13C isotope, 
and z is the charge state of the ion. 
In addition, untargeted metabolite extraction is not only based on analysis of an IDMS sample, but also on separate LC-MS analysis of the corresponding n. l. and the [U-13C] samples, respectively. Separate analysis 
allows mapping of the corresponding isotopologue peaks in the IDMS sample which
significantly enhances grouping of the unlabeled and [U-13C] labeled isotopologue peak pairs. 
Since the number of carbon atoms can be determined from the isotopologue mass distances, the approach not only significantly reduces the number of false positive metabolites, but also enhances metabolite identification.

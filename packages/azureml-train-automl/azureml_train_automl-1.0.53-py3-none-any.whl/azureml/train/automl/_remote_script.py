...
def _load_transformed_data_context_from_cache(
    ...
    if automl_settings_obj is not None and automl_settings_obj.enable_cache and \
            automl_settings_obj.preprocess and cache_store is not None:
        try:
            start = time.time()
...
def driver_wrapper(
    ...
    try:
        ...
        onnx_mdl_desc = {'ParentRunId': parent_run_id}
        if transformed_data_context is None or cache_store is None:
            ...
            # No cached data, init the ONNX converter with raw data.
            ...
            # Initialize the ONNX converter with cached converter metadata, or the raw x_train data.
        ...
        if onnx_cvt.is_initialized():
            ...
            logger.info('Failed to initializ ONNX converter for run {}.'.format(run_id))

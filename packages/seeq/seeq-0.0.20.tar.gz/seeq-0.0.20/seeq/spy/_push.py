...
def _push_metadata(metadata, workbook_id, datasource_output, archive, errors):
    ...
    for index, row in metadata_df.iterrows():
        try:
            ...
            if 'Signal' in _type:
                signal_input = SignalWithIdInputV1()
                ...
                _add_no_dupe(put_signals_input.signals, signal_input)
                ...
                _add_no_dupe(condition_batch_input.conditions, condition_input)
        ...
        if count % batch_size == 0:
            ...
            _flush(put_signals_input, condition_batch_input, asset_batch_input, tree_batch_input, push_results_df,
                   count, total)

    _flush(put_signals_input, condition_batch_input, asset_batch_input, tree_batch_input, push_results_df, count, total)
...
def reify_workbook(workbook_path, create=True):
    ...
    for i in range(0, len(workbook_path)):
        ...
        if parent_id:
            folders = folders_api.get_folders(folder_id=parent_id,
                                              limit=10000)  # type: FolderOutputListV1
        else:
            folders = folders_api.get_folders(limit=10000)  # type: FolderOutputListV1
...
def _flush(put_signals_input, condition_batch_input, asset_batch_input, tree_batch_input, push_results_df, count,
           total):
    signals_api = SignalsApi(_login.client)
    conditions_api = ConditionsApi(_login.client)
    ...
    print('(%d/%d) Pushing %d signals, %d conditions, %d assets, %d relationships' % \
          (count, total,
           len(put_signals_input.signals),
           len(condition_batch_input.conditions),
           len(asset_batch_input.assets),
           len(tree_batch_input.relationships)))
...
def _reify_path(path, workbook_id, datasource_output, scoped_data_id, cache, roots, asset_batch_input,
    ...
    for path_item in path_items:
        ...
        if asset_input.data_id not in cache:
            ...
            _add_no_dupe(asset_batch_input.assets, asset_input)
    ...
    _add_no_dupe(tree_batch_input.relationships, tree_input, 'child_data_id')

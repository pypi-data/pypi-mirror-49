...
class Resource:
    ...
    def instance_location(self) -> str:
        ...
        for key in [key for key in self._keys if key in key_diff]:
            try:
                key_vals.append(urllib.parse.quote_plus(operator.attrgetter(key)(representative)))
            except AttributeError:
                pass
    ...
    def _get_collection(
        ...
        Raises:
            `netapp_ontap.error.NetAppRestError`: If there is no connection
                available to use either passed in or on the library.
        """
    ...
    @classmethod
    @utils.api
    def _find(cls, *args, connection: HostConnection = None, **kwargs) -> "Resource":
        ...
        Raises:
            `netapp_ontap.error.NetAppRestError`: If the API call  did not return
                exactly 1 matching resource.
        """
        results = list(
    ...
    def _action(
        ...
        **kwargs
        ...
        if body is not None:
            ...
            body_data = {}
        response = self._session.post(url, json=body_data, params=kwargs)
        response.raise_for_status()
    ...
    def _get_changed_data(
        ...
        Args:
            ...
            only: If provided, only these fields will be considered when looking
                for postable fields in the object.
            check_keys: If an object is being patched, its own keys cannot change
                and so we will usually not bother checking. However, if a sub-object
                is changing for an object, then the keys matter because it means
                we are trying to swap out the sub-object.
        ...
        for field in schema.fields:
            ...
            if not isinstance(schema.fields[field], fields.Nested):
                # if field is read-only, it can't be changed so we can ignore
                if field not in schema.patchable_fields:
                    continue
                if not check_keys and field in self._keys:
                    continue
        ...
        return _filter_by_fields(changed_data, only=only)

    def _get_postable_data(
        self,
        only: List[str] = None,
        child_keys: bool = False
    ) -> dict:
        ...
        Args:
            ...
            child_keys: If True, the key values of a the current Resource will
                be considederd POSTable regardless of their documentation.
        ...
        for field in schema.fields:
            current_value = getattr(self, field, None)
            if not isinstance(schema.fields[field], fields.Nested):
                # if field is not postable, ignore it
                if field not in schema.postable_fields:
                    if not child_keys or not field in self._keys:
                        continue
                current_value = schema.fields[field].serialize(field, self)
            if current_value is not None and current_value != missing:
...
def _get_postable_value(
    ...
    """Return the current value of the field. If the value is a dict, recursively
    ...
    if isinstance(current_value, Resource):
        current_value = current_value._get_postable_data(  # pylint: disable=protected-access
            only=schema.fields[field].only, child_keys=True
        )
...
def _get_current_value(
    ...
    if isinstance(current_value, Resource):
        if previous_value is None:
            current_value = _filter_by_fields(
                current_value.to_dict(),
                only=[
                    key
                    for key in current_value._keys  # pylint: disable=protected-access
        ...
        for value in current_value:
            if isinstance(value, Resource):
                changed = value._get_changed_data(  # pylint: disable=protected-access
                    only=schema.fields[field].only, check_keys=True
                )
                if changed:
                    changed_items.append(changed)
            else:
...
def _filter_by_fields(data, only: List[str] = None) -> dict:
    """Return a dictionary from the input which only had the requested fields.

    Args:
        data: The original dictionary
        only: A list of fields. If provided, only these fields will be in the
            returned data set.

    Returns:
        A dictionary which is a subset of the original.
    """

    if not only:
        only = list(data.keys())

    return {key: value for key, value in data.items() if key in only}

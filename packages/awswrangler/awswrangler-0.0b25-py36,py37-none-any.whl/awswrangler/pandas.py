            ...
            for ini, end in bounders:
                ...
                if count == 1:  # first chunk
                    last_char = Pandas._find_terminator(
                        ...
                        quotechar=quotechar,
                        lineterminator=lineterminator)
                    forgotten_bytes = chunk_size - last_char
    ...
    def _find_terminator(body, quoting, quotechar, lineterminator):
        try:
            if quoting == csv.QUOTE_ALL:
                token = f"{quotechar}{lineterminator}{quotechar}"
                index = body.rindex(token) + len(quotechar)
                while True:
                    right = False
                    if index + len(quotechar) + 1 <= len(body) - 1:
                        if body[index + len(quotechar) + 1] == ",":
                            right = True
                        else:
                            right = False
                    if index - len(quotechar) - 1 >= 0:
                        if body[index - len(quotechar) - 1] == ",":
                            left = True
                        else:
                            left = False
                    elif index - len(quotechar) >= 0:
                        left = False
                    else:
                        raise LineTerminatorNotFound()
                    if not right and not left:
                        break
                    index = body[:index].rindex(token) + len(quotechar)
            else:

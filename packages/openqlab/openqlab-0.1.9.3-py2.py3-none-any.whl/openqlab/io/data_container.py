...
def wrapper_factory(func, base):
    if func in base.__dict__:
        ...
        raise KeyError('No function called \'{0}\' in original functions from {1}'.format(func, base))
...
class MetaDataContainer(type):
    ...
    reindexing = ['add_prefix', 'add_suffix', 'at_time', 'between_time', 'drop', 'drop_duplicates', 'filter', 'first',
                  'last', 'reindex', 'reindex_like', 'rename', 'rename_axis', 'reset_index',
...
class DataContainer(pd.DataFrame, metaclass=MetaDataContainer):
    ...
    def __init__(self, *args, header=None, type=None, **kwargs):  # pylint: disable=redefined-builtin

...
PSINFO_T = struct.Struct("iiiIIIIIIIILLLLHHLLLLLL16s80siiLLciILLcccchi8sLLIIIIII")

MAP_T = struct.Struct("LL64sQiiii")
    ...
    def iter_region(
        ...
        with open("/proc/%d/map" % (self.pid)) as maps_file:
            while True:
                ...
                start, size, _, _, flags, _, _, _ = MAP_T.unpack(mapping)
                ...
                if optimisations:
                    if "i" in optimisations and not flags & MA_ANON:
                        continue
                    if "s" in optimisations and flags & MA_SHM:
                        continue
                    # in sunos it's quite common when this flag is set, so let's use other letter
                    if "X" in optimisations and flags & MA_EXEC:
                        continue
                    if "r" in optimisations and not flags & MA_WRITE:
                        continue

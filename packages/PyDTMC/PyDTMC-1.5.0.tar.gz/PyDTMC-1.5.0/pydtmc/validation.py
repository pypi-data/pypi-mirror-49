...
def validate_states(states: _Any, current_states: _List[str], stype: str, flex: bool) -> _List[int]:
    ...
    if (stype != 'walk') and (len(set(states)) < states_length):
        raise ValueError('The "@arg@" parameter must contain only unique values.')
...
def validate_vector(vector: _Any, vtype: str, flex: bool, size: _Optional[int] = None) -> _np.ndarray:

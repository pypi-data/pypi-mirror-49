...
def textToAscii(content):
    ...
    ----------
        Array
    '''
    Logger.printMessage(message='{methodName}'.format(methodName='textToAscii'), description='Length: {length} - {content} ...'.format(length=len(content), content=content[0:10]), debug_module=True)
...
def asciiToHex(content):
    ...
    ----------
        Array
    '''
    Logger.printMessage(message='{methodName}'.format(methodName='AsciiToHex'), description='Length: {length} - {content} ...'.format(length=len(content), content=content[0:10]), debug_module=True)
...
def hexToBase64(content):
    ...
    ----------
        Array
    '''
    Logger.printMessage(message='{methodName}'.format(methodName='HexToBase64'), description='Length: {length} - {content} ...'.format(length=len(content), content=content[0:10]), debug_module=True)
...
def joinBase64(content):
    ...
    ----------
        String
    '''
    Logger.printMessage(message='{methodName}'.format(methodName='joinBase64'), description='Length: {length} - {content} ...'.format(length=len(content), content=content[0:10]), debug_module=True)
    return (''.join([mensaje[i].decode('utf-8') for i in range(0, len(mensaje))])).encode()
...
def asciiToBase64(content):
    ...
    ----------
        Array
    '''
    Logger.printMessage(message='{methodName}'.format(methodName='asciiToBase64'), description='Length: {length} - {content} ...'.format(length=len(content), content=content[0:10]), debug_module=True)
    return [content[i:i+4] for i in range(0, len(content), 4)]

def base64ToHex(content):
    Logger.printMessage(message='{methodName}'.format(methodName='base64ToHex'), description='Length: {length} - {content} ...'.format(length=len(content), mensaje=content[0:10]), debug_module=True)
    return [base64.b64decode(b64) for b64 in content]

def hexToDecimal(content):
    Logger.printMessage(message='{methodName}'.format(methodName='hexToDecimal'), description='Length: {length} - {content} ...'.format(length=len(content), mensaje=content[0:10]), debug_module=True)
    return [int(hexa.decode("UTF-8"), 16) for hexa in content]

def decimalToAscii(content):
    Logger.printMessage(message='{methodName}'.format(methodName='decimalToAscii'), description='Length: {length} - {content} ...'.format(length=len(content), mensaje=content[0:10]), debug_module=True)

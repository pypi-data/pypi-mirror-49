...
def main(prog=None,args=None):
    global PROJECT_ADDRESS, PROJECT_BRANCH, TEST_PLATFORM, NET_ENV, PROJECT_VERSION, PROJECT_BUILD, NOTIFICATION_EMAILS, PACKAGE_BRANCH, DEV_BRANCH, LOG_OR_NOT,TAG_NAME,PACKAGE_ORDER, CACHE_PATH, CERT_PASSWORD
    #处理系统方法
    sys_action(sys.argv)  

    args = sys.argv[2:]
    shortargs = 'a:p:b:e:n:d:m:gk:c:t:o:w:' #短选项模式
    longargs = ['address=', 'platform=', 'branch=', 'environment=', 'version=', 'build=', 'email=', 'log', 'package=','code=','tag=','order=','cache=','password='] #长选项模式
    try:
        try:

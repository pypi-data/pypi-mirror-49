...
from .utils.runtime import RuntimeDump, load_runtime, write_runtime
...
class Executor(object):
    ...
    def reload_config(self):
        try:
            ...
            if "host" not in self.config.keys():
    ...
    def register(self, port):
        ...
        try:
            data = {
                "type": self.config["type"],
                "tags": self.config["tags"],
                "host": self.config["host"] if "host" in self.config.keys() else "0.0.0.0",
                "port": port,
                "path": "/health" if self.health_check_options["path"] is None else self.health_check_options["path"]
            }
            if self.name is not None:
                data["name"] = self.name

            re = requests.post(self.config["controller_url"] + "/executor/register", json=data)
            response = re.json()
    ...
    @retry(wait=wait_fixed(3))
    def end_report(self, task, has_output, result, errors):
        try:
            data = {
                "type": "end",
                "task_id": task.task_id,
    ...
    def data_process(self, task, data, partition=None, offset=None):
            ...
            for output in outputs:
                ...
                if output["output_type"]:
                    afterprocessed, errors = process_data(processed, config,
                                                          ...
                                                          mode="total")
                    logger.debug("afterprocessed[%s]" % output["output_type"])
                    logger.debug(afterprocessed)
        ...
        self.end_report(task, has_output, result, error_collector.export())
    ...
    def inside_kafka_process(self):
        ...
        while True:
            ...
            try:
                ...
                task = Task(task_id=headers["task_id"], branch_id=headers["branch_id"], position=headers["current_position"],
                            data_type=headers["type"] ,properties=headers["properties"], end_condition=headers["end_conditions"])
    ...
    def init_api_process(self, port, queue):
        self.app = Flask(__name__)
        self.queue = queue
        @self.app.route(self.health_check_options['path'] or "/health", methods=['GET'])
        def health_check_function():
            headers = {
                "Content-Type": "application/json",
                "whoami": self.name
            }
            if self.health_check_options["action"] is None:
                return json.dumps({"state": True, "info": "Default health check response.."}), 200, headers
            else:
                try:
                    resp = self.health_check_options["action"]()
                    if type(resp) == str:
                        ...
                        logger.warning("Unsupported return type: %s!" % (str(type(resp))))
                        raise TypeError("Unsupported return type!")
                except:
                    logger.warning("Error in health check response:\n", exc_info=True)
                    return json.dumps({"state": True, "info": "Error in health check response"}), 200, headers

        @self.app.route("/segment", methods=["PUT"])
        def update_kafka():
            try:
                segment = request.json["segment"]
            except:
                logger.error("Error in parse params:\n", exc_info=True)
                return jsonify({"state": False, "info": "error in parsing params"})
            self.kafka_arrangement = segment
            self.queue.put_nowait(segment)
            logger.info("update kafka segments!")
            logger.debug(self.kafka_arrangement)

            return jsonify({"state": True, "info": "Success"})
    ...
    # -------------------------  Others  -------------------------------------
    def health_check(self, path=None):
        '''
    ...
    def start(self):
        ...
        initialize_logger(stderr_log_level=self.config["log_level"].upper())
        ...
        self.queue = Queue(10)
        self.api_process  = Process("api", action=self.init_api_process, args=(port, self.queue))
    ...
    def restart(self):
        # get name
        self.runtime = RuntimeDump(data={}, mode=False)

...
class DCCButton(base.BaseWidget, object):
    ...
    def ui(self):
        ...
        icon_path = artellalauncher.resource.get('icons', 'color', '{}.png'.format(dcc_name))
        if not os.path.isfile(icon_path):
            dcc_icon = artellalauncher.resource.icon('artella')
        else:
            dcc_icon = artellalauncher.resource.icon(dcc_name)
...
class DCCSelector(tpQtLib.MainWindow, object):
    ...
    def __init__(self, launcher, parent=None):
        ...
        self._main_title._button_minimized.setVisible(False)
        self._main_title._button_maximized.setVisible(False)

        self.setMinimumWidth(450)
        total_rows = int(math.ceil(self._departments['All'].count()/self.COLUMNS_COUNT))
        minimum_height = (total_rows * 105) + 210
    ...
    def _on_uninstall(self):
        ...
        if install_path and os.path.isdir(install_path):
            ...
            qtutils.show_info(self, '{} Tools uninstalled!'.format(self.launcher.name.title()), '{} Tools uninstalled successfully!'.format(self.launcher.name.title()))
    ...
    #
    #         QMessageBox.information(self, 'Close DCCs', 'Please close any opened DCC (Maya, Houdini or Nuke) beSfore continuing')

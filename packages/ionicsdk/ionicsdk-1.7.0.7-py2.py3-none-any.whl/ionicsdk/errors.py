...
This happens when a key update request is performed via ISAgent::updateKeys() or ISAgent::updateKey()
...
To remedy this error, the client must retrieve the key again from the key server (via ISAgent::getKey()
        or variants), apply the desired changes to it (via implementation of ISKeyAttributesMapMerger
        such as ISKeyAttributesMapMergerDefault), and perform another update request via 
        ISAgent::updateKeys() or ISAgent::updateKey().
...
This happens when parsing an HTTP request and discovering an unknown / unsupported
    ...
    ##      This happens when a key update request is performed via ISAgent::updateKeys() or
    ##      ISAgent::updateKey() and the key server denies the request because it detected that the client
    ...
    ##      ISAgent::getKey() or variants), apply the desired changes to it (via implementation of
    ##      ISKeyAttributesMapMerger such as ISKeyAttributesMapMergerDefault), and perform another
    ##      update request via ISAgent::updateKeys() or ISAgent::updateKey().
    AGENT_STALE_KEY_ATTRIBUTES      = 40031
    ...
    ##      This happens when parsing an HTTP request and discovering an unknown / unsupportedHTTP method.

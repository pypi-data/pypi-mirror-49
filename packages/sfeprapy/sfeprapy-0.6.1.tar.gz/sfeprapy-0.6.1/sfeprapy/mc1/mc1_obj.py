...
class MonteCarloCase:
    ...
    def __init__(self, path_wd: str, name: str, func: (types.FunctionType, types.BuiltinFunctionType)):
    ...
    def in2sc(self):
        ...
        fire_time = np.arange(0, input_param['fire_time_duration'] + input_param['fire_time_step'],
        ...
        df_mc_params.index.name = ['index']  # todo is this correct? shouldn't it be a str?
...
class MonteCarlo:
    ...
    def select_input_file(self, path_master_csv:str=None):
        ...
        if path_master_csv is None:
            ...
            self.path_master_csv = os.path.realpath(path_master_csv.replace('"', '').replace("'", ''))
    ...
    def out_combined_T(self):
        ...
        for case in self.monte_carlo_cases:
            if case.is_live:
                dict_res[case.name] = case.mc_results_get('solver_steel_temperature_solved')

from . import gcjs,ggzy,zfcg,qycg,zlsrc
import os
...
def write_all(dirname,**krg):
    ...
    if not os.path.exists(os.path.join(dirname,'zlsrc')):
        os.mkdir(os.path.join(dirname,'zlsrc'))
    qycg.write_dags(os.path.join(dirname,'zlsrc'),**krg)
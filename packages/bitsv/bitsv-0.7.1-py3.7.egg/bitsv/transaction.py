...
MESSAGE_LIMIT = 100000  # Actual message needs to take account of +1 for OP_RETURN and +5 for op_pushdata4 etc.

# FIXME - need to add checks for 100kb total transaction size
FINAL_TX_SIZE_LIMIT = 100000  # Beware 100kb total tx size limit (obviates MESSAGE_LIMIT). Tested with BitIndexAPI.
...
def sanitize_tx_data(unspents, outputs, fee, leftover, combine=True, message=None, compressed=True, custom_pushdata=False):
    ...
    if message and (custom_pushdata is False):
        ...
        if len(message) >= MESSAGE_LIMIT:
            ...
            raise ValueError("Message lenth: {}. Cannot exceed {} bytes with custom_pushdata.".format(len(message), MESSAGE_LIMIT))
        else:
            messages.append((message, 0))

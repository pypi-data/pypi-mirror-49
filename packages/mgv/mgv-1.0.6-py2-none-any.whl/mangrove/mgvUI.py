            ...
            if len(path):
                if not os.path.exists(path):
                    ...
                    try:
                        ...
                        self.actionWindow.parent.gui.notify('Can\'t create "%s" !', 1)
...
class MgvRename(object):
    ...
    def __init__(self, editor):
        ...
        QtWidgets.QWidget.connect(self.root.nameEdit, QtCore.SIGNAL('textChanged(QString)'),
                              lambda a, x=self.root.treeViewNew: self.refresh(x))
        QtWidgets.QWidget.connect(self.root.cancelButton, QtCore.SIGNAL('clicked()'), self.cancel)
        QtWidgets.QWidget.connect(self.root.applyButton, QtCore.SIGNAL('clicked()'), self.apply)
    ...
    def refreshElement(self, element, mode):
        if isinstance(element, basestring):
            ...
            if mode:
                ...
                if newname != name:
                    item = QtWidgets.QStandardItem(newname)
                    item.setForeground(QtCore.Qt.red)
                    return item
            return QtWidgets.QStandardItem(name)
        if len(element):
            name = os.path.basename(element[0])
            parent = QtWidgets.QStandardItem(name)
            if mode:
                newname = name.replace(str(self.root.nameLabel.text()), str(self.root.nameEdit.text()))
                if newname != name:
                    parent = QtWidgets.QStandardItem(newname)
                    parent.setForeground(QtCore.Qt.red)
            if len(element) > 1:
                for sub in element[1:]:
                    parent.appendRow(self.refreshElement(sub, mode))
            return parent
        return QtWidgets.QStandardItem()
            ...
            for param in self.node.type.getParameters(self.node.versionActive.typeForceVersion):
                if param.visibility:
                    ...
                    if param.type == 'float':
                        ...
                        QtWidgets.QWidget.connect(edit, QtCore.SIGNAL('valueChanged(double)'),
...
class MgvVariableWidget(object):
    ...
    def __init__(self, gui, ver, layout, variable):
        ...
        QtWidgets.QWidget.connect(self.widget.delButton, QtCore.SIGNAL('clicked()'), self.delete)
        self.toggle(save=False)
        QtWidgets.QWidget.connect(self.widget.checkBox, QtCore.SIGNAL('stateChanged(int)'), lambda a: self.toggle())
        QtWidgets.QWidget.connect(self.widget.nameEdit, QtCore.SIGNAL('textChanged(QString)'),
                              lambda a: self.nameChanged(a))
        QtWidgets.QWidget.connect(self.widget.valueEdit, QtCore.SIGNAL('textChanged(QString)'),

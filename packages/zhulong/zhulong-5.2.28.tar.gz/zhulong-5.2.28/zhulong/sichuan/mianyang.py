...
def f1(driver, num):
    ...
    elif 'http://egpmall.my.gov.cn/Style/MianYang_New/' in url:
        df = f1_4(driver, num)
        return df
...
def f2(driver):
    ...
    if ('http://ztb.my.gov.cn/ceinwz/WebInfo_List.aspx?' in url) or ('http://caigou.my.gov.cn/ceinwz/WebInfo_List.aspx?' in url):
        ...
        try:
            ...
            num = 1
    elif 'http://egpmall.my.gov.cn/Style/MianYang_New/NoticeList.aspx' in url:
        t = 0
        try:
            while True:
                t += 1
...
def f3(driver, url):
    driver.get(url)
    if 'http://egpmall.my.gov.cn/' in url:
        # driver.get(url)
        locator = (By.CLASS_NAME, "row")
        ...
        return div
    elif 'http://egpsys.my.gov.cn/' in url:
        try:
            # driver.get(url)
    ...
    if '/ceinwz/admin_show.aspx' in url:
        # driver.get(url)
        locator = (By.CLASS_NAME, "newsImage")
...
data = [
    ...
    ["zfcg_zhaobiao_jinjia_gg",
     "http://egpmall.my.gov.cn/Style/MianYang_New/NoticeList.aspx",
    ...
    ["zfcg_yuyanshou_jinjia_gg",
     "http://egpmall.my.gov.cn/Style/MianYang_New/NoticeList.aspx",
     ["name", "ggstart_time", "href", "info"], zfcg_yanshouyugg(f1), zfcg_yanshouyugg(f2)],

    ["zfcg_yanshou_jinjia_gg",
     "http://egpmall.my.gov.cn/Style/MianYang_New/NoticeList.aspx",
     ["name", "ggstart_time", "href", "info"], zfcg_yanshou(f1), zfcg_yanshou(f2)],
...
if __name__=='__main__':
    ...
    # driver=webdriver.Chrome()
    # url = "http://ztb.my.gov.cn/ceinwz/WebInfo_List.aspx?newsid=200,206,212,218,224,230,236,242,248,254&jsgc=0000000&zfcg=&tdjy=&cqjy=&qtjy=&PubDateSort=0&ShowPre=1&CbsZgys=0&zbfs=&qxxx=0&showqxname=0&NewsShowPre=1&wsjj=0&showCgr=1&ShowOverDate=0&showdate=1&FromUrl=sbcllzb11"
    # driver.get(url)
    # # df = f3(driver, url)
    # df = f2(driver)
    # print(df)
    #
    # driver=webdriver.Chrome()
    # url = "http://ztb.my.gov.cn/ceinwz/WebInfo_List.aspx?newsid=100,106,112,118,124,130,136,142,148,154&jsgc=0100000&zfcg=&tdjy=&cqjy=&qtjy=&PubDateSort=0&ShowPre=1&CbsZgys=0&zbfs=&qxxx=0&showqxname=0&NewsShowPre=1&wsjj=0&showCgr=0&ShowOverDate=0&showdate=1&FromUrl=jsgcsgjl"
    # driver.get(url)

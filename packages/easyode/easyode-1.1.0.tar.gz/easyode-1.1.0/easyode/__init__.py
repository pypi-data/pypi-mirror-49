...
class EquaSys() :
    '''Create a system of equadiff'''
    def __init__(self,Equadiff) :
        self.Equadiffs = Equadiff
    
    def Graph3D(self,T, deg=0, IsAxEqual=False) :
        '''Plot 3D representation of the system
        Arg :
            -T : array numpy | parameter interval where the function has to be calculated
            -deg : int | derivation degree that you want to plot'''

        Sol = []
        for Equa in self.Equadiffs :
            Sol.append(inte.odeint(Equa.DerivFunc(),Equa.InitialCond,T))

        fig = plt.figure()
        ax = fig.add_subplot(111, projection='3d')

        ax.plot(Sol[0][:,deg],Sol[1][:,deg],T)
        if IsAxEqual :
            ax.axis('equal')

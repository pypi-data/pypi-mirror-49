...
class IMAMResource(Interface):
    ...
    def onArchiveRequest(self, mam):
        ...
        @type mam: L{MAMRequest<wokkel.mam.MAMRequest>}
...
class MAMService(subprotocols.XMPPHandler, subprotocols.IQHandlerMixin):
    ...
    _mainFilters = {'start': {'fieldType': 'text-single',
                                'var': 'start',
                                'label': 'Starting time',
    ...
    def _onArchiveRequest(self, iq):
        ...
        This will send the list of archived message and return MAM <fin> element
        @return: The MAM <fin> element
        @rtype: L{Element<twisted.words.xish.domish.Element>}
        """
        iq.handled = True
        ...
        def cb(result):
            msg_data, rsm_elt, attributes = result

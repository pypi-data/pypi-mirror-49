...
SVMProblem read_problem(const char *filename, double bias, int64_t *error_code)
{
	int64_t max_index, inst_max_index, i;
	int64_t elements, j;
	...
	*error_code = 0;
	...
	for(i=0;i<prob.l;i++)
	{
		...
		while(1)
		{
			...
			errno = 0;
			x_space[j].index = (int64_t)strtoll(idx,&endptr,10);
...
void merge_problems(const char *srcs[], const int num_srcs, int64_t* offsets, const char *output_filename, char training, int64_t *error_code) { 
	int i, j;
	const double bias = -1;

...
## Overview
AutoSupport is NetApp's *call home* mechanism. AutoSupport sends configuration details, status details, and error reporting details to NetApp.<p/>
This endpoint supports both GET and PATCH calls. GET is used to retrieve AutoSupport configuration details for the cluster and PATCH is used to modify the AutoSupport configuration of the cluster. GET calls can also be used to check AutoSupport connectivity.
---
## Examples
...
The following example retrieves AutoSupport connectivity issues for the cluster. The `fields=issues` parameter must be specified, for the response to return any connectivity issues. The `corrective_action` section might contain commands which needs to be executed on the ONTAP CLI.<p/>
...
Example: true """
    from_ = fields.Str()
    r""" The email address from which the AutoSupport messages are sent.
...
Example: true """
    issues = fields.List(fields.Nested("AutosupportIssuesSchema", unknown=EXCLUDE))
 ...
 """
    mail_hosts = fields.List(fields.Str)
    r""" The name of the mail server(s) used to deliver AutoSupport messages via SMTP.

Example: ["mailhost1.example.com","mailhost2.example.com"] """
    partner_addresses = fields.List(fields.Str)
    r""" The list of partner address(es).
...
Example: https://proxy.company.com """
    ...
    r""" The email address(es) to which the AutoSupport messages are sent.

Example: ["example1@netapp.com","example2@netapp.com"] """
    transport = fields.Str(validate=enum_validation(['smtp', 'http', 'https']))
...
class Autosupport(Resource):
    ...
    get.__doc__ = r"""Retrieves the AutoSupport configuration of the cluster and if requested, returns any connectivity issues with the AutoSupport configuration.<p/>
</br>Important note:
...
"""
    ...
    patch.__doc__ = r"""Updates the AutoSupport configuration for the entire cluster.
### Related ONTAP commands

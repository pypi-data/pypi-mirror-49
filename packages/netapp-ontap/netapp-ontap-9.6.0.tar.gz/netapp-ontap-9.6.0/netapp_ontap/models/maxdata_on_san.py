# pylint: disable=trailing-newlines
# pylint: disable=line-too-long
# pylint: disable=too-many-lines
# pylint: disable=unused-import
# pylint: disable=invalid-name

r"""
Copyright &copy; 2019 NetApp Inc.
All rights reserved.


"""

from marshmallow import EXCLUDE, fields  # type: ignore
from netapp_ontap.resource import Resource, ResourceSchema


__all__ = ["MaxdataOnSan", "MaxdataOnSanSchema"]
__pdoc__ = {
    "MaxdataOnSanSchema.resource": False,
    "MaxdataOnSan": False,
}


class MaxdataOnSanSchema(ResourceSchema):
    """The fields of the MaxdataOnSan object"""

    app_type = fields.Str()
    r""" Type of the application that is being deployed on the L2. Required in the POST body

Valid choices:

* mongodb
* oracle
* san """
    application_components = fields.Nested("MaxdataOnSanApplicationComponentsSchema", unknown=EXCLUDE, many=True)
    r""" application-components. Optional in the POST or PATCH body
 """
    metadata = fields.Nested("MaxdataOnSanMetadataSchema", unknown=EXCLUDE, many=True)
    r""" The metadata field of the maxdata_on_san.
 """
    new_igroups = fields.Nested("MaxdataOnSanNewIgroupsSchema", unknown=EXCLUDE, many=True)
    r""" The list of initiator groups to create. Optional in the POST or PATCH body
 """
    ocsm_url = fields.Str()
    r""" The OnCommand System Manager URL for this application
 """
    os_type = fields.Str()
    r""" The name of the host OS running the application. Required in the POST body

Valid choices:

* aix
* hpux
* hyper_v
* linux
* netware
* openvms
* solaris
* solaris_efi
* vmware
* windows
* windows_2008
* windows_gpt
* xen """

    @property
    def resource(self):
        return MaxdataOnSan

    @property
    def patchable_fields(self):
        return [
        ]

    @property
    def postable_fields(self):
        return [
            "app_type",
            "application_components",
            "metadata",
            "new_igroups",
            "os_type",
        ]


class MaxdataOnSan(Resource):  # pylint: disable=missing-docstring

    _schema = MaxdataOnSanSchema

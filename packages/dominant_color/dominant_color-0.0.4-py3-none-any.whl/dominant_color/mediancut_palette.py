...
def mediancut_palette(image, bins=5, quality=10):
    ...
    palette = MMCQ.quantize(valid_pixels, bins)
    total_volume = sum(volume for color, volume in palette)
    return [(100 * volume / total_volume, color) for color, volume in palette]
...
class MMCQ(object):
    ...
    def quantize(pixels, max_color):
        ...
        pq2.sort()
        return list(
            reversed([(vbox.avg, vbox.count * vbox.volume) for vbox in pq2.contents])
        )

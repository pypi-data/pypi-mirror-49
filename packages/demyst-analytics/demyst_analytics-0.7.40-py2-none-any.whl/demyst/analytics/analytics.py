...
class Analytics(object):
    ...
    def __init__(self, inputs=None, config_file=None, region=None, env=None, key=None, username=None, password=None, jwt=None):
        ...
        self.print_startup_trial_message()
    ...
    def search(self, inputs=None, tags=None, view="html", strict=False):
        if isinstance(inputs, pd.DataFrame):
            inputs = list(inputs)
        return track_function(lambda: self.do_search(inputs=inputs, tags=tags, view=view, strict=strict),
                              self.C, "API : Python : Search",
                              { "tags": tags, "view": view, "inputs": inputs })

    def do_search(self, inputs=None, tags=None, view="html", strict=False):
        params = {
            'inputs[]': inputs,
            'tags[]': tags
        }
        ...
        if (view == "html"):
            ...
            raise RuntimeError("View must be html or json.")
    ...
    def validate(self, inputs=None, providers=None, notebook=True):
        ...
        if providers is None:
            providers = []
        validation = Validation(inputs, providers, self.C)
        if notebook:
            return validation
        else:
    ...
    def credits(self):
        ...
        self.print_credits_trial_message(balance)
        return balance
    ...
    def trial_period_end(self):
        headers = { "Accept": "application/json" }
        resp_json = self.C.auth_get(self.C.get("MANTA_URL") + "organization.json", headers=headers).json()
    ...
    def print_startup_trial_message(self):
        end_date = self.trial_period_end()
        if end_date:
            print("Your trial period ends on " + end_date + ". Contact client@demystdata.com to get started.")

    def print_credits_trial_message(self, balance):
        end_date = self.trial_period_end()
        if end_date:

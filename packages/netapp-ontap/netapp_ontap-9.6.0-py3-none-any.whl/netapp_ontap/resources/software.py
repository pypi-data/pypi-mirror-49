...
<br/>The client can start the upgrade process at the cluster-level. There are no options available to start the upgrade for a specific node or HA pairs.
---
...
```
The client can check the update progress information in the GET /cluster/software endpoint. The following example shows how to check the progress of an update after setting the `skip_warnings` field to `true`.      <br/>
```
# The API:
/api/cluster/software
# The call:
...
class SoftwareSchema(ResourceSchema):
 ...
 """
    action = fields.Str(validate=enum_validation(['pause', 'cancel', 'resume']))
...
Example: 5220 """
    metrocluster = fields.Nested("SoftwareReferenceMetroclusterSchema", unknown=EXCLUDE)
    r""" The metrocluster field of the software.
 """
    nodes = fields.List(fields.Nested("SoftwareNodeSchema", unknown=EXCLUDE))
...
Example: ONTAP_X_1 """
    state = fields.Str(validate=enum_validation(['in_progress', 'waiting', 'paused_by_user', 'paused_on_error', 'completed', 'canceled', 'failed', 'pause_pending', 'cancel_pending']))
...
* pause_pending
* cancel_pending """
    status_details = fields.List(fields.Nested("SoftwareStatusDetailsSchema", unknown=EXCLUDE))
    r""" Display status details.
 """
    update_details = fields.List(fields.Nested("SoftwareUpdateDetailsSchema", unknown=EXCLUDE))
    r""" Display update procress details.
 """
    validation_results = fields.List(fields.Nested("SoftwareValidationSchema", unknown=EXCLUDE))
...
class Software(Resource):
    ...
    get.__doc__ = r"""Retrieves the software profile of a cluster.
...
"""
    ...
    patch.__doc__ = r"""Upgrades the cluster software version. Setting `version`  triggers the installation of the package to start. To validate the package for installation but not perform the installation, use the `validate_only` field on request.
Important note:
    ...
    def download(
        ...
        **kwargs
        ...
        return super()._action(
            "download", body=body, poll=poll, poll_interval=poll_interval,
            poll_timeout=poll_timeout, **kwargs
        )

    download.__doc__ = r"""Downloads a software package from the server.
### Required properties

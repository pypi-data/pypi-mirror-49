...
class LensDataStack(object):
    def __init__(
        ...
        self.images_1d = list(
            map(
                lambda image, mask: mask.array_1d_from_array_2d(array_2d=image),
                self.images,
                self.masks,
            )
        )

        self.noise_maps_1d = list(
            map(
                lambda noise_map, mask: mask.array_1d_from_array_2d(array_2d=noise_map),
                self.noise_maps,
                self.masks,
            )
        )
        ...
        self.masks_1d = list(
            map(lambda mask: mask.array_1d_from_array_2d(array_2d=mask), self.masks)
        )
        ...
        self.convolvers_image = list(
            map(
                lambda psf, mask: convolution.ConvolverImage(
                    mask=mask,
        ...
        self.convolvers_mapping_matrix = list(
            map(
                lambda psf, mask: inversion_convolution.ConvolverMappingMatrix(
                    mask=mask,
                    psf=psf.resized_scaled_array_from_array(
                        new_shape=mapping_matrix_psf_shape
                    ),
                ),
                self.psfs,
                self.masks,
        ...
        self.grid_stacks = list(
            map(
                lambda mask: grids.GridStack.grid_stack_from_mask_sub_grid_size_and_psf_shape(
                    mask=mask, sub_grid_size=sub_grid_size, psf_shape=image_psf_shape
                ),
                self.masks,
            )
        )

        self.padded_grid_stacks = list(
            map(
                lambda mask: grids.GridStack.padded_grid_stack_from_psf_shape(
                    mask=mask, sub_grid_size=sub_grid_size, psf_shape=image_psf_shape
                ),
                self.masks,
            )
        ...
        self.borders = list(
            map(lambda mask: grids.RegularGridBorder.from_mask(mask=mask), self.masks)
        )
    ...
    def map_to_scaled_arrays(self):
        return list(
            map(
                lambda grid_stack: grid_stack.regular.scaled_array_2d_from_array_1d,
                self.grid_stacks,
            )
        )
...
class LensHyperDataStack(LensDataStack):
    def __init__(
        ...
        self.hyper_model_images_1d = list(
            map(
                lambda mask, hyper_model_image: mask.array_1d_from_array_2d(
                    array_2d=hyper_model_image
                ),
                self.masks,
                self.hyper_model_images,
            )
        )
        ...
        for image_index in range(len(self.hyper_galaxy_images_stack)):
            ...
            self.hyper_galaxy_images_1d_stack.append(
                list(
                    map(
                        lambda mask, hyper_galaxy_image_stack: mask.array_1d_from_array_2d(
                            array_2d=hyper_galaxy_image_stack
                        ),
                        self.masks,
                        self.hyper_galaxy_images_stack[image_index],
                    )
                )
            )

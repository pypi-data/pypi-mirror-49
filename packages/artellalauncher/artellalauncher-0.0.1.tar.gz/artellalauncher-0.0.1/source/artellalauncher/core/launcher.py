...
from artellalauncher.core import defines, config, updater
from artellalauncher.widgets import console, dccselector
from artellapipe.core import artellalib


class DccData(object):
    def __init__(self, name, enabled, default_version, supported_versions, installation_paths, departments, plugins):
...
class ArtellaLauncher(QObject, object):
    ...
    def init(self):
        ...
        self._config = config.create_config(launcher_name=self.name, console=None, window=self, dcc_install_path=None)
        self._updater = self.UPDATER_CLASS(launcher=self)

        dcc_selector = dccselector.DCCSelector(launcher=self)
    ...
    def init_config(self):
        ...
        if not self._dccs:
            artellalauncher.logger.warning('No DCCs enabled!')
            return
        ...
        for dcc_name, dcc_data in self._dccs.items():
            ...
            try:
                dcc_module = importlib.import_module('{}dcc'.format(dcc_name.lower().replace(' ', '')), 'artelladccs')
            except ImportError:
                continue
    ...
    def _on_dcc_selected(self, selected_dcc, selected_version):
        ...
        QApplication.instance().processEvents()
        cfg = config.create_config(launcher_name=self.name, console=self._console, window=self, dcc_install_path=installation_path)
        if not cfg:
            self._splash.close()
            self._console.close()
        ...
        if not install_path or not os.path.exists(install_path):
            ...
            install_path = os.path.abspath(self.set_installation_path())
            if not install_path:
                sys.exit()

            install_path = path_utils.clean_path(install_path)
            id_path = path_utils.clean_path(artellapipe.project.id_path)
        ...
        folders_to_register = artellapipe.project.get_folders_to_register()
        if folders_to_register:
            if os.environ.get('PYTHONPATH'):
                ...
                os.environ['PYTHONPATH'] = config.value(env_var)
                for p in folders_to_register:
                    os.environ['PYTHONPATH'] = os.environ['PYTHONPATH'] + ';' + p

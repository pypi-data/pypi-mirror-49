...
def make_continuous(R):
    """Flip signs of rotors to be more nearly continuous

    Depending on how they are created, certain rotor-valued time series may have spurious sign
    changes.  Assuming you want the time series to be more nearly continuous, this function leaves
    the first rotor as is, but progressively flips the signs of subsequent rotors to ensure that
    each one is as close in value as possible to the one before it.  For example, if the input
    rotors were just the sequence (1, -1, 1, -1), this function flips their signs and returns the
    rotors (1, 1, 1, 1).

    Parameters
    ----------
    R: array-like of quaternions

    """
    R = quaternion.as_float_array(R, dtype=np.quaternion)

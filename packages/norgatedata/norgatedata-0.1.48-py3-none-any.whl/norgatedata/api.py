...
__all__ = ['StockPriceAdjustmentType','PaddingType',
           ...
           'lowest_ever_tick_size','margin','point_value','tick_size','first_notice_date',
...
def point_value(symbol):
    r = get_api_data('security' + '/pointvalue',None);
    validate_api_response(r,symbol)
    return str(r.data);

def tick_size(symbol):
    r = get_api_data('security' + '/ticksize',None);
    validate_api_response(r,symbol)
    return str(r.data);

def first_notice_date(symbol):
    r = get_api_data('security' + '/firstnoticedate',None);
    validate_api_response(r,symbol)
    return str(r.data);
...
def classification(symbol,schemename,classificationresulttype):
    ...
    r = get_api_data('classification',symbol,parameters);
    validate_api_response(r,symbol)
    result = bytes.decode(r.data);
    return result
...
def corresponding_industry_index(symbol,indexfamilycode,level,indexreturntype):
    ...
    r = get_api_data('industryindex',symbol,parameters);
    validate_api_response(r,symbol)
    result = bytes.decode(r.data);
    return result

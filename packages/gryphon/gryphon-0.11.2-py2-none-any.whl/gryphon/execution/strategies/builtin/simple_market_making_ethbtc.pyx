...
class SimpleMarketMakingETHBTC(Strategy):
    def __init__(self, db, harness, strategy_configuration):
        super(SimpleMarketMakingETHBTC, self).__init__(db, harness)

        # Configurable properties with defaults.
        self.spread = Decimal('0.01')
        self.base_volume = Money('0.005', 'ETH')
    ...
    def configure(self, strategy_configuration):
        super(SimpleMarketMakingETHBTC, self).configure(strategy_configuration)

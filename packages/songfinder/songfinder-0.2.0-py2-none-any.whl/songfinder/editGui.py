...
class EditGui(object):
	def __init__(self, frame, dataBase=None, screens=None, \
		...
		if screens and screens[0][0].width > 2000:
			width=56
		else:
			width=48
		if screens and screens[0][0].height > 2000:
			height=46
		else:
			height=32
	...
	def _createNewSongWindow(self, copiedSong=''):
		...
		if self._screens:
			geometryStr = '%s+%d+%d'%(geometryStr, \
							self._screens[0][1].width//2-100, \
							self._screens[0][1].height//4-25)
	...
	def printer(self, event=None, toPrintDict=None, loadDiapo=False):
		if not toPrintDict:
			toPrintDict=dict()
		maxPriority = 0
		toPrint = None

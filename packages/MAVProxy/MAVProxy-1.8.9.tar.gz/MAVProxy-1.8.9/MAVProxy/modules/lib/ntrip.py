...
class NtripClient(object):
    def __init__(self,
        ...
        if mavlink_dest is not None:
            self.mavlink_conn = mavutil.mavlink_connection(mavlink_dest)
                    ...
                    while data:
                        try:
                            ...
                            if self.mavlink_conn:
                                bdata = data[:]
                                while len(bdata) > 0:
                                    blen = min(110, len(bdata))
                                    send_data = bytearray(bdata[:blen])
                                    if blen < 110:
                                        send_data.extend(bytearray([0]*(110-blen)))
                                    self.mavlink_conn.mav.gps_inject_data_send(0, 0, blen, send_data)
                                    bdata = bdata[blen:]
...
if __name__ == '__main__':
    ...
    parser.add_option("-M", "--mavlink", type='string', default=None, help="send GPS_INJECT_DATA to mavlink target")
    ...
    if options.mavlink:
        ntripArgs['mavlink_dest'] = options.mavlink

...
from neptune.internal.common.datastream import FileChunkStream, dummy, compress_to_tar_gz_in_memory
...
def upload_to_storage(files_list, dir_list, upload_api_fun, upload_tarstream_api_fun, callback=dummy, **kwargs):
    for package in split_upload_files(dir_list + files_list):
        if package.is_empty():
            break
        ...
        if uploading_multiple_entries or creating_a_single_empty_dir:
            ...
            upload_tarstream_api_fun(**dict(kwargs, data=data))
            callback(package.size)

        else:
            file_chunk_stream = FileChunkStream(package.items[0], callback=callback)

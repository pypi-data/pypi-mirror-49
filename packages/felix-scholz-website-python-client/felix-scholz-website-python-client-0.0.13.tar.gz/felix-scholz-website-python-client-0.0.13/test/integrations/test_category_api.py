...
class CategoryApiTest(IntegrationTestCase):
    ...
    def test_upgrade_or_create(self):
        category = Category(Category.hash_identifier('test_' + str(datetime.today().timestamp())))
        category.title = 'Test api category upgrade_or_create'
        category.slug = 'test-api-category-upgrade_or_create'

        response: AsyncResult = self.api.update_or_create(category, True)
        r, status, header = response.get()
        pprint(r)

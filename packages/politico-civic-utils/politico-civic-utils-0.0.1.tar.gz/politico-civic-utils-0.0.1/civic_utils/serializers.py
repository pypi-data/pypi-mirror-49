...
from civic_utils.models import NaturalKeyMixin


class CommandLineSerializerMixin(object):
    def serialize_to_cli(self, data=None, stream=None, progress_output=None,
                         object_count=0, renderer=None, **options):
        if data is None:
            data = self.instance
    ...
    def end_cli_serialization(self):
        ...
        self.cli_stream.write(self.cli_renderer_klass().render(
            self.cli_exported_objects,
...
class CommandLineDeserializerMixin(object):
    def to_python(self, raw_data):
        ...
        for field_name, relation_info in info.relations.items():
            ...
            if not issubclass(relation_info.related_model, NaturalKeyMixin):
                continue
            ...
            relation_natural_key_fields = [
                ...
                for natural_key_component
                in relation_info.related_model.natural_key_fields
            ]
...
class CommandLineCapableListSerializer(CommandLineSerializerMixin,
                                       CommandLineDeserializerMixin,
                                       serializers.ListSerializer):
    pass


class CommandLineListSerializer(CommandLineDeserializerMixin,
...
class NaturalKeySerializerMixin(object):
    ...
    def build_nested_field(self, field_name, relation_info, nested_depth):
        if issubclass(relation_info.related_model, NaturalKeyMixin):
            field_class = NaturalKeySerializer.for_model(
                relation_info.related_model,
                validate_key=False,
            )
    ...
    def run_validation(self, data):
        ...
        natural_key_components = dict(zip(

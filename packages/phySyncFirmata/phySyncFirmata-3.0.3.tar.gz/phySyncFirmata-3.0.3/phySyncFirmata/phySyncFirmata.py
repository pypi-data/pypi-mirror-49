...
import logging
from .util import pin_list_to_board_dict, to_two_bytes, two_byte_iter_to_str, Iterator , init_logging_commands
...
class Board(object):
    ...
    def __init__(self, port, layout=None, baudrate=115200, name=None, timeout=None,debug=False,tutorial_mode=False):
        self.debug=debug
        self.logger = logging.getLogger(__name__)
        self.tutorial_mode=tutorial_mode
        if(self.debug):
            # print("phySyncFirmata started in debug Mode")
            init_logging_commands(self.logger)
            self.logger.critical("phySyncFirmata started in debug Mode")
        ...
        if self.sp is not None:
            self.logger.debug("serial port resource acquired.")
        self.name = name
        self._layout = layout
        if not self.name:
            self.name = port
        ...
        if self.tutorial_mode:
            if self.debug:
                ...
                self.logger.critical("Tutorial mode started")
                print("Protocol details -->>","https://github.com/firmata/protocol/blob/master/protocol.md")
    ...
    def send_as_two_bytes(self, val):
        if (self.debug):
            # print('Board:send_as_two_bytes-->> ',(bytearray([val % 128, val >> 7])))
            self.logger.debug('Board:send_as_two_bytes-->> '+str((bytearray([val % 128, val >> 7]))))
    ...
    def send_sysex(self, sysex_cmd, data):
        ...
        self.sp.write(msg)
        if self.debug:
            # print("Board:send_sysex -->> ",data)
            self.logger.debug("Board:send_sysex -->> "+str(data))
    ...
    def exit(self):
        ...
        if hasattr(self, 'sp'):
            if self.sp:
                self.sp.close()
                self.logger.debug("set serial port resource free.")
...
class Port(object):
    ...
    def __init__(self, board, port_number, num_pins=8):
        ...
        self.debug=self.board.debug
        self.logger=self.board.logger
    ...
    def enable_reporting(self):
        ...
        if self.debug:
            # print('PORT:enable_reporting -->>',msg)
            self.logger.debug('PORT:enable_reporting -->>'+str(msg))
    ...
    def disable_reporting(self):
        ...
        self.board.sp.write(msg)
        if self.debug:
            # print('PORT:disable_reporting',msg)
            self.logger.debug('PORT:disable_reporting'+str(msg))
    ...
    #    print("type pinnr", type(pin_nr))
        ...
        self.board.sp.write(msg)
        if self.debug:
            # print("PORT:write-->",msg)
            self.logger.debug("PORT:write-->"+str(msg))
...
class Pin(object):
    ...
    def __init__(self, board, pin_number, type=ANALOG, port=None):
        ...
        self.logger=board.logger
        self.tutorial_mode=board.tutorial_mode
    ...
    def _set_mode(self, mode):
        if self.tutorial_mode:
            print("setter for mode on PIN called. -->>"+str(mode))
        ...
        if self.debug:
            # print('PIN:_set_mode-->>',bytearray([SET_PIN_MODE, self.pin_number, mode]))
            self.logger.debug('PIN:_set_mode-->>'+str(bytearray([SET_PIN_MODE, self.pin_number, mode])))
        if mode == INPUT:
            self.enable_reporting()

    def _get_mode(self):
        print("getter for mode on PIN called.")
        return self._mode
    ...
    def enable_reporting(self):
        ...
        if self.type == ANALOG:
            ...
            self.board.sp.write(msg)
            if self.debug:
                # print("PIN:enable_reporting-->>",msg)
                self.logger.debug("PIN:enable_reporting-->>"+str(msg))
        else:
    ...
    def disable_reporting(self):
        ...
        if self.type == ANALOG:
            ...
            self.board.sp.write(msg)
            if self.debug:
                # print("PIN:disable_reporting-->>",msg)
                self.logger.debug("PIN:disable_reporting-->>"+str(msg))
        else:
        ...
        if value is not self.value:
            ...
            if self.mode is OUTPUT:
                if self.port:
                    ...
                    self.board.sp.write(msg)
                    if self.debug:
                        # print("PIN:write::output-->>",msg)
                        self.logger.debug("PIN:write::output-->>"+str(msg))
            elif self.mode is PWM:
                value = int(round(value * 255))
                msg = bytearray([ANALOG_MESSAGE + self.pin_number, value % 128, value >> 7])
                self.board.sp.write(msg)
                if self.debug:
                        # print("PIN:write::PWM-->>",msg)
                        self.logger.debug("PIN:write::PWM-->>"+str(msg))
            elif self.mode is SERVO:
                value = int(value)
                msg = bytearray([ANALOG_MESSAGE + self.pin_number, value % 128, value >> 7])
                self.board.sp.write(msg)
                if self.debug:
                    # print("PIN:write::SERVO-->>",msg)
                    self.logger.debug("PIN:write::SERVO-->>"+str(msg))

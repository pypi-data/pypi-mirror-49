...
def editor(app, engine, tshclass, serie_names,
    ...
    def cached_serie_names(engine):
        return serie_names(engine)
    ...
    dashboard.layout = html.Div([
        ...
        html.Div(dcc.Dropdown(id='ts_selector', value=None), style={'display': 'none'}),
        html.Div(html.Button(id='refresh_data', n_clicks=0, children='Refresh data in Table'), style={'display': 'none'}),
        ...
        html.Div([
            ...
            html.Tr([
                ...
                html.Tr([
                    html.Td([html.P('Manual differences:',
                           style={'font-family': 'Helvetica',
                                  "font-size": "130%"}
                           ),
    ...
    def adaptable_dropdown(href):
        ...
        all_names = cached_serie_names(engine)
    ...
    def display_info(info_serie):
        ...
        if additionnal_info is not None:
            ...
            inside_table = [html.Tr([html.Td(elt) for elt in tuple]) for tuple in data]
    ...
    def dynamic_button(_):
        return html.Button(id='refresh_data',
                           n_clicks=0,
    ...
    def editable_table(engine, tsh, id_serie, fromdate=None, todate=None):
        with engine.begin() as cn:
            ts, marker = tsh.get_ts_marker(cn, id_serie)
    ...
    def calculate_diff(rows, info_serie):
        ...
        ts_base = tsh.get(engine, id_serie)
        try:
            diff = tsh.diff(ts_base, ts_modif)
            info_diff['diff'] = diff
        except Exception as error:
            return json.dumps({'msg':'Error :' + str(error), 'diff': None})
    ...
    def display_diff(info_diff):
        ...
        if info_diff['diff']:
            ...
            inside_table = [html.Tr('{} : {}'.format(*elt)) for elt in tudiff]
    ...
    def insert_diff(n_clicks, info_serie, info_diff, rev_id_stored):
        ...
        try:
            with engine.begin() as cn:
                diff = tsh.insert(cn, ts, id_serie, author, dict(manual=True))
        except Exception as error:
            return str(error)

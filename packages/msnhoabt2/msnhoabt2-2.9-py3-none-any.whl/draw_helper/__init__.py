...
def content(image_path,text,font_file,tmp_path,wrap_text_len=70,font_size=33):
    ...
    line_height = font.getsize('hg')[1]
    heightX=(line_height * len(lines))+10
    if(heightX<250):
        heightX=250
    img_box = Image.new('RGBA', (image_size[0], heightX), (0, 0, 0, 130))
    draw = ImageDraw.Draw(img_box)
...
def filter_text(text):
        return re.sub(r'\r\n\s+','. ',strip_html5_whitespace((remove_tags(remove_tags_with_content(text, which_ones=('sup', 'script'))))))

...
def get_html_static(url, encoding='utf8'):
    import requests
...
def get_html_phantom(url, phantomjs_path, encoding='utf8'):
...
def get_html_chrome(url, executable_path, encoding='utf8'):
...
def get_html(url, method='static', executable_path=None, encoding='utf8'):
    ...
    if method == 'static':
        src = get_html_static(url, encoding=encoding)
    elif method == 'phantom':
        src = get_html_phantom(url, executable_path, encoding=encoding)
    elif method == 'chrome':
        src = get_html_chrome(url, executable_path, encoding=encoding)
    else:
        raise ValueError("Unrecongnized method {}.".format(method))
    return src


def get_soup(url, method='static', executable_path=None, encoding='utf8'):
    from bs4 import BeautifulSoup
    src = get_html(url, method, executable_path, encoding)

...
def unlock_request(address, port, path, nodename, user, fromport):
    ...
    try:
        ...
        if e.errno == 113:
            print >> sys.__stderr__, "ERROR: %s:%s > No route to host !" % (address, port)
            return "noroute"
    finally:
        s.close()
    return text
    ...
    try:
        ...
        for i, output_text in execute(cmd):
            if isinstance(output_text, basestring):
                if not textEdit:
                    if i == 0:
                        print >> sys.__stdout__, output_text[:-1]
                    else:
                        print >> sys.__stderr__, output_text[:-1]
                else:
                    if i == 0:
...
class MgvTemplate(object):
    ...
    def create(self):
        ...
        mgvWrapper.createNode(self, "Graph", name=self.name, path=path)
    ...
    def _saveState(self, node_version):
        ...
        if not os.path.exists(rep):
            try:
                ...
                print >> sys.__stderr__, "Can't create %s !" % rep
        ...
        try:
            ...
            print >> sys.__stderr__, "Can't create %s !" % path
            if node_version.node.item:
                    ...
                    if os.path.exists(version.dico[typeFile.name]):
                        try:
                            ...
                            if self.item:
                                ...
                                print >> sys.__stderr__, "Removing %s has failed !" % version.dico[typeFile.name]
            ...
            if os.path.exists(path):
                try:
                    ...
                    if self.item:
                        ...
                        print >> sys.__stderr__, "Removing directory of %s has failed !" % self.name
                    return False
...
%s
    ...
    print >> sys.__stderr__, "ERROR IN PROJECT HEAD"
    print >> sys.__stderr__, traceback.format_exc()
try:
%s
except Exception as e:
    print >> sys.__stderr__, "ERROR IN TYPE HEAD"
    print >> sys.__stderr__, traceback.format_exc()
try:
%s
except Exception as e:
...
""" % (projHead, typeHead, script)
        ...
        scriptfull = """import os, sys, pickle, traceback
sys.path.append('%s')

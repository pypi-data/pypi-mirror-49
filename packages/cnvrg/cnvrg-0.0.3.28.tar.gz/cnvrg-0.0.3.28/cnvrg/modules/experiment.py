...
class Experiment(CnvrgJob):
    ...
    @staticmethod
    def init(title=None, project=None):
        project = project or Project()
        resp = apis_helper.post(url_join(project.get_base_url(), 'experiments', 'local'),
                                data={"title": title, "commit": project.get_current_commit(), **env_helper.get_origin_job()})
        ...
        return exp
    @staticmethod
    def __run_callable(callable, title=None, project=None, cwd=None):
        experiment = Experiment.init(title=title, project=project)
    ...
    @staticmethod
    def run(command=None, callable=None, title=None, project=None, compute=None, computes=None, datasets=None, dataset=None, local=False, working_directory=None):
        project = project or Project()
        ## support
        ...
        if callable:
            return Experiment.__run_callable(callable, title=title, project=project)
        elif local:
            return Experiment.__run_local(cmd=command, title=title, project=project, cwd=working_directory)
        else:

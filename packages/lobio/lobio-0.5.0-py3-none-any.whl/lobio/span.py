...
class Span(Container, Iterable, Sized):
    ...
    def __init__(self, a, b, l, cyclic=False, index=0):
        if b is not None and a > b and not cyclic:
            raise IndexError(
                "Start {} cannot exceed end {} for linear spans".format(a, b)
            )
        if not index <= a <= index + l:
            raise IndexError("Start must be within ({}, {})".format(index, index + l))
        if not index <= b <= index + l:
            raise IndexError("End must be within ({}, {})".format(index, index + l))
        self.a = a
        self.b = b
    ...
    def new(self, a, b):
        return self.__class__(a, b, self.context_length, self.cyclic)
    ...
    def __str__(self):
        return "<{} {} {}>".format(
            self.__class__.__name__, (self.a, self.b, self.context_length), self.cyclic
        )

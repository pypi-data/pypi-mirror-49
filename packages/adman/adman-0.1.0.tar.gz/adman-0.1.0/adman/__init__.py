...
class AdPosixUserManager:
    ...
    def get_users(self, rdn=None, filt=None, attrs=None, scope=None):
        if attrs is None:
            attrs = AdUser._known_attrs.keys()

        f = USER_SEARCH_FILTER
        if filt is not None:    # Append caller filter
            f = f & filt

        userlist = self._search(base_rdn=rdn, filt=f, attrs=attrs, scope=scope)
    ...
    def get_groups(self, rdn=None, filt=None, attrs=None):
        if attrs is None:
            attrs = AdGroup._known_attrs.keys()

        f = GROUP_SEARCH_FILTER
        if filt is not None:    # Append caller filter
            f = f & filt

        grouplist = self._search(rdn, f, attrs)
    ...
    def __init__(self, ad, dn, **attrs):
        ...
        for attr, (type_, ro, ldap_attr) in self._known_attrs2.items():
            ...
            if val is not None:
                ...
                if type_ is str:
                    val = val[0].decode()   # TODO: encoding?
                else:
                    val = type_(val[0])
...
class AdUser(LdapChangeTrackObject):
    _known_attrs = {
        ...
        'userPrincipalName':    (str, False),
    }


class AdGroup(LdapChangeTrackObject):
    _known_attrs = {
        # Name => (Type, Read-only)
        'gidNumber':            (int, False),
        'objectSid':            (SID.from_bytes, True),

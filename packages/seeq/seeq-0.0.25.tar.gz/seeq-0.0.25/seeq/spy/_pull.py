...
def pull(items, start=None, end=None, grid='15min', header='__auto__', capsules_as='auto', tz_convert=None):
    if not isinstance(items, pd.DataFrame):
        raise RuntimeError('"items" must be a pandas.DataFrame')
    ...
    for phase in ['signals', 'conditions', 'scalars', 'final']:
        ...
        for index, row in query_df.iterrows():
            ...
            if phase == 'signals' and 'Signal' in item_type:
                ...
                _pull_condition(capsules_as, formulas_api, formula, parameters, item_id, item_name, output_df,
                                pd_start, pd_end, total_capsules, tz_convert, column_names)
                ...
                _pull_scalar(formulas_api, formula, parameters, item_id, item_name, output_df, column_names)
...
def _pull_condition(capsules_as, formulas_api, formula, parameters, item_id, item_name, output_df, pd_start, pd_end,
                    total_capsules, tz, column_names):
    offset = 0
    while True:
...
def _pull_scalar(formulas_api, formula, parameters, item_id, item_name, output_df, column_names):
    formula_run_output, _, http_headers = formulas_api.run_formula_with_http_info(
        formula=formula,

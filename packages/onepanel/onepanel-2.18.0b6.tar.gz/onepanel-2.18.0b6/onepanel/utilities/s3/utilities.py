...
class S3Uploader:
    ...
    def __init__(self, s3_wrapper, workers=None, upload_threads=None):
        ...
        if workers is None:
            workers = (os.cpu_count() or 1) * 10

        if upload_threads is None:
            upload_threads = (os.cpu_count() or 1) * 10
    ...
    def _process_upload(self):
        ...
        with self._lock:
            ...
            if self._current_uploads >= self._max_uploads:
                return

            while self._current_uploads < self._max_uploads:
                try:
                    file_path = next(self.file_paths)
...
class S3Facade:
    def __init__(self, s3_wrapper, max_threads=None):
        ...
        :param s3_wrapper:
        :type s3_wrapper S3Wrapper
        :param max_threads the maximum number of threads to allow per operation. Defaults to (os.cpu_count() or 1) * 10
        :type max_threads int
        """
        self.s3_wrapper = s3_wrapper

        if max_threads is None:
            max_threads = (os.cpu_count() or 1) * 10

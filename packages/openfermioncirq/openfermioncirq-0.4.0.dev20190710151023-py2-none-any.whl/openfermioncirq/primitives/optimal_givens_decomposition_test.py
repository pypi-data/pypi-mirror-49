...
def test_givens_inverse():
    ...
    ab_rotation = givens_matrix_elements(a, b, which=u'right')
...
def test_row_eliminate():
    ...
    gmat = givens_matrix_elements(unitary[1, 0], unitary[2, 0], which=u'right')
    givens_rotate(unitary, gmat, 1, 2, which=u'row')
    assert numpy.isclose(unitary[2, 0], 0.0)

    # eliminate U[1, 0] by rotating in 0, 1
    gmat = givens_matrix_elements(unitary[0, 0], unitary[1, 0], which=u'right')
    givens_rotate(unitary, gmat, 0, 1, which=u'row')
    assert numpy.isclose(unitary[1, 0], 0.0)

    # eliminate U[2, 1] by rotating in 1, 2
    gmat = givens_matrix_elements(unitary[1, 1], unitary[2, 1], which=u'right')
    givens_rotate(unitary, gmat, 1, 2, which=u'row')
...
def test_col_eliminate():
    ...
    gmat = givens_matrix_elements(unitary[2, 0], unitary[2, 1], which=u'left')
    ...
    givens_rotate(unitary, gmat.conj(), 0, 1, which=u'col')
...
def test_circuit_generation_and_accuracy():
    for dim in xrange(2, 10):
        ...
        for i, j in product(xrange(dim), repeat=2):

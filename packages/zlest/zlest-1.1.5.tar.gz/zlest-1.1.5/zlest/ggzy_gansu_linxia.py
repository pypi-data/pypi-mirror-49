...
import sys
import time
from zlest.util.etl import gg_meta, gg_html, est_meta, est_html
from zhulong.util.fake_useragent import UserAgent

from zlest.util.etl import add_info

_name_ = 'ggzy_gansu_linxia'
...
def chang_type(f):
    def inner(*args):
        ...
        if ctext == '招标公告':
            page_count = len(driver.page_source)
            val = driver.find_element_by_xpath('//dl[@class="byTradingDetailParent clear"][1]//a').get_attribute(
                'href')[-25:]

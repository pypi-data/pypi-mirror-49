import sys
import os
from zlairflow.dag.data.data_zlest1 import para
...
def write_dag(quyu, dirname, **krg):
    ...
    filename = "v8_%s.py" % quyu
    path1 = os.path.join(os.path.dirname(__file__), 'template', 'zlest1.txt')
    ...
    d = DAG('##abc_anhui_anqing##'
            ...
            ,max_active_runs=1) 
    """
    content = content.replace("##v2_zfcg_anhui_anqing##", "v8_%s" % quyu)

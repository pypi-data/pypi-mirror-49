...
class tzoffset(datetime.tzinfo):
    ...
    def __init__(self, **kwargs):
        # Must initialise with seconds OR minutes OR hours
        if len(kwargs) != 1 or (not 'seconds' in kwargs and not 'hours' in kwargs and not 'minutes' in kwargs):
            raise Exception("tzoffset must be initialised with precisely one of 'seconds', 'minutes' or 'hours' keyword arguments")

        self._offset = datetime.timedelta(seconds=kwargs.get("seconds",0),

...
__div_timer__ = """
    Timer : print a timer for given time.
    A counter and a message can be printed too.
"""
__div_chronos__ = """
    Chronos : a stopwatch for you script. Can save results in a tsv file.
"""
...
def main():
    ...
    if _user_yes():
        print("""
    You can print the number of iterations, here for 50 iterations""")
        for _ in Wheel(50):
            sleep(0.1)
        print("""
    Or the iterated value, here for an iteration over 50 paths""")
        sleep(2)
        paths = ['fakepath_%d' % index for index in range(50)]
    ...
    if _user_yes():
        ...
        Timer(delay = 5)
        print("""
    You can use it for each iteration in a loop.
    Here for a four iterations loop, pausing 2 seconds each time
    and overwriting the line in terminal (useful for big loops).""")
        sleep(10)
        for _ in range(1, 5):
            Timer(delay = 2, message = 'Pausing at step %d' % _, overwrite = True)
        print('')
    ...
    if _user_yes():
        print("""
    Use Chronos to time stages of your script. See README for usage.
    You can also use it to time iteration inside a loop.
    Here for a three iterations loop, with a pause equals to the iteration
    value.""")
        sleep(10)
        chrono = Chronos()
        for _ in range(1, 4):
        ...
        print('\n\n    Recorded times are :')
        for lap, duration in chrono.durations.items():
            print('    lap %s : %d seconds' % (lap, duration))
    _user_exit()
    ...
    __pen__ \
        ...
        .write(message = __div_logger__, color = __div_style__['color'],
               style = __div_style__['style'], reset = True, newline = True)
    if _user_yes():
        print("""

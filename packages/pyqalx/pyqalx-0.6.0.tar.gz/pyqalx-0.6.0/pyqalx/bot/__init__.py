...
class Bot:
    ...
    def start(self,
        ...
        if add_queue:
            ...
            user_session.queue.get_or_create(name=self.queue_name)
...
class Worker(Process):
    ...
    def _check_queue_credentials(self):
        ...
        if self.queue._credentials_expiring_soon():
            self.logger.error(f'refreshing security credentials for queue '
                              f'`{self.queue["name"]}`')
            queue_adapter = QalxQueue(session=self.session)
            self.queue = queue_adapter.reload(self.queue)

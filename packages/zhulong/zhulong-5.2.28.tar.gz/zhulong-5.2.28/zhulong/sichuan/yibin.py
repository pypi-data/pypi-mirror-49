...
def f3(driver, url):
    ...
    try:
        ...
        if '无法访问此网站' in driver.page_source:
            return '无法访问此网站'
        locator = (By.XPATH, "//div[@class='wrap'][string-length()>400]")
        WebDriverWait(driver, 2).until(EC.presence_of_all_elements_located(locator))
        flag = 2
    locator = (By.XPATH, "//div[@class='wrap'][string-length()>500] | //div[@class='w1200 divview'][string-length()>100] | //div[@class='zbright_content'][string-length()>100]")
    WebDriverWait(driver, 2).until(EC.presence_of_all_elements_located(locator))
...
if __name__=='__main__':
    ...
    #         print(d)
    # driver = webdriver.Chrome()
    # df = f3(driver, 'http://www.ccgp-sichuan.gov.cn/view/staticpags/shiji_jzxtpcg/40288687533a3c6b0153820a457e2975.html')
    # print(df)
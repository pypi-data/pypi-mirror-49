...
def cargar_data_s3(data,
    ...
    return copiar_archivo_s3(nombre_bucket, ruta_bucket, nombre_archivo,extension,body)


def copiar_archivo_s3(nombre_bucket,
                       ruta_bucket,
                       nombre_archivo,
                       extension,
                       body):
...
def ejecutar_query_redshift(query, conexion):
    ...
    try:
        ...
        try:
            cur.execute(query)
            print("Se ejecuto el query correctamente.")
        except Exception as e:
            estado = False

...
def check_hardware_settings(threads: int, debug: bool = False) -> Tuple[int, float]:
    ...
    availCores: int = cpu_count(logical=False)
    availMem: float = round(virtual_memory().total / 1073741824., 2)
    if debug:
        sys.stdout.write("\nSYSTEM INFORMATION:")
        sys.stdout.write("\nTotal physical cores:\t{:d}".format(availCores))
        sys.stdout.write("\nRequested physical cores:\t{:d}".format(threads))
        sys.stdout.write("\nTotal physical memory:\t{:.2f}".format(availMem))

    # adjust the number of threads is required
    if threads > availCores:
        sys.stderr.write("\nWARNING: the number of CPUs requested ({:d}) is higher than the total available physical cores ({:d})".format(threads, availCores))
        sys.stderr.write("\nThe number of CPUs will be set (at best) to {:d}".format(availCores))
        threads = availCores

    # adjust memory per cores
    memPerCore: float = availMem / threads
    minPerCoreMem: float = 1.3

    # adjust the number of threads if required
    if memPerCore < minPerCoreMem:
        while True:
            threads -= 1
            memPerCore = availMem / threads
            if memPerCore >= minPerCoreMem:
                sys.stdout.write("\n\nINFO")
...
def infer_orthogroups_mcl(orthoDbDir: str, outDir: str, sharedDir: str, sqlTblDir: str, outName: str, pairsList: List[str], inflation: float = 1.5, maxGenePerSp: int = 30, threads: int=4, debug: bool=False):
    ...
    sys.stdout.write('Inparalogs merging elapsed time (seconds):\t{:s}\n'.format(str(round(time.perf_counter() - tmp_timer_start, 3))))
...
def main():
    ...
    # adjust the number of threads is required
    newThreads, memPerCore = check_hardware_settings(threads, debug=debug)
    threads = newThreads
    del newThreads
    ...
    print("Memory per CPU (Gigabytes):\t{:.2f}".format(memPerCore))
    ...
    infoDict["Memory per CPU:"] = str(memPerCore)

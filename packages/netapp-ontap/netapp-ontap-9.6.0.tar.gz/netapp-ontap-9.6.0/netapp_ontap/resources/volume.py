...
Volumes with SnapLock type Compliance or Enterprise, are referred to as SnapLock volumes. Volumes with SnapLock type cannot be of FlexGroup style. Once a SnapLock aggregate is created, by default, volumes created inside the aggregate inherit the "snaplock" property from the aggregate.<br/>
ONTAP storage APIs allow you to create, modify, and monitor volumes and aggregates.<br/>
## Storage efficiency
Storage efficiency is used to remove duplicate blocks in the data and to compress the data. Efficiency has deduplication, compression, cross volume deduplication, and compaction options. On all-flash systems, all efficiencies will be enabled by default on volume creation. Options such as "background/inline/both" will be treated as both, which means both background and inline will be enabled for any efficiency option. The option "none" will disable both background and inline efficiency.<br/>
...
## QoS
QoS policy and settings enforce Service Level Objectives (SLO) on a volume. SLO can be set by specifying qos.max_throughput_iops and/or qos.max_throughput_mbps or qos.min_throughput_iops. Specifying min_throughput_iops is only supported on volumes hosted on an a node which is flash optimized. A pre-created QoS policy can also be used by specifying qos.name or qos.uuid property. <br/>
...
## Volume APIs
The following APIs are used to perform operations related with FlexVols and FlexGroup volumes:
...
class VolumeSchema(ResourceSchema):
 ...
 """
    application = fields.Nested("VolumeApplicationSchema", unknown=EXCLUDE)
    r""" The application field of the volume.
 """
    autosize = fields.Nested("VolumeAutosizeSchema", unknown=EXCLUDE)
    r""" The autosize field of the volume.
 """
    clone = fields.Nested("VolumeCloneSchema", unknown=EXCLUDE)
    r""" The clone field of the volume.
 """
    comment = fields.Str(validate=len_validation(minimum=0, maximum=1023))
    r""" A comment for the volume. Valid in POST or PATCH.
 """
    constituents_per_aggregate = fields.Integer(validate=integer_validation(minimum=1, maximum=1000))
...
Example: 2018-06-04T19:00:00.000+0000 """
    efficiency = fields.Nested("VolumeEfficiencySchema", unknown=EXCLUDE)
    r""" The efficiency field of the volume.
 """
    encryption = fields.Nested("VolumeEncryptionSchema", unknown=EXCLUDE)
    r""" The encryption field of the volume.
 """
    error_state = fields.Nested("VolumeErrorStateSchema", unknown=EXCLUDE)
    r""" The error_state field of the volume.
 """
    files = fields.Nested("VolumeFilesSchema", unknown=EXCLUDE)
    r""" The files field of the volume.
 """
    flexcache_endpoint_type = fields.Str(validate=enum_validation(['none', 'cache', 'origin']))
...
* zh_tw.utf_8 """
 ...
 """
    movement = fields.Nested("VolumeMovementSchema", unknown=EXCLUDE)
...
Example: vol_cs_dept """
    nas = fields.Nested("VolumeNasSchema", unknown=EXCLUDE)
    r""" The nas field of the volume.
 """
    qos = fields.Nested("VolumeQosSchema", unknown=EXCLUDE)
    r""" The qos field of the volume.
 """
    quota = fields.Nested("VolumeQuotaSchema", unknown=EXCLUDE)
    r""" The quota field of the volume.
 """
    size = fields.Integer()
    r""" Physical size of the volume. The minimum size for a FlexVol volume is 20MB and the minimum size for a FlexGroup volume is 200MB per constituent. The recommended size for a FlexGroup volume is a minimum of 100GB per constituent. For all volumes, the default size is equal to the minimum size.
 """
    snaplock = fields.Nested("VolumeSnaplockSchema", unknown=EXCLUDE)
 ...
 """
    space = fields.Nested("VolumeSpaceSchema", unknown=EXCLUDE)
    r""" The space field of the volume.
 """
    state = fields.Str(validate=enum_validation(['error', 'mixed', 'offline', 'online']))
    r""" Volume state. A volume can only be brought online if it is offline. The 'mixed' state applies to FlexGroup volumes only and cannot be specified as a target state. An 'error' state implies that the volume is not in a state to serve data.
...
* online """
 ...
 """
    style = fields.Str(validate=enum_validation(['flexvol', 'flexgroup']))
...
* flexgroup """
 ...
 """
    tiering = fields.Nested("VolumeTieringSchema", unknown=EXCLUDE)
    r""" The tiering field of the volume.
 """
    type = fields.Str(validate=enum_validation(['rw', 'dp', 'ls']))
...
class Volume(Resource):
    ...
    get_collection.__func__.__doc__ = r"""Retrieves volumes.
...
* [`DOC /storage/volumes`](#docs-storage-storage_volumes)"""
    ...
    patch_collection.__func__.__doc__ = r"""Updates the attributes of a volume. For movement, use the "validate_only" field on the request to validate but not perform the operation. The PATCH API can be used to enable or disable quotas for a FlexVol or a FlexGroup volume.
### Related ONTAP commands
* `volume modify`
* `volume clone modify`
...
* [`DOC /storage/volumes`](#docs-storage-storage_volumes)"""
    ...
    delete_collection.__func__.__doc__ = r"""Deletes a volume. If the UUID belongs to a volume, all of its blocks are freed and returned to its containing aggregate. If a volume is online, it is offlined before deletion.
### Related ONTAP commands
* `volume delete`
* `volume clone delete`
...
* [`DOC /storage/volumes`](#docs-storage-storage_volumes)"""
    ...
    find.__func__.__doc__ = r"""Retrieves volumes.
### Expensive properties
There is an added cost to retrieving values for these properties. They are not included by default in GET results and must be explicitly requested using the `fields` query parameter. See [`DOC Requesting specific fields`](#docs-docs-Requesting-specific-fields) to learn more.
* `application.*`
* `encryption.*`
...
* [`DOC /storage/volumes`](#docs-storage-storage_volumes)"""
    ...
    get.__doc__ = r"""Retrieves a volume. The GET API can be used to retrieve the quota state for a FlexVol or a FlexGroup volume.
### Expensive properties
There is an added cost to retrieving values for these properties. They are not included by default in GET results and must be explicitly requested using the `fields` query parameter. See [`DOC Requesting specific fields`](#docs-docs-Requesting-specific-fields) to learn more.
* `application.*`
* `encryption.*`
...
* [`DOC /storage/volumes`](#docs-storage-storage_volumes)"""
    ...
    post.__doc__ = r"""Creates a volume on a specified SVM and storage aggregates.
### Required properties
...
* [`DOC /storage/volumes`](#docs-storage-storage_volumes)"""
    ...
    patch.__doc__ = r"""Updates the attributes of a volume. For movement, use the "validate_only" field on the request to validate but not perform the operation. The PATCH API can be used to enable or disable quotas for a FlexVol or a FlexGroup volume.
### Related ONTAP commands
* `volume modify`
* `volume clone modify`
* `volume efficiency modify`
...
* [`DOC /storage/volumes`](#docs-storage-storage_volumes)"""
    ...
    delete.__doc__ = r"""Deletes a volume. If the UUID belongs to a volume, all of its blocks are freed and returned to its containing aggregate. If a volume is online, it is offlined before deletion.
### Related ONTAP commands
* `volume delete`
* `volume clone delete`

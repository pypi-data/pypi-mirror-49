...
def server_error(request, template_name='500.html'):
    ...
    temp = django.template.loader.get_template(template_name)
    if not botsglobal.settings.DEBUG:
        exc_info = None
    ...
    if request.method == 'GET':
        ...
        if 'fromselect' in request.POST:         #from SelectConfirm form
            ...
            if ta_object.confirmed == False and ta_object.confirmtype.startswith('ask'):
                ...
                messages.add_message(request, messages.ERROR, _('Manual confirm not possible.'))
        ...
        if 'submit' in request.POST:
            ...
            if form.is_valid():
                ...
                try:
                    ...
                    messages.add_message(request, messages.ERROR, unicode(msg))
                else:
...
def runengine(request, *kw, **kwargs):
    ...
    if request.method == 'GET':
        ...
        if botsglobal.ini.getboolean('jobqueue', 'enabled', False):
            ...
            # reports back if bots-engine is started succesful. **not reported: problems with running.

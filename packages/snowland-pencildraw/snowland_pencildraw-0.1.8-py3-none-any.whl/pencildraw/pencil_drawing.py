...
import numpy as np
from scipy.signal import convolve, medfilt2d
from scipy.sparse import spdiags
from scipy.sparse.linalg import cg
from skimage import transform, img_as_float, io, data

from pencildraw.pencils import *
...
def natural_histogram_matching(img):
    img_map = np.arange(257) / 256.0
    img_map[-1] += 1e-12  # 加个小数防止溢出
    po = np.zeros(256)
    for i in range(256):
...
def pencil_drawing(img, ks, width, dirNum, gammaS, gammaI, pencil=pencil0):
    """
    ...
    if isinstance(pencil, str):
        p = io.imread(pencil, as_grey=True)
    else:
        p = pencil
    # p = img_as_float(p)
...
def genPencil(im, p, j):
    ...
    # Initialization
    p = transform.resize(p, (h, w), mode='constant', anti_aliasing=False)
    p = p.reshape((1, h * w))
    logP = np.log(p)
    logP = spdiags(logP, [0], h * w, h * w)
    j = transform.resize(j, (h, w), mode='constant', anti_aliasing=False)
    j = j.reshape((h * w, 1))
...
if __name__ == '__main__':
    ...
    plt.subplot(121)
    plt.imshow(im)
    I = pencil_drawing(im, 8, 1, 8, 1.0, 1.0, pencil=pencil3)
    io.imsave('11.jpg', I)
    plt.axis('off')

...
class LinProcess(BaseProcess):
    ...
    def iter_region(
        self, start_offset=None, end_offset=None, protec=None, optimisations=None
    ):
        """
            optimisations :
        ...
        with open("/proc/" + str(self.pid) + "/maps", "r") as maps_file:
            for line in maps_file:
                m = re.match(
                    r"([0-9A-Fa-f]+)-([0-9A-Fa-f]+)\s+([-rwpsx]+)\s+([0-9A-Fa-f]+)\s+([0-9A-Fa-f]+:[0-9A-Fa-f]+)\s+([0-9]+)\s*(.*)",
                    line,
                )
                if not m:
                    continue
                start, end, region_protec, _, _, inode, _ = (
                    int(m.group(1), 16),
                    int(m.group(2), 16),
                    m.group(3),
                    m.group(4),
                ...
                if "r" in region_protec:  # TODO: handle protec parameter
                    if optimisations:
                        if "i" in optimisations and inode != 0:
                            continue
                        if "s" in optimisations and "s" in region_protec:
                            continue
                        if "x" in optimisations and "x" in region_protec:
                            continue
                        if "r" in optimisations and not "w" in region_protec:
                            continue
                    yield start, chunk
    ...
    def write_bytes(self, address, data):
        ...
        for i in range(0, len(data), word_size):
            ...
            err = c_ptrace(
                ctypes.c_int(PTRACE_POKEDATA),
                c_pid,
                int(address) + i,
                int(word.encode("hex"), 16),
            )
            if err != 0:

...
def get_installation_paths(maya_versions):
    ...
    if not locations:
        artellalauncher.logger.warning('Maya installations not found in your computer. Maya cannot be launched!')
        return None
    ...
    for maya_version, maya_location in locations.items():
        ...
        if maya_executable is None or not os.path.isfile(maya_executable):
            artellalauncher.logger.warning('Maya {} installation path: {} is not valid!'.format(maya_version, maya_location))
            continue

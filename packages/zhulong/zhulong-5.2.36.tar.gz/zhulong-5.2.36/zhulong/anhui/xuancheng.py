...
def f1(driver, num):
    ...
    locator = (By.XPATH, '//tbody[@class="content-list"]/tr[1]//a')
    ...
    if int(cnum) != num:
        val = driver.find_element_by_xpath('//tbody[@class="content-list"]/tr[1]//a').get_attribute(
            "href")[- 30:-5]
        ...
        locator = (By.XPATH, '//tbody[@class="content-list"]/tr[1]//a[not(contains(@href,"%s"))]' % val)
        WebDriverWait(driver, 10).until(EC.presence_of_element_located(locator))
    ...
    trs = soup.find('tbody', class_="content-list").find_all('tr',recursive=False)
...
def f3(driver, url):
    ...
    locator = (By.XPATH, '//div[@class="tab-view"]//div[contains(@data-role,"tab-content") and (not(@class) or @class="")][string-length()>50] | '
                         '//div[@id="nr"]/div[@class="detail-body news_content"][string-length()>100]')
...
if __name__ == '__main__':
    work(conp=["postgres", "since2015", "192.168.3.171", "anhui", "xuancheng"],num=1)

    pass
    # driver=webdriver.Chrome()
    # f=f3(driver,'http://ggzyjy.xuancheng.gov.cn/xcspfront/jsgc/016003/016003002/20190712/cbc6e44d-8cc7-4e88-a831-5a0d66511d9b.html')
    # print(f)

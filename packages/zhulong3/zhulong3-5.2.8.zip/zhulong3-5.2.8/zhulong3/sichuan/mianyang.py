...
def f3(driver, url):
    ...
    if '/ceinwz/admin_show.aspx' in url:
        ...
        if '<iframe id="frmBestwordHtml' in str(driver.page_source):
            ...
            flag = 2
            locator = (By.XPATH,
                       "//table[@width='75%'][string-length()>30] | //div[@class='wrap'][string-length()>30] | //form[contains(@name, 'form1')][string-length()>30]")
            WebDriverWait(driver, 10).until(EC.presence_of_all_elements_located(locator))
...
if __name__ == '__main__':
    work(conp=["postgres", "zhulong.com.cn", "192.168.169.47", "gcjs", "sichuan_mianyang"],pageloadtimeout=200,pageloadstrategy="none",num=1,headless=False )
    ...
    #     print(df.values)
    # driver = webdriver.Chrome()
    # d = f3(driver, 'http://ztb.my.gov.cn/ceinwz/Hyzq/hyzbggzfcgDetail.aspx?sgzbbm=MYJY(2017)0243&zgys=0')
    # print(d)

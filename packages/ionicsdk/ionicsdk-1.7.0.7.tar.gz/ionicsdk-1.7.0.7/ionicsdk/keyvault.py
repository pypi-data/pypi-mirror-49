...
Classes:
    KeyVault - stores keys locally in an encrypted 'vault' - implemented platform specific vaults for Mac, iOS, and Windows.
...
class KeyVaultKeyRecord(object):
    """!Class pass key information in and out of the Key Vaults.
        ...
        attributes (dict of lists) - Protection key attributes.  For example, when creating key(s), key attributes
        ...
        mutableAttributes (dict of lists) - mutable protection key attributes.  For example, when creating key(s),
        ...
        obligations (dict of lists) - Used to store protection key policy obligations.  For example, when retrieving
            a key with GetKey(), policy obligations associated with the retrieved key will be represented here.
            
        issuedServerTimeUtcSeconds - The time at which this key was issued, measured in UTC seconds according to
            server time.
        
        expirationServerTimeUtcSeconds - nExpirationServerTimeUtcSeconds The time at which this key expires,
            measured in UTC seconds according to server time.
        
        state (int) - This is ONLY set on KeyVaultCustom callbacks and is otherwise unused.  See KeyVaultCustom
            class for details.
    """
...
class KeyVault(object):
    ...
    def SetKey(self, keyVaultKeyRecord, addIfNotFound):
        """!Add or update a key into the key vault.
            ...
            If the key is found, but its 'issued' time (ISKeyVaultKey::getIssuedServerTimeUtcSeconds()) is 
...
class KeyVaultAppleKeyChain(KeyVault):
    """!KeyVault service object for iOS, which uses the Apple key chain to store keys
        
        protection key - The AES GCM key to use for protecting the key vault data
        
        protection auth data - The AES GCM Additional Authenticated Data (AAD) to use for protecting the key vault data
        ...
        AccessGroup - The keychain access group to use for storing the key vault protection key
    ...
    def SetAccessGroup(self, accessGroup):
        """!
            Setter for the Apple key chain access group (iOS only)  Can be None.

...
from ehelply_bootstrapper.bootstrap import LOADABLE_REDIS, LOADABLE_RABBITMQ, LOADABLE_SENTRY, LOADABLE_SOCKET, \
    LOADABLE_FASTAPI, LOADABLE_MONGO, LOADABLE_AWS
...
"""
config_path: str = str(Path(__file__).parents[1]) + "/config"
...
class ServiceBootstrap(Bootstrap):
    ...
    def post_load(self):
        State.logger.info("No application code added to the post_load function inside of the service_bootstrap")
        if LOADABLE_FASTAPI not in loadables:
            State.logger.info("The service will now close as a result.")
        else:
            State.logger.info("The service will now delegate control to the fastapi server")

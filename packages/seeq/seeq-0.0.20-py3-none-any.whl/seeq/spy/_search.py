...
def search(query, workbook=_common.DEFAULT_WORKBOOK_PATH, all_properties=False):
    """
    ...
    for query in queries:
        ...
        if _common.present(query, 'Datasource Name'):
            ...
            if len(datasource_results.items) > 1:
                raise Exception('Multiple datasources found that match "%s"' % _common.get(query, 'Datasource Name'))
            elif len(datasource_results.items) == 1:
        ...
        if _common.present(query, 'Asset') and not _common.present(query, 'Path'):
            raise Exception('"Path" query parameter must be present when "Asset" parameter present')
        ...
        if _common.present(query, 'Path'):
            ...
            for part in path:
                ...
                if not found:
                    print('Could not found asset "%s"' % query['Path'])
                    return
        ...
        def _do_search(offset):
            ...
            print('Retrieved %d items...' % offset)
        ...
        def _gather_results(result):
            ...
            _add_to_dict('Value Unit Of Measure', result.value_unit_of_measure)
            if all_properties:
            ...
            for required_prop_name, required_prop_value in query.items():
                if required_prop_name not in prop_dict:
                    return

                if not re.match(required_prop_value, prop_dict[required_prop_name]):
                    return

...
u"""Operations for preparing useful quantum states."""
...
def prepare_gaussian_state(qubits,
                           quadratic_hamiltonian,
                           occupied_orbitals=None,
                           initial_state=0
                           ):
    u"""Prepare a fermionic Gaussian state from a computational basis state.
...
def _spin_symmetric_gaussian_circuit(
    ...
    for spin_sector in xrange(2):
        ...
        spin_indices = [index_map(i) for i in xrange(n_qubits // 2)]
        spin_qubits = [qubits[i] for i in spin_indices]

        # Flip bits so that the correct starting orbitals are occupied
        yield (cirq.X(spin_qubits[j]) for j in xrange(n_qubits // 2)
...
def prepare_slater_determinant(qubits,
                               slater_determinant_matrix,
                               initial_state=0
                               ):
    ur"""Prepare a Slater determinant from a computational basis state.
...
def _occupied_orbitals(computational_basis_state, n_qubits):
    ...
    return set(j for j in xrange(len(bitstring)) if bitstring[j] == u'1')

...
class ParameterEH(IOBase):
    ...
    def __init__(self, parameter_path=None, value_descriptor: Descriptor = None, mode = 'Set', **kwargs):
        """
    ...
    def transformation_function(self, hub, value) -> None:
        """Mutates the building based on the value provided."""
        if self.mode is 'Multiply':
            pf.set_by_path(hub.__dict__, self.parameter_path, (value * pf.get_by_path(hub.__dict__, self.parameter_path)))
        if self.mode is 'Add':
            pf.set_by_path(hub.__dict__, self.parameter_path, (value + pf.get_by_path(hub.__dict__, self.parameter_path)))
        if self.mode is 'Set':

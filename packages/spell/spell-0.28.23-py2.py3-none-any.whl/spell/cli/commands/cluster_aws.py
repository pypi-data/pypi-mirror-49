...
@click.option("-c", "--cluster", "cluster_name", type=str,
              help="The cluster name of the cluster that you would"
                   " like to configure this model serving cluster to work with.")
...
def eks_init(ctx, aws_profile, cluster_name, create_new_vpc, node_private_networking,
...
@click.option("--cluster-name", type=str, help="Name of cluster to update")
def update(ctx, profile, cluster_name):
    """
    ...
    spell_client = ctx.obj["client"]
    with api_client_exception_handler():
        cluster = spell_client.get_spell_cluster(cluster_name)

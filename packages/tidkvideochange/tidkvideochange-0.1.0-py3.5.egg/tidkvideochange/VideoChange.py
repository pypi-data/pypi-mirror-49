...
def VideoChangeVideoText(path, threshold, output, pred):
    ...
    with TemporaryDirectory() as temp_dir:
        ...
        subprocess.run(["scenedetect", "--input", path, "--output", output_frames_path, "--stats", "{}.stats.csv".format(output), "detect-content", "-t", str(threshold), "save-images", "-n", "1", "list-scenes"])
        ...
        if output is not None:
            ...
            if pred:
                ...
                return infer_frames_with_batches(output_frames_path)

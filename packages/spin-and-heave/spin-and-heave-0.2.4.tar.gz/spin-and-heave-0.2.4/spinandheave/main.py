...
where `lambda` is the dir including the lambda code and requirements/modules""",
    ...
    buildit = os.path.join(cwd, "scripts", "build.sh")
    print(Bcolors.OKGREEN+"building {} in docker".format(source)+Bcolors.ENDC)
    cmd = buildit+" "+swd+" "+runtime+" "+cwd+" "+source
    print(cmd)
    os.system(cmd)
    try:
        zipfile = os.path.join(cwd, source+".zip")
        zipsize = str(os.path.getsize(zipfile) >> 20)+"MB"
        print(Bcolors.OKGREEN+"created "+Bcolors.ORANGE+source+".zip "+Bcolors.WARNING+zipsize+Bcolors.ENDC)

...
class NameTruncatedFormatter(logging.Formatter):
    def __init__(self, max_length=40, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.max_length = max_length

    def format(self, record):
        match = re.search(r'%\(name\)-?(\d*)s', self._fmt)
        if match:
            if match.group(1):
                max_length = int(match.group(1))
            else:
                max_length = self.max_length
                self._fmt = self._fmt.replace('%(name)s', '%(name)-{}s'.format(max_length))
                self._style._fmt = self._fmt

            name = record.name
            if len(name) > max_length:
                parts = name.split('.')
                truncated_parts = [part[0] for part in parts[0:-1]]
                truncated_parts.append(parts[-1])
                record.name = '.'.join(truncated_parts)

...
def test_store_retrieve_file():
    ...
    data = pycws.create_member(
        URL, 'admin', 'admin', user_name, user_pw)
    user_id = data.get('memberId')
    if not user_id:
        user_id = pycws._member_by_user(
            URL, 'admin', 'admin', user_name)
    # assert data["returnCode"] == 200
    # assert data.get("memberId") is not None

    data = pycws.create_circle(
    ...
    data = pycws.add_trustee(
        URL, 'admin', 'admin', circle_id, user_id, 'WRITE')
    # assert data["returnCode"] == 200
    ...
    data = pycws.fetch_file(
        URL, 'admin', 'admin', file_id)
    assert data["returnCode"] == 200
    assert data['metadata'][0]['dataId'] == file_id
    assert base64.b64decode(data['data']) == blob

    data = pycws.delete_file(
        URL, 'admin', 'admin', file_id)
    assert data["returnCode"] == 200

    data = pycws.delete_circle(
        URL, 'admin', 'admin', circle_id)
    assert data["returnCode"] == 200

    data = pycws.delete_member(
        URL, 'admin', 'admin', user_name)
    assert data["returnCode"] == 200

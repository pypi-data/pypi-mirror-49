...
def f1(driver,num):
    ...
    if num!=cnum:
        ...
        val=driver.find_element_by_xpath("//div[@class='infolist-main']//li[1]//a").get_attribute('href')[-15:]
        driver.get(url)
    ...
    for li in lis:
        ...
        tmp=[a["title"].strip(),ggstart_time, sturl+a["href"]]
        data.append(tmp)
...
def f2(driver):
    global sturl
    url = driver.current_url
    sturl = re.findall(r'(^http[a-z:/\.]{1,}(?:com|cn))/.*?', url)[0]

...
class BaseCallback(BaseCallbackImages):
    ...
    def _project_token(self):
        return self.experiment_args['project_token']
    ...
    def _validate_auth_params(self, owner_id, project_token, project_id):
    ...
    def set_hyperparams(self, total_epochs=None, batch_size=None, epoch_size=None, max_iterations=None,
                        ...
                        samples_count=None, params=None, **kwargs):
        ...
        if params:
            self.properties[HyperParamTypes.PARAMS] = params
    ...
    def _call_new_experiment(self, throw_exceptions=None):
        ...
        host = 'missinglink.ai'if self.experiment_args['host'] == 'https://missinglinkai.appspot.com' else 'missinglink-staging.firebaseapp.com'
        try:
            experiment_url = 'https://{host}/console/{org_id}/projects/{project_id}/experiments/{experiment_id}'.format(host=host, **res)

            logging.info('new experiment create at %s', experiment_url)
        except KeyError:
            pass
    ...
    def _train_begin(self, params=None, structure_hash=None, **kwargs):
        self.logger.info('train begin params: %s, %s', params, kwargs)

        params = params or {}

        if HyperParamTypes.PARAMS in self.properties:
            params = self.properties.pop(HyperParamTypes.PARAMS)
    ...
    def _train_end(self, **kwargs):
        if not self.has_ended:
            ...
            iterations = int(kwargs.pop('iterations', 0))
    ...
    def epoch_end(self, epoch, metric_data=None, weights_hash=None, **kwargs):
        self.logger.info('epoch %s ended %s (weights_hash: %s)', epoch, metric_data, weights_hash)
    ...
    def batch_end2(self, iteration, metric_data, batch=None, epoch=None, update_hyperparams=False, weights_hash=None, **kwargs):
    ...
    def _update_computed_metrics(self, metric_data, phase):
        ...
        for func_name, func in self._COMPUTED_METRICS_METHODS.items():
            for metric_name, metric_value in metric_data.items():
                if not self._update_computed_metric(computed_values, func, func_name, metric_name, metric_value, phase):
                    continue
    ...
    @classmethod
    def _build_computed_metric_name(cls, func_name, metric_name):
    ...
    def _test_iteration_end(self, expected, predictions, probabilities, is_finished=False, **kwargs):
        class_mapping = kwargs.pop('class_mapping', self._class_mapping)
        class_mapping = self._convert_class_mapping(class_mapping)
        ...
        if isinstance(generator, QueryDataGenerator):
            try:
                if ReportsSaver.try_enable_test_report(ctx, self.experiment_args['project_token']) is True:
                    ...
                    self._report_saver = ReportsSaver(self.experiment_id, self.experiment_token, self.experiment_args['project_token'], ctx, self.logger, self._test_token)
            except Exception:
...
class BaseContextValidator(object):
    ...
    def context(self, context, *args, **kwargs):
        phase = self.enter(context, *args, **kwargs)
        try:
            yield phase
        finally:
            self.exit(context)
    ...
    @property
    def in_train_context(self):
        return self.last_context == Context.TRAIN

    def is_in_context(self, context):
        for current_context, _ in self._contexts:
    ...
    def enter(self, context, *args, **kwargs):
        ...
        phase = klass(self.last_phase, metric_prefix, *args, **kwargs)

        method_name = '_validate_{}_context'.format(context.lower())
...
class ContextStacks(object):
    def __init__(self):
        self.__stack = []

    @property
    def top_scope(self):
        return self.__stack[-1][2]

    def enter(self, name, method, *args, **kwargs):
        cm = method(*args, **kwargs)
        scope = cm.__enter__()
        self.__stack.append((name, cm, scope))

        return scope

    def exit(self, name):
        phase_name, ctx, scope = self.__stack.pop()

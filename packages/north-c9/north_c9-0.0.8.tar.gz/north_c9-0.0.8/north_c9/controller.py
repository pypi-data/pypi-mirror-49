from typing import Optional, List, Any, Dict, Union, Tuple, Iterable
import time
import random
from threading import Lock
from ftdi_serial import Serial, Device, SerialException
...
class C9Controller:
    ...
    def __init__(self,
        ...
        if use_joystick:
            ...
            self.joystick = N9Joystick(self, ignore_missing_joystick=True)
    ...
    def request_command(self, name: str, args: Iterable[Any]=[], flags: Dict[str, bool]={}, timeout: Optional[float]=None,
        ...
        try:
            ...
            if crc_fail or response.startswith('ERROR'):
                ...
                self.error_count += 1
                self.command_error_count[error_number] = self.command_error_count.get(error_number, 0) + 1

                print(self.network_address, '# Error count')
                for k, v in self.command_error_count.items():
                   print(f'{k}: {v}')
                ...
                raise C9Error(int(error_number), error_message.strip())
            ...
            if retries == 0:
                raise err
            self.error_count += 1
            self.command_error_count[-1] = self.command_error_count.get(-1, 0) + 1
            print(self.network_address, '# Error count')
            for k, v in self.command_error_count.items():
                print(f'{k}: {v}')
            timeout = retry_timeout_scale(self.retries, retries) * self.retry_timeout
            print(self.network_address, 'Retry timeout:', timeout)
            time.sleep(timeout)
    ...
    def home(self, *axes: int, if_needed: bool=True, skip: bool=False):
        """
        ...
        """
        response = self.request_command('HOME', axes or [None], flags={'C': if_needed, 'K': skip}, timeout=120)
    ...
    def get_output(self, output: int) -> bool:
        """
        Return the state of the given output

        :param output: output number
        :return: output state
        """
        state = self.request_command('OUTP', [output])
        return state == '1'

    def output(self, output: int, state: bool):
        """
        Set the state of given output

        :param output: output number
        :param state: output state (True is on)
        """
        value = 1 if state else 0
        self.request_command('OUTP', [output, value])
        self.dispatch_output_event('state', output, state)

    def output_toggle(self, output: int):
        """
        Toggle the state of the given output

        :param output: output number
        """
        state = self.get_output(output)
        self.output(output, not state)
    ...
    def get_speed(self) -> Tuple[int, int]:
        """
        Return the current velocity and acceleration

        :return: velocity, acceleration tuple
        """
        speed = self.request_command('SPED', []).split(' ')
        velocity, acceleration = speed
        return int(velocity), int(acceleration)

    def speed(self, velocity: int, acceleration: int):
        """
        Sets the default velocity and acceleration for movements

        :param velocity: velocity in counts / s
        :param acceleration: acceleration in counts / s^2
        """
        self.request_command('SPED', [velocity, acceleration])
    ...
    def move_arm(self, x: Optional[float]=None, y: Optional[float]=None, z: Optional[float]=None,
                 ...
                 relative: bool=False, wait: bool=True) -> List[int]:
        """

...
def check_is_pure(installed):
    ...
    pyver = 'cp{}{}'.format(sys.version_info.major, sys.version_info.minor)
    abi = 'none'
    is_pure = True
        ...
        if "site-packages" in installpath:
            if installpath.split('.')[-1] == suffix:
                ...
                if fname_split[0] == 'cpython':
                    ...
                    pyver = 'cp' +fname_split[1][:-1]
                    return False, pyver, abi
                is_pure = False

    return is_pure, pyver, abi
...
def prepare_metadata_for_build_wheel(metadata_directory,
    ...
    is_pure, _, _ = check_is_pure(meson_introspect(builddir, 'installed'))
...
class WheelBuilder:
    ...
    def build(self, wheel_directory, config_settings, metadata_dir):
        ...
        is_pure, pyver, abi = check_is_pure(installed_files)
        platform_tag = 'any' if is_pure else '{}_{}'.format(sys.platform, platform.machine())

...
# Sequence operations ############################################# 
def cut(seq, len_chunk=None, n_chunks=None, progress=True):
    """Cut a sequence into n chunks or evenly sized chunks.

    Parameter
    ========
    seq : iterable
    len_chunk : int
        Number of items in every chunk. Default None. Can not be specified along with n_chunks.
    n_chunks : int
        Number of chunks to slice into. Default None. Can not be specified along with len_chunks.
    progress : bool
        Set to True to return ('1/10', value) pair. Resemble the behavior of built-in enumerate().
    """   
    assert (len_chunk is None and n_chunks is not None) or \
            (len_chunk is not None and n_chunks is None), "Specify len_chunk or n_chunks, but not both."
    if len_chunk is not None:
        return chunk_size(seq, len_chunk, progress=progress)
    elif n_chunks is not None:
        return chunk_even(seq, n_chunks, progress=progress)

def chunk_even(seq, num, progress=True):
    ...
    if progress:
        ...
        tot = len(idx)
        pgs = ['{}/{}'.format(i, tot) for i in idx]
        seq = list(zip(pgs, seq))
    while last < len(seq):
        yield seq[int(last):int(last + avg)]
        last += avg

def chunk_size(seq, n, progress=True):

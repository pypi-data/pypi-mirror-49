...
class PodExtractor(object):
    ...
    def _get_all_content(self):
        ...
        if self.fileInfo.location is FileLocation.disk:
            LoggerAdapter.logblue(f'-> reading src file content: {self.fileInfo.filepath}')
            with open(self.fileInfo.filepath, encoding='utf-8') as f:
                f_text = f.read()
        elif self.fileInfo.location is FileLocation.remote:
            LoggerAdapter.logblue(f'-> requesting dst file content: {self.fileInfo.filepath}')
...
class PodComparer(object):
    def compare(self, src_extractor, dst_extractor):
        ...
        def print_pairs(pairs, header_desc):
            ...
            for pair in sorted(pairs, key=lambda x: x.name):
                ...
                if src == 'None' or dst == 'None':
                    compare = 'src-none' if src == 'None' else 'dst-none'
                    color = 2
                else:
                    comRslt = VersionComparer().compare(src, dst)
                    compare = comRslt.value
                    if comRslt.isLower():
                        color = 1
                    elif comRslt.isUpper():
                        color = 2
                # 这里分段着色比较特殊，分开写log
        ...
        intersection_equal_pairs = [x for x in intersection_pairs if x.srcPod == x.dstPod][0:2]

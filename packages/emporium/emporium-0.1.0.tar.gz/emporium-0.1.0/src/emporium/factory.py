from emporium.base import LocalStore, InMemoryStore
...
def create_store(config, **extras):
    ...
    :param config: Dictionary with the store configuration.  Should include a
        key `type` to determine which store gets instantiated.
    :param extras: Key-value pairs mapping type to a `from_config` like constructor.

...
from save_to_db.exceptions import ItemMustHaveTheSameIdentity
...
class TestItemProcess(TestBase):
    ...
    def test_defaults_injection_order_in_bulk(self):
        ...
        def assertItem(bulk_one):
            self.assertEqual(bulk_one['two_x_1__one_x_1__f_string'],
                             'default-1')
...
#     def test_bulk_gen_reverse(self):
...
#         
#         # --- one-to-many ---
#         item = ItemsGeneralOne(f_integer=1)
#         item_two = item['two_1_x'].gen(f_integer=2)
#         
#         self.assertIn(item_two, item['two_1_x'])
#         self.assertEqual(item['f_integer'], 1)
#         self.assertEqual(item['two_1_x'][0]['f_integer'], 2)
#         
#         self.assertNotIn('one_x_1', item['two_1_x'][0])
#         item.process()
#         self.assertIn('one_x_1', item['two_1_x'][0])
# 
#         self.assertIs(item['two_1_x'][0]['one_x_1'], item)
# 
#         # --- many-to-many ---
#         item = ItemsGeneralOne(f_integer=1)
#         item_two = item['two_x_x'].gen(f_integer=2)
#         
#         self.assertIn(item_two, item['two_x_x'])
#         
#         self.assertNotIn('one_x_x', item_two)
#         item_one = item_two['one_x_x'].gen(f_integer=3)
#          
#         self.assertIn(item_one, item_two['one_x_x'])
#         self.assertEqual(len(item_two['one_x_x']), 1)  # only 1 item
#         
#         item.process()
#         
#         self.assertEqual(len(item_two['one_x_x']), 2)  # other item added
#         f_integer_set = {
#             item['f_integer'] for item in item_two['one_x_x']
#         }
#         self.assertEqual(f_integer_set, {1, 3})
    
    
    def test_item_indentity_exception(self):
        class ItemsGeneralOne(Item):
            model_cls = self.ModelGeneralOne
        ...
        # --- one-to-many identity exception ---
        item = ItemsGeneralOne(f_integer=1)
        item['two_1_x'].gen(f_integer=2)
        
        item['two_1_x'][0]['one_x_1'](f_integer=3)
        # here will be a conflict
        self.assertIsNot(item['two_1_x'][0]['one_x_1'], item)
        # reverse is 'one_x_1', but it cannot have two items
        self.assertTrue(len(item['two_1_x']), 2)
        
        with self.assertRaises(ItemMustHaveTheSameIdentity):        
            item.process() 
        
        # --- one-to-one identity exception ---
        item = ItemsGeneralOne(f_integer=1)
        item['two_1_1'](f_integer=2)
        item['two_1_1__one_1_1'] = ItemsGeneralOne(f_integer=3)

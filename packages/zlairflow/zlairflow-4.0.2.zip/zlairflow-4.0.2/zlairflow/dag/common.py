import sys
import os
from zlairflow.dag.data import data_commom
...
def write_dag(quyu, dirname, conp="kunming",**krg):
    para = {
        "tag": "cdc",
...
def write_dags_common(dirname, conp="kunming",**krg):
    for w in data_commom.para:
        quyu = '_'.join(w[:2])
        timeout = w[2]
        krg.update({"timeout": timeout})

        write_dag(quyu, dirname,conp, **krg)

def write_dags_a(dirname, conp="kunming", **krg):
...
def write_dags_b(dirname, conp="kunming", **krg):
...
def write_dags_c(dirname, conp="kunming", **krg):

...
def get_python_install(installed):
    ...
    for installpath in installed.values():
        if "site-packages" in installpath:
            ...
            if installpath.split('.')[-1] == suffix:
                ...
                elif os.path.dirname(dirname) == 'python2.7':
                    pyvers.add('cp27')
                    abi = 'cp27m'
                else:
                    raise RuntimeError('%s not handled' % dirname)
            else:
    ...
    if not is_pure:
        pyvers = [ver for ver in pyvers if 'cp' not in ver]
...
def prepare_metadata_for_build_wheel(metadata_directory,
    ...
    print('Config settings: %s' % config_settings, file=open('res.txt', 'w'))
    if not builddir:

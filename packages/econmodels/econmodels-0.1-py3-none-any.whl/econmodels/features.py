...
def draw_plots(input_data, feature, target_col, trend_correlation=None,c=None):
    """
    ...
    if c == None:
        ...
        ax1.plot(input_data[target_col + '_mean'], marker='o',c=c)
    ...
    if c == None:
        ...
        ax2.bar(np.arange(len(input_data)), input_data['Samples_in_bin'], alpha=0.5,color=c)
...
def get_trend_changes(grouped_data, feature, target_col, threshold=0.1):
    """
...
def univariate_plotter(feature, data, target_col, bins=10, data_test=0,c=None):
    """
    ...
    if data[feature].dtype == 'O':
        ...
        if has_test:
            ...
            draw_plots(input_data=grouped, feature=feature, target_col=target_col,c=c)
            print(' {:^100} '.format('Test data plots'))

            draw_plots(input_data=grouped_test, feature=feature, target_col=target_col, trend_correlation=trend_corr,c=c)
        else:
            draw_plots(input_data=grouped, feature=feature, target_col=target_col,c=c)
        print(
            '--------------------------------------------------------------------------------------------------------------')
...
def Get_univariate_plots(data, target_col, features_list=0, bins=10, data_test=0,c=None):
    """
    ...
    for cols in features_list:
        if cols != target_col and data[cols].dtype == 'O':
            ...
            univariate_plotter(feature=cols, data=data, target_col=target_col, bins=10, data_test=data_test,c=c)

import os
from typing import Dict, Union, List, cast, BinaryIO, TextIO
import tempfile
...
def known_importers() -> Dict[str, BaseImporter]:
    importer_dict: dict = {importer.NAME: importer for importer in BaseImporter.get_subclasses()}
    return importer_dict


def known_auto_importers() -> Dict[str, BaseImporter]:
    auto_importers = {name: importer for name, importer in known_importers().items() if importer.AUTOIMPORTER}
...
def list_formats():
    ...
    for i in known_importers().keys():
        print('\t', i)
    print('\nThe following formats can be automatically detected and thus the\n'
          'type keyword can be omitted during import:\n')
    for i in known_auto_importers().keys():
        print('\t', i)


def _import(stream: BinaryIO, selected_importers: Dict[str, BaseImporter], **kwargs):
    for name, imp in selected_importers.items():
        try:
            i = imp(stream, **kwargs)  # type: ignore
            return i.read()
        except ip.utils.UnknownFileType:
            pass
    raise UndefinedImporter('AutoImporter: unable to find importer for {0}'.format(stream))


def read(files: Union[str, TextIO, Path, List[Union[str, TextIO, Path]]],
         append: bool = False,
         importer: str = None,
         as_list: bool = False,
         **kwargs) -> Union[DataContainer, List[DataContainer]]:
    """
    ...
    if importer:
        ...
        selected_importers = known_auto_importers()
    ...
    for data_file in files_list:
        data.append(_import(data_file, selected_importers, **kwargs))

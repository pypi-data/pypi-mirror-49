...
class Pyvko(ApiBased):
    ...
    def current_user(self) -> User:
        user_response = self.__api.users.get(**{"v": 5.92})

        user_id = user_response[0]["id"]

        user = User(api=self.__api, user_id=user_id)
    ...
    def get_group(self, url: str) -> Group:
        ...
        group_response = self.__api.groups.getById(**group_request)

        group = Group(api=self.__api, group_object=group_response[0])

        return group

    def get_photos_uploader(self) -> PhotosUploader:
        return PhotosUploader(self.__api)

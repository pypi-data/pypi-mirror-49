...
def transform_data(raw_data_context: RawDataContext,
                   ...
                   verifier: Optional[VerifierManager] = None) -> TransformedDataContext:
    """
...
def _get_data_snapshot(data: DataInputType, is_forecasting: bool = False) -> Any:
    ...
    if isinstance(data, pd.DataFrame):
        if is_forecasting:
            ...
            df_str = first_row.to_json(orient='records')
        else:
            df_str = data[:1].to_json(orient='records')
        sample_df_str = 'pd.DataFrame(data=' + df_str + ')'
        return sample_df_str
    elif isinstance(data, np.ndarray):
        np_array_str = json.dumps(data[0].tolist())
        ...
        np_array_str = json.dumps(data[0, :].todense().tolist())

...
def db(conp=None):
    if conp == "aliyun" or conp is None:
    ...
    "cdc_total" int4,
    "html_total" int4,
    "headless" boolean COLLATE "default",
...
def dbdata(host_dict):
    ...
    all_quyus = OrderedDict([("DB_A",DB_A),("DB_B",DB_B),("DB_C",DB_C),("DB_D",DB_D)])

    data = []
    for k in all_quyus.keys():

        quyus=[ ]
        for sheng in all_quyus[k]:
            tmp=quyu_dict[sheng]
            tmp=list(set(tmp))
            tmp.sort()
            quyus.extend(tmp)
        ...
        for quyu in quyus:
            ...
            tmp=['zlsrc',quyu,'postgres','since2015',host_dict[k],db,schema,None,None,5,None,None,'normal',40,
                 5,5,10,False,5,1,1,None,None]
            data.append(tmp)

    columns = ['package', 'quyu', 'usr', 'password', 'host', 'db', 'schema', 'total', 'num', 'cdc_total', 'html_total',
               'headless', 'pageloadstrategy', 'pageloadtimeout','ipNum', 'thread_retry','retry','add_ip_flag','page_retry',
               'image_show_gg','image_show_html','get_ip_url','info']

    df=pd.DataFrame(data=data,columns=columns)
    return df 

def get_hosts(conp=None):
    if conp == "aliyun" or conp is None:
        conp=['postgres','since2015','192.168.4.201','postgres','public']
    elif conp == "kunming":
        conp=['postgres','since2015','192.168.169.89','postgres','public']
...
def todb(conp=None):
    hosts_dict=get_hosts(conp)
    df=dbdata(hosts_dict)
    db_write(df,'cfg',dbtype="postgresql",if_exists='append',conp=conp)


def append_db(conp=None):
    """
    增量更新 cfg 表
    :param conp:
    :return:
    """
    host_dict=get_hosts(conp)
    df=dbdata(host_dict)
    datadict={"bd_key":BIGINT(),'total':BIGINT(),'num':BIGINT(),'cdc_total':BIGINT(),'html_total':BIGINT(),
              ...
              'add_ip_flag':BOOLEAN(),'page_retry':BIGINT(),'image_show_gg':BIGINT(),'image_show_html':BIGINT()}

    db_write(df,'cfg_tmp',dbtype="postgresql",if_exists='replace',conp=conp,datadict=datadict)

    sql="insert into cfg select * from cfg_tmp where quyu not in(select quyu from cfg)"
    db_command(sql,dbtype="postgresql",conp=conp)


def resetdb(conp=None):
    """
    重新生成 cfg 表
    :param conp:
    :return:
...
# todb(conp=['postgres','since2015','192.168.3.171','postgres','public'])

...
from cima.goes.gcs import get_blobs, get_dataset_from_blob, BlobsGroup
...
def _get_indexed_tiles(bands: Bands, band, lats_from, lats_to, lons_from, lons_to, step, overlap):
    blobs: BlobsGroup = get_blobs(2018, 360, 12, bands)
...
def generate_tiles(bands: Bands,
    ...
    for band in bands:
        tasks.append(
            Task(func=_get_indexed_tiles, args=[bands, band, lats_from, lats_to, lons_from, lons_to, step, overlap]))
    responses = run_concurrent(tasks, workers=2)

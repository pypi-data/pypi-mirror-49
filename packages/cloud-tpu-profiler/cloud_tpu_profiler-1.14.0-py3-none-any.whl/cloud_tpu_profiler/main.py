...
def main(unused_argv=None):
  ...
  if FLAGS.service_addr:
    if FLAGS.tpu:
      tf.compat.v1.logging.warn('Both --service_addr and --tpu are set. '
  ...
  if LooseVersion(tf_version) < LooseVersion('1.9'):
    tf.compat.v1.logging.warn('Attempt to profile with legacy support under '
                              'TensorFlow version %s' % tf_version)
  else:

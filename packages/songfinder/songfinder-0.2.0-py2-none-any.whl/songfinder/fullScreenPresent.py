...
class Presentation(object):
	def __init__(self, frame, diapoList, screens=None, closeCallback=None, **kwargs):
		...
		if globalvar.myOs == 'darwin':
			self._presentationWindow.attributes("-fullscreen", True)
			self._presentationWindow.geometry('%dx%d'%(self._screens[-1][0].width, self._screens[-1][0].height))
		else:
	...
	def show(self):
		inputRatio = screen.getRatio(settings.GENSETTINGS.get('Parameters', 'ratio'), self._screens[-1][0].ratio)
		self._presentationWindow.geometry(str(self._screens[-1][0]))
		print(str(self._screens[-1][0]))
		if inputRatio != 0:
			self._width = math.floor(min(inputRatio*self._screens[-1][0].height, self._screens[-1][0].width))
			self._height = math.floor(min(self._screens[-1][0].width//inputRatio, self._screens[-1][0].height))
		else:
			self._width = self._screens[-1][0].width
			self._height = self._screens[-1][0].height
		self._createTheme()
		self._prefetch()
		self.printer()
	...
	def _prefetch(self):
		progressBar = simpleProgress.SimpleProgress("Création du cache des images", screens=self._screens)

...
class CommandLine:
    ...
    async def read_command(self, compl=None, prompt='> ', clear=True):
        async with self._lock:
            ...
            # If result was not obtained (user cancelled typing a command),
            # clear should occur uncondiionally.
            if not result:
                clear = True

            # Move command from input buffer to the prompt, which won't be reset
            # after read_command exits. This allows us to reset a command buffer
            # right now and not worry that it will be displayed together with
            # any future prints (through e.g. pprint() call)
            if clear is False:
                self.set_prompt(prompt + result)
            with self.ensure_clear(prompt=clear):
                return result

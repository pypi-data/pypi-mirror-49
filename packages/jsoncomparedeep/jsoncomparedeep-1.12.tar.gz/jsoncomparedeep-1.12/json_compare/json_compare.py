...
class Jcompare(object):
        ...
        if not self._ignore_list_seq:
            ...
            for i in range(len(a)):
                for j in range(len(a)):
                    if self._common_comp(a[i], b[j], printdiff=False):
                        counts_a[i][1] = j
                        counts_a[i][0] += 1
                    if self._common_comp(b[i], a[j], printdiff=False):
                        counts_b[i][1] = j
                        counts_b[i][0] += 1
    ...
    def _common_comp(self, a, b, root=(), printdiff=True):
        if self._ignore_path:
            if u"/{}".format("/".join(root)) in self._ignore_path:
                return True
    ...
    def compare(self, a, b, ignore_list_seq=True, re_compare=True, ignore_path=None):
        ...
        if type(b) in [six.text_type, six.binary_type]:
            ...
            json_loaded_b = b
        if flag:
            return self.compare(json_loaded_a, json_loaded_b, ignore_list_seq, re_compare)
        ...
        if self._ignore_path:
            assert all([path[0] == u"/" for path in self._ignore_path]), "invalid ignore path"

        if self.print_before:
            print(self._escape("a is {}".format(a)))
            print(self._escape("b is {}".format(b)))
            print("ignore_list_seq = {}, re_compare = {}, ignore_path = {}".format(
                ignore_list_seq, re_compare, ignore_path))
...
if __name__ == "__main__":
    from test_json_compare import run_tests

...
class WSPacketWriteStream(WSBasePacketStream):
    ...
    def close(self, rsv):
        self.write_packets(WSPacket(opcode = WSOpcode.CLOSE, rsv = rsv))
        super().close()
...
class WSPacket(object):
    ...
    def __repr__(self):
        return "WSPacket(fin={fin} rsv={rsv} mask={mask} opcode={opcode} len={len})".format(fin=self.fin, rsv=bin(self.rsv).zfill(3), mask=(self.mask.hex() if self.mask is not None else None), opcode=self.opcode.name, len=len(self))
    ...
    def encode(self):
        ...
        data += struct.pack('B', (int(self.mask is not None) << 7) | length_bits) + length_field
        ...
        print(data.hex(), self, WSPacket.decode(data), WSPacket.decode(data).packets[0].content)
        print()
        return bytes(data)

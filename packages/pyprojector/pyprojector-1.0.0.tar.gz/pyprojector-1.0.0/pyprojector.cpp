...
struct WatchIcons {
    ...
    void watch(NOTIFYICONDATA * data) {
        EnterCriticalSection(&lock);
        head = new Item {data, head};
        LeaveCriticalSection(&lock);
    }
    void unwatch(NOTIFYICONDATA * data) {
        EnterCriticalSection(&lock);
        Item ** it = &head;
        while (*it) {
            if ((*it)->data == data) {
    ...
    ~WatchIcons() {
        ...
        Item * it = head;
        while (it) {
            Shell_NotifyIcon(NIM_DELETE, it->data);
            it = it->next;
        }
        LeaveCriticalSection(&lock);
    }
...
LRESULT CALLBACK WindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    if (Window * window = (Window *)GetWindowLongPtr(hWnd, GWLP_USERDATA)) {
        switch (uMsg) {
            case WM_DESTROY: {
                watch_icons.unwatch(&window->ndata);
...
void window_thread(Window * window) {
    ...
    if (!window->glReadPixels) {
        SetEvent(window->ready);
        window->error = "glReadPixels is missing";
        return;
    }
    ...
    watch_icons.watch(&window->ndata);
...
PyObject * Window_meth_screenshot(Window * self) {
    PyObject * res = PyBytes_FromStringAndSize(NULL, self->width * self->height * 3);
    EnterCriticalSection(&self->lock);
    self->glBindFramebuffer(GL_FRAMEBUFFER, self->framebuffer);
    self->glReadPixels(0, 0, self->width, self->height, GL_RGB, GL_UNSIGNED_BYTE, PyBytes_AS_STRING(res));
    LeaveCriticalSection(&self->lock);
    return res;
}

PyObject * Window_meth_enter(Window * self) {
    self->backup_hdc = wglGetCurrentDC();
    self->backup_hrc = wglGetCurrentContext();
    wglMakeCurrent(self->hdc, self->hrc2);
    Py_RETURN_NONE;
}

PyObject * Window_meth_exit(Window * self) {
    wglMakeCurrent(self->backup_hdc, self->backup_hrc);
    Py_RETURN_NONE;
}

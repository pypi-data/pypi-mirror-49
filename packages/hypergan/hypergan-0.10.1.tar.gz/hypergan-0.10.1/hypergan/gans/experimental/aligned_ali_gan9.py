    ...
    def create(self):
        ...
        with tf.device(self.device):
            ...
            if config.style:
                ...
                zga = self.create_component(config.encoder, input=tf.concat(self.inputs.xb, axis=3), name='xb_to_za')
                self.zga = zga
                za = zga.sample
                ga = self.create_component(config.generator, input=za, name='a_generator')
                zgb = self.create_component(config.encoder, input=tf.concat(self.inputs.xa, axis=3), name='xa_to_zb')
                self.zgb = zgb
                zb = zgb.sample
                gb = self.create_component(config.generator, input=zb, name='a_generator', reuse=True)
                #ga = hc.Config({"sample":tf.slice(g.sample, [0,0,0,0], [-1,-1,-1,3])})
                #gb = hc.Config({"sample":tf.slice(gb.sample, [0,0,0,3], [-1,-1,-1,3])})
                ga.set_layer("input", gb.sample)
                rza = self.create_component(config.encoder, input=gb.sample, name='xb_to_za', reuse=True)
                #rza = hc.Config({"sample":tf.slice(rz.sample, [0,0,0,0], [-1,-1,-1,3])})
                #rzb = hc.Config({"sample":tf.slice(rz.sample, [0,0,0,3], [-1,-1,-1,3])})
                xa_hat = self.create_component(config.generator, input=rza.sample, name='a_generator', reuse=True).sample
                ga.set_layer("input", ga.sample)
                rzb = self.create_component(config.encoder, input=ga.sample, name='xa_to_zb', reuse=True)
                xb_hat = self.create_component(config.generator, input=rzb.sample, name='a_generator', reuse=True).sample
                #xa_hat = tf.slice(gre.sample, [0,0,0,0], [-1,-1,-1,3])
                #xb_hat = tf.slice(gre.sample, [0,0,0,3], [-1,-1,-1,3])
                g_vars1 = ga.variables()+zga.variables()
                self.g = ga
                self.z = zga
                self.gaa = ga
                print ("SET GA")
            ...
            t0 = tf.concat([xb, xa], axis=3)
            t1 = tf.concat([gb.sample, ga.sample], axis=3)
            f0 = tf.concat([za, zb], axis=3)
            f1 = tf.concat([zb, za], axis=3)
            stack = [t0, t1]

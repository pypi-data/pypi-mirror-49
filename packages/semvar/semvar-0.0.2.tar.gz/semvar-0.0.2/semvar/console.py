...
def do_bump(args):
    ...
    files = list(filter(makefilter(data),
                        list(set(glob.glob('./**/*', recursive=True)+glob.glob('./**/.*',
                                                                               recursive=True)+glob.glob('./**/*.*', recursive=True)))
                        ))
    ...
    for file in files:
        ...
        for oi in occurance_indexes:
            ...
            new_file_path = os.path.abspath(
                os.path.join('.semvar.diffs', f"{file}__{oi:05}"))

...
def submit_notebook(assignment = None):
    ...
    time.sleep(3)
    print("[DEBUG] Save timer extended to 7 seconds")
    time.sleep(4)
    ...
    def get_notebook_name():
        ...
        for ss in servers:
            print("[DEBUG] URL:", urljoin(ss['url'], 'api/sessions'))
            print("[DEBUG] Params:", {'token': ss.get('token', '')})
            response = requests.get(urljoin(ss['url'], 'api/sessions'),
                                    params={'token': ss.get('token', '')})
            print("[DEBUG] Response:", response.text)
            for nn in json.loads(response.text):
                try:
                    print("[DEBUG] Kernel:", nn)
                    if nn['kernel']['id'] == kernel_id:
                        relative_path = nn['notebook']['path']
                        return os.path.join(ss['notebook_dir'], relative_path)
                except:
                    print("[DEBUG] Failed to get kernel:", nn)

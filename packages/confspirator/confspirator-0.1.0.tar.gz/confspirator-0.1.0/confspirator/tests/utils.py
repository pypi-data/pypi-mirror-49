...
# NOTE(adriant): needed until we drop py2
...
class modify_conf(object):
    ...
    def decorate_callable(self, func):
        @wraps(func, assigned=_available_attrs(func))
    ...
    def _do_operation(self, root_key, path, operation):
        keys = path.split(".")
        if keys[0] != root_key:
            self.enable_exception = exceptions.NoSuchConfig(path)
            self.disable()

        end_key = keys[-1]
        if operation["operation"] == "override":
            if len(keys) == 1:
                self.enable_exception = exceptions.InValidTestOperation(
                    "Cannot override root conf"
                )
                self.disable()

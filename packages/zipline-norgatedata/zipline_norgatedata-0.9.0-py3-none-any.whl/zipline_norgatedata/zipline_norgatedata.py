...
def create_norgatedata_futures_bundle(bundlename,watchlists,start_session,end_session):
    ...
    def ingest(environ,
        ...
        for symbol in symbols:
            print (translate_futures_symbol(symbol))
...
def determine_symbols(bundlename,watchlists,startdate):
    if (len(watchlists) == 0):
...
def _pricing_iter_futures(symbols, metadata, sessions, show_progress, start_session, end_session):
    with maybe_show_progress(symbols, show_progress,
        ...
        for sid, symbol in enumerate(it):
            # 2000 is the maximum here - anything before is not defined as a session apparently (!) - probably the calendars need work in zipline
            # Padding must be all market days, otherwise it will bork zipline's expection that there's a bar for every day
            ...
            zerovalues = {'volume' : 0}
            df.fillna(value=zerovalues,inplace=True)

...
class Config(MutableMapping):
    ...
    def __init__(self, pre_config_filepath=None):
        ...
        if not pre_config_filepath:
            ...
            default_cfg_filepath = os.path.join(
                '..', 'cfg', 'which_config.yaml')
    ...
    def get_credentials(self, name, keys=['user', 'passwd']):
        ...
        """
        # Get credentials
        keys_path = os.path.join(*ensure_collection(self.cfg['cfg_path']['keys']))
    ...
    def get_project_data_directory(
        ...
        base_dir = os.path.join(self.get_data_root_directory(**kwargs),
                                base_subdir)
        if container:
            user_project_dir = container.cfg.get('project_dir')
...
def load_config(cfg, name, parent=None):
    """Load container configuration from main configuration dictionary,
    from given component, or from file.

    :param cfg: Mediator configuration.
    :param name: Container name.
    :param component: Parent container from the configuration of which,
    ...
    :rtype: dict
    """
    cfg_from_comp = parent.cfg.get(name) if parent is not None else None
    user_cfg_path = cfg['cfg_path'].get(name)
    if name in cfg:
        ...
        cont_cfg = cfg_from_comp
    else:
        # None worked, no configuration found
        cont_cfg = None
        log.warning('No {} configuration found'.format(name))

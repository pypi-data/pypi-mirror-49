...
class DuplicationChecker(AbstractChecker):
    ...
    def checkLatLngsWithMultipleAddresses(self):
        ...
        blankMask = resUnfiltered["latitude"].apply(ValidatorUtils.is_not_blank) & resUnfiltered["longitude"].apply(ValidatorUtils.is_not_blank)
        res = resUnfiltered[blankMask & (resUnfiltered["num_addrs"] > 1)]
        if len(res) > 0:
    ...
    def warnIfSameAddrHasMultipleLatLngs(self):
        ...
        res.rename({"latitude": "same_lat_count", "longitude": "same_lng_count"}, inplace=True)
        if len(res) > 0:

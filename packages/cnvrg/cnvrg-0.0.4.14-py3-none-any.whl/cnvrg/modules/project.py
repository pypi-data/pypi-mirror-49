...
class Project(CnvrgFiles):
    ...
    def run_task(self, cmd, **kwargs):
        ...
        hyper_search_url = url_join(self.web_url(),
                                    "experiments?grid={grid}".format(grid=hyper_search.get("slug")[0:4]))
        logger_helper.log_message(
            "Grid {grid_slug} is running, follow at: {url}".format(grid_slug=hyper_search.get("slug"),
    ...
    #     options = "#{options} -nb" if new_branch
    #     options = "#{options} --force=true" if force
    #     options = "#{options} --message='#{message}'" if message.present?
    #     options = "#{options} --output_dir=#{output_dir}" if output_dir.present? and project.
    #     try(:is_git?)
    #     options = "#{options} --in-exp=true --job_type='#{job.class.name}' --job='#{job.slug}' " if job.present?

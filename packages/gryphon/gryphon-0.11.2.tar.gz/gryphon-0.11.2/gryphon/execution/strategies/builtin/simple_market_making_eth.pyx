...
class SimpleMarketMakingETH(Strategy):
    def __init__(self, db, harness, strategy_configuration):
        super(SimpleMarketMakingETH, self).__init__(db, harness)

        # Configurable properties with defaults.
        self.spread = Decimal('0.01')
        self.base_volume = Money('0.01', 'ETH')
    ...
    def configure(self, strategy_configuration):
        super(SimpleMarketMakingETH, self).configure(strategy_configuration)

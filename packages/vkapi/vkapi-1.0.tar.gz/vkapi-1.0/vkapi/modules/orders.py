...
class Orders(ApiModule):
    ...
    def cancelSubscription(self, user_id, subscription_id, pending_cancel=0):
        return self._get_response(locals())

    def changeState(self, order_id, action, app_order_id=None, test_mode=0):
        return self._get_response(locals())

    def get(self, offset=0, count=100, test_mode=0):
        return self._get_response(locals())

    def getAmount(self, user_id, votes):
        return self._get_response(locals())

    def getById(self, order_id=None, order_ids=None, test_mode=0):
        return self._get_response(locals())

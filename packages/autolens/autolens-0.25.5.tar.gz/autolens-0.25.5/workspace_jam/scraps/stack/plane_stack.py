...
class PlaneStack(pl.AbstractPlane):
    def __init__(
        ...
        if compute_deflections:
            ...
            self.deflection_stacks = list(
                map(
                    lambda grid_stack: grid_stack.apply_function(calculate_deflections),
                    self.grid_stacks,
                )
            )
    ...
    def trace_grids_to_next_plane(self):
        ...
        return list(
            map(
                lambda grid, deflections: grid.map_function(minus, deflections),
                self.grid_stacks,
                self.deflection_stacks,
            )
        )
    ...
    def has_padded_grid_stack(self):
        return any(
            list(
                map(
                    lambda grid_stack: isinstance(
                        grid_stack.regular, grids.PaddedRegularGrid
                    ),
                    self.grid_stacks,
                )
            )
    ...
    def mapper(self):
        ...
        if len(galaxies_with_pixelization) == 1:
            ...
            return pixelization.mapper_from_grid_stack_and_border(
                self.grid_stacks[0], self.borders
            )
    ...
    def image_plane_images_for_simulation(self):
        ...
        return list(
            map(
                lambda image_plane_image_1d, grid_stack: grid_stack.regular.padded_array_2d_from_padded_array_1d(
                    padded_array_1d=image_plane_image_1d
                ),
                self.image_plane_images_1d,
                self.grid_stacks,
            )
    ...
    def image_plane_images_1d(self):
        return list(
            map(
                lambda grid_stack: galaxy_util.intensities_of_galaxies_from_grid(
                    grid=grid_stack.sub, galaxies=self.galaxies
                ),
                self.grid_stacks,
            )
        )
    ...
    def image_plane_images_1d_of_galaxies(self):
        return list(
            map(
                lambda grid_stack: [
                    galaxy_util.intensities_of_galaxies_from_grid(
                        grid=grid_stack.sub, galaxies=[galaxy]
                    )
    ...
    def image_plane_blurring_images_1d(self):
        return list(
            map(
                lambda grid_stack: galaxy_util.intensities_of_galaxies_from_grid(
                    grid=grid_stack.blurring, galaxies=self.galaxies
                ),
                self.grid_stacks,
            )
        )
    ...
    def plane_images(self):
        return list(
            map(
                lambda grid_stack: lens_util.plane_image_of_galaxies_from_grid(
                    shape=grid_stack.regular.mask.shape,
    ...
    def yticks(self):
        ...
        return list(
            map(
                lambda grid_stack: np.linspace(
                    np.amin(grid_stack.regular[:, 0]),
                    np.amax(grid_stack.regular[:, 0]),
                    4,
                ),
                self.grid_stacks,
            )
    ...
    def xticks(self):
        ...
        return list(
            map(
                lambda grid_stack: np.linspace(
                    np.amin(grid_stack[0].regular[:, 1]),
                    np.amax(grid_stack[0].regular[:, 1]),
                    4,
                ),
                self.grid_stacks,
            )

...
def getSimiScore(word1,word2,vec_type='ifchange'):
    try:
        data ={'type':vec_type,'word1':word1,'word2':word2}
        serverName='w2v'
        return doTask(serverName,data)
    except Exception as e:
        raiseException('%s exception \nplease contact supporter %s for this exception ! \n%s'%(serverName,supportConf[serverName],e))

def getWordVec(word,vec_type='ifchange'):
    try:
        if isinstance(word,str):
            word = [word]
        data = {'words':word,'type':vec_type}
        serverName='w2v'
        return doTask(serverName,data)
    except Exception as e:
        raiseException('%s exception \nplease contact supporter %s for this exception ! \n%s'%(serverName,supportConf[serverName],e))

def getMostSimiWords(word,topn=10,vec_type='ifchange'):
    try:
        data = {'words':word,'topn':topn,'type':vec_type}
        serverName='w2v'
        return doTask(serverName,data)
    except Exception as e:
        raiseException('%s exception \nplease contact supporter %s for this exception ! \n%s'%(serverName,supportConf[serverName],e))
...
# Sentence2Vector
def getSentenceVec(sentences,vec_type='ifchange'):
    try:
        if isinstance(sentences,list):
            data = {'senlist':sentences,'type':vec_type}
...
def getSentenceSimi(text1,text2,precision,vec_type='ifchange'):
    try:
        data = {'text1':text1,'text2':text2,'precision':precision,'type':vec_type}

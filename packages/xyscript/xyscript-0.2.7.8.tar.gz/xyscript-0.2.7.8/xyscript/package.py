...
class Package:
    ...
    def _platform_package(self,shell_str):
        ...
        if "fastlane.tools just saved" in text or "fastlane.tools finished successfully" in text:
            UPLOAD_TESTFLIGHT_COUNT = 1
            return True
        elif "An error occurred while uploading the package" in text or "An error occurred while uploading the file" in text or "[!] Error uploading ipa file, for more information see above" in text  and UPLOAD_TESTFLIGHT_COUNT <= 3:
            faillog("upload to testflight failed " + UPLOAD_TESTFLIGHT_COUNT + " times")
            UPLOAD_TESTFLIGHT_COUNT = UPLOAD_TESTFLIGHT_COUNT +1
            warninglog("start uploading " + UPLOAD_TESTFLIGHT_COUNT + " time")
            self.reupload_update_testflight()
        else:
            UPLOAD_TESTFLIGHT_COUNT = 1
            return False

    def reupload_update_testflight(self):
        shell_str = "fastlane packagetestflight2 platform:testflight"
        self._platform_package(shell_str)

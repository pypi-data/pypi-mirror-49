...
def CreateGraphBar(DS, AGGREGATION_TYPE, AGGREGATION_ATTRIBUTE_NAME, BAR_ATTRIBUTE_NAME, LOOP_ATTRIBUTE_NAME, SERIES_ATTRIBUTE_NAME, GRAPH_TITLE, 
    ...
    if(AGGREGATION_TYPE == "COUNT"):
        WORKING_DS['ID'] = range(1, len(WORKING_DS) + 1)
    ...
    if(AGGREGATION_TYPE == "SUM"):
        SUMMARIZED_DS = WORKING_DS.groupby([LOOP_ATTRIBUTE_NAME, BAR_ATTRIBUTE_NAME, SERIES_ATTRIBUTE_NAME]).sum()
    elif (AGGREGATION_TYPE == "COUNT"):
        SUMMARIZED_DS = WORKING_DS.groupby([LOOP_ATTRIBUTE_NAME, BAR_ATTRIBUTE_NAME, SERIES_ATTRIBUTE_NAME]).count()

    max_values = SUMMARIZED_DS.max(axis=0)
...
#            max_value = max_values[i]
    ...
    for loopItem in LOOP_ITEMS:
        ...
        if(AGGREGATION_TYPE == "SUM"):
            SUMMARIZED_DS = SUMMARIZED_DS.pivot_table(AGGREGATION_ATTRIBUTE_NAME, BAR_ATTRIBUTE_NAME, SERIES_ATTRIBUTE_NAME, aggfunc="sum")
        elif (AGGREGATION_TYPE == "COUNT"):
        ...
        plt.title("{} {} {}".format(GRAPH_TITLE, LOOP_ATTRIBUTE_NAME, loopItem))

...
def est_t_gg_ent(conp):
    user,password,ip,db,schema=conp
    sql="""
    create table %s.t_gg_ent (
    html_key bigint,
    href text  not null  ,
    quyu text not null,
...
#为 gg表新增\删除分区
def add_partition_t_gg_ent(quyu,conp):
    user,password,ip,db,schema=conp
    sql="alter table %s.t_gg_ent add partition %s values('%s')"%(schema,quyu,quyu)
    db_command(sql,dbtype='postgresql',conp=conp)

def drop_partition_t_gg_ent(quyu,conp):
    user,password,ip,db,schema=conp
    sql="alter table %s.t_gg_ent drop partition for('%s')"%(schema,quyu)
    db_command(sql,dbtype='postgresql',conp=conp)



def update_t_gg_ent(quyu,conp):

    user,password,ip,db,schema=conp

    sql="""
    insert into mine.t_gg_ent_1_prt_%s 


    select html_key ,href,quyu,mine.findent(txt) as txt from mine.t_gg_txt_1_prt_%s as b 

    where not exists(select 1 from mine.t_gg_ent_1_prt_%s as a where a.html_key=b.html_key ) limit 10000
...
def add_quyu(quyu,conp_hawq):
    ...
    FROM pg_partitions
    WHERE tablename='t_gg_ent' and schemaname='%s'
    """%(schema)
    ...
    if quyu in df["partitionname"].values:
        ...
        add_partition_t_gg_ent(quyu,conp_hawq)
    print("2、准备更新t_gg_ent表--%s"%quyu)
    update_t_gg_ent(quyu,conp_hawq)
...
def restart_quyu(quyu,conp_hawq):
    ...
    if quyu in df["partitionname"].values:
        print("%s-partition已经存在"%quyu)
        drop_partition_t_gg_ent(quyu,conp_hawq)

"""
Randomized LassoSklearn/Logistic: feature selection based on LassoSklearn and
...
from .least_angle import lars_path, LassoSklearnLarsIC
from .logistic import LogisticRegressionSklearn
...
@deprecated("The class RandomizedLassoSklearn is deprecated in 0.19"
            " and will be removed in 0.21.")
class RandomizedLassoSklearn(BaseRandomizedLinearModel):
    """Randomized LassoSklearn.

    Randomized LassoSklearn works by subsampling the training data and
    computing a LassoSklearn estimate where the penalty of a random subset of
    ...
    alpha : float, 'aic', or 'bic', optional
        The regularization parameter alpha parameter in the LassoSklearn.
    ...
    Examples
    --------
    >>> from h2o4gpu.linear_model import RandomizedLassoSklearn
    >>> randomized_lasso = RandomizedLassoSklearn() # doctest: +SKIP
    ...
    See also
    --------
    RandomizedLogisticRegressionSklearn, LassoSklearn, ElasticNetSklearn
    """
    ...
    def _make_estimator_and_params(self, X, y):
        ...
        if isinstance(alpha, six.string_types) and alpha in ('aic', 'bic'):
            model = LassoSklearnLarsIC(precompute=self.precompute,
...
def _randomized_logistic(X, y, weights, mask, C=1., verbose=False,
    ...
    for this_C, this_scores in zip(C, scores.T):
        ...
        clf = LogisticRegressionSklearn(C=this_C, tol=tol, penalty='l1', dual=False,
...
@deprecated("The class RandomizedLogisticRegressionSklearn is deprecated in 0.19"
            " and will be removed in 0.21.")
class RandomizedLogisticRegressionSklearn(BaseRandomizedLinearModel):
    """Randomized Logistic Regression

    Randomized Logistic Regression works by subsampling the training
    data and fitting a L1-penalized LogisticRegressionSklearn model where the
    ...
    C : float or array-like of shape [n_reg_parameter], optional, default=1
        The regularization parameter C in the LogisticRegressionSklearn.
        When C is an array, fit will take each regularization parameter in C
        one by one for LogisticRegressionSklearn and store results for each one
    ...
    tol : float, optional, default=1e-3
         tolerance for stopping criteria of LogisticRegressionSklearn
    ...
    Examples
    --------
    >>> from h2o4gpu.linear_model import RandomizedLogisticRegressionSklearn
    >>> randomized_logistic = RandomizedLogisticRegressionSklearn() # doctest: +SKIP
    ...
    See also
    --------
    RandomizedLassoSklearn, LogisticRegressionSklearn
    """
...
def lasso_stability_path(X, y, scaling=0.5, random_state=None,
                         ...
                         verbose=False):
    """Stability path based on randomized LassoSklearn estimates

...
def load_inp_mesh(fn, meshname=None):
    reader = inp.InpReader(fn)
    if meshname is None:
...
# source model with material properties we want to transfer to the target model
source_surf_fn = 
source_inp_fn = 
source_mat_fn = 
source_surf = vtktools.loadpoly(source_surf_fn)
source_mesh = load_inp_mesh(source_inp_fn)
source_mat = np.loadtxt(source_mat_file)

# target model we want to map material properties to
target_surf_fn = 
target_inp_fn = 
target_surf = vtktools.loadpoly(target_surf_fn)
target_mesh = load_inp_mesh(target_inp_fn)

# output text file with per-element material property
out_mat_fn = 

#=============================================================#
# rigidly register source surf points to target surf points
init_trans = (target_surf.v.mean(0) - source_surf.v.mean(0)).tolist()
...
target_tree = cKDTree(target_surf.v)
...
def slave_func_tpsp(x):
    ...
    d = sourcetree.query(target_surf.v)[0]
    return d
...
def slave_func_2way(x):
    sourcetree = cKDTree(x)
    d_tpsp = sourcetree.query(target_surf.v)[0]
...
source_elem_centroids = source_mesh.calcElemCentroids()
target_elem_centroids = target_mesh.calcElemCentroids()

# for each target centroid find closest source centroid
source_centroid_tree = cKDTree(source_elem_centroids)
centroid_dist, centroid_mapping = source_centroid_tree.query(
                                    target_elem_centroids,
                                    k=closestElems,
                                    )
...
np.savetxt(output_mat_file, np.vstack([target_mesh.elemNumbers, target_mat]).T)

#=============================================================#
# view
v = fieldvi.Fieldvi()
v.addData('target surface', target_surf.v, renderArgs={'mode':'point', 'color':(1,0,0)})
v.addData('source surface', source_surf.v, renderArgs={'mode':'point'})

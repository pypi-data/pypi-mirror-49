...
#--------Constructor ---------------------------  
    def __init__(self, username, password, dataSource=None, config=None):
        if (config):
            ...
            self.certs = None if parser.get('ssl_verify', 'certificate') == '' else parser.get('ssl_verify', 'certificate')
...
#-------Helper Functions---------------------------------------------------
        ...
        try:
            ...
            if (self.certs):
                json_Response = requests.post(token_url, json=json_tokenReq,
                                              proxies={"http":self.proxy}, verify=self.certs).json()
            else:
                json_Response = requests.post(token_url, json=json_tokenReq,
    ...
    def _getProxy(self, proxyServ, proxyUser, proxyPwd, proxyPrt=None):
       if(proxyUser):
           proxyUrl = "http://" + proxyUser +":" + proxyPwd + "@" + proxyServ
       else:

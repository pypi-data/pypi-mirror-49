...
let FrameworkControllerTrainingService = class FrameworkControllerTrainingService extends kubernetesTrainingService_1.KubernetesTrainingService {
    ...
    async uploadCodeFiles(trialJobId, trialLocalTempFolder) {
        ...
        if (this.fcClusterConfig.storageType === 'azureStorage') {
            ...
            try {
                ...
                ${this.azureStorageShare}/${path.join('nni', experimentStartupInfo_1.getExperimentId(), trialJobId, 'output')}`;
            }
            catch (error) {
                this.log.error(error);
    ...
    generateFrameworkControllerJobConfig(trialJobId, trialWorkingFolder, frameworkcontrollerJobName, podResources) {
        ...
        for (const index of this.fcTrialConfig.taskRoles.keys()) {
            ...
            const taskRole = this.generateTaskRoleConfig(trialWorkingFolder, this.fcTrialConfig.taskRoles[index].image, `run_${this.fcTrialConfig.taskRoles[index].name}.sh`, podResources[index], containerPort);
            taskRoles.push({

...
class Actuator(ModifierBase):
    ...
    def make(self, time_dim_name='time', **kwargs):
        ...
        filename = '{}_{}{}.nc'.format(
            self.name, self.med.cfg['area'],
            self.get_modifier_postfix(**kwargs))
        ...
        if self.task_mng.get('make'):
            ...
            if not self.cfg.get('no_verbose'):
                log.info('Making {} modifier from {} data-source'.format(
            ...
            ds = data_src.load(transform=Composer(*functions),
                               variables=self.input_variables, **kwargs)
            ...
            if not self.cfg.get('no_verbose'):
                log.info('{} modifier already initialized: reading '
                         'from {}'.format(self.name, filepath))
            # Read dataset and copy to unlock
            with xr.open_dataset(filepath) as ds:
                ds_us = ds.copy(deep=True)
    ...
    def apply(self, ds, time_dim_name='time', **kwargs):
        ...
        if not self.cfg.get('no_verbose'):
            log.info('Up-sampling dataset with {} modifier'.format(self.name))

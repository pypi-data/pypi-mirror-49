    ...
    class GridPhase(af.phase.as_grid_search(phase_imaging.LensSourcePlanePhase)):
        ...
        def pass_priors(self, results):
            ...
            if pl_hyper_background_sky:
                ...
                self.hyper_image_sky = results.last.inversion.constant.hyper_image_sky

            if pl_hyper_background_noise:

                self.hyper_noise_background = (
                    results.last.inversion.constant.hyper_noise_background
                )
        ...
        def pass_priors(self, results):
            ...
            if pl_hyper_galaxies:
                ...
                self.lens_galaxies.lens.hyper_galaxy = results.from_phase(
                    "phase_1_lens_sersic_pl_shear_source_inversion"
                ).hyper_galaxy.constant.lens_galaxies.lens.hyper_galaxy

                self.source_galaxies.source.hyper_galaxy = results.from_phase(
                    "phase_1_lens_sersic_pl_shear_source_inversion"
                ).hyper_galaxy.constant.source_galaxies.source.hyper_galaxy

            if pl_hyper_background_sky:

                self.hyper_image_sky = results.from_phase(
                    "phase_1_lens_sersic_pl_shear_source_inversion"
                ).inversion.constant.hyper_image_sky

            if pl_hyper_background_noise:

                self.hyper_noise_background = results.from_phase(
                    "phase_1_lens_sersic_pl_shear_source_inversion"
                ).inversion.constant.hyper_noise_background

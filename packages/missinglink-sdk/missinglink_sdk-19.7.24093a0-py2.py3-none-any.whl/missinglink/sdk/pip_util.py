...
class PipPackageProvider(PackageProvider):
    ...
    @classmethod
    def _latest_pre_release_versions(cls, all_versions):
        print_update_forced("Working with PRERELEASED version")
        ...
        for version in all_versions:
            if cls.is_pre_released_version(version):
                return version

        print_update_warning("NO PRERELEASED version found, fallback to latest stable")

        return None

    def _latest_version(self, package, pre_release):
        url = '{server}/{package}/json'.format(server=self._pip_server, package=package)
        r = requests.get(url)
        r.raise_for_status()

        package_info = r.json()
        all_versions = package_info['releases'].keys()
        latest_prerelease_version = None
        if pre_release:
            latest_prerelease_version = self._latest_pre_release_versions(all_versions)

        return latest_prerelease_version or package_info['info']['version']

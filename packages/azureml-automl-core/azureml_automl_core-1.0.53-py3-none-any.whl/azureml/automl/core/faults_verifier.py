...
from azureml.automl.core._vendor.automl.client.core.common.activity_logger import TelemetryActivityLogger
from azureml.automl.core.column_purpose_detection.types import StatsAndColumnPurposeType
from azureml.automl.core._vendor.automl.client.core.common.constants import AutoMLJson
...
class VerifierManager:
    ...
    LOCAL_RESULT_PATH = "verifier_results.json"

    def __init__(self):
        self.data_faults_names = []  # type: List[str]
        self.data_faults_dict = {}  # type: Dict[str, VerifiedFault]

    def _clear_obj(self) -> None:
        self.data_faults_names = []
        self.data_faults_dict = {}
    ...
    def get_json_str(self) -> str:
        return json.dumps([self.data_faults_dict[m].to_dict() for m in self.data_faults_names])
    ...
    def get_elaborate_json(self, run_context: AutoMLAbstractRunContext,
                           remote_path: str, logger: TelemetryActivityLogger) -> None:
        try:
            ...
            ret_dict = {"schema_type": AutoMLJson.SCHEMA_TYPE_FAULT_VERIFIER,
            ...
            with run_context.get_run():
                try:
                    file_path = os.path.join(remote_path, self.LOCAL_RESULT_PATH)
                    self.upload_to_file(run_context, ret_str, file_path)
                    self._clear_obj()
            ...
            logger.warning("Reference file, `faults_verifier_message.json` does not exist.")

    def to_json_file(self, path: str) -> None:
        with open(path, "w") as f:
            f.write(self.get_json_str())

    def to_json_file_run_id(self, run_id: str) -> None:
        self.to_json_file("{}_{}.json".format(run_id, "verifier_results"))

    def upload_to_artifact(self, automl_run_context: AutoMLAbstractRunContext) -> None:
        automl_run_context.save_str_output(
            self.get_json_str(), self.LOCAL_RESULT_PATH)
    ...
    def update_data_verifier_from_transformer(self, data_transformer: DataTransformer) -> None:
        """
        Detect missing value imputation in DataTransformer and update VerifierManager object with fault.

        :param data_transformer: DataTransformer object.
        :return: None
        """
        column_names = data_transformer._columns_types_mapping.keys() \
            if data_transformer._columns_types_mapping else []
        if not data_transformer.transformer_and_mapper_list:
            return
        for transformer_mapper in data_transformer.transformer_and_mapper_list:
            for f in transformer_mapper.mapper.features:
                if isinstance(f[0], List):
                    ...
                    col_name = f[0]
                if col_name in column_names and isinstance(f[1][0], Imputer):
                    ...
                    self.add_data_fault_parameter(VerifiedFaultsTypes.MISSING_VALUE_IMPUTATION, {
                        ...
                        ValidParameterKeys.IMPUTATION_TYPE: f[1][0].strategy
                    })
        if not self.has_fault_member(VerifiedFaultsTypes.MISSING_VALUE_IMPUTATION):
    ...
    def update_data_verifier_for_text_class_validation(self, stats_and_column_purpose:
        ...
        for col in stats_and_column_purpose:
            ...
            if column_purpose == FeatureTypeRecognizer.CategoricalHash or column_purpose == FeatureTypeRecognizer.Text:
                content_type = ValidParameterValues.CATEGORICALHASH \
                    if column_purpose == FeatureTypeRecognizer.CategoricalHash else ValidParameterValues.TEXT
                if not self.has_fault_member(VerifiedFaultsTypes.DETECT_HIGH_CARDINAL_FEATURES):
                    self.add_data_fault(
                        VerifiedFaultsTypes.DETECT_HIGH_CARDINAL_FEATURES, VerifierResults.FIXED)
        ...
        if not self.has_fault_member(VerifiedFaultsTypes.DETECT_HIGH_CARDINAL_FEATURES):
            self.add_data_fault(VerifiedFaultsTypes.DETECT_HIGH_CARDINAL_FEATURES, VerifierResults.DONE)

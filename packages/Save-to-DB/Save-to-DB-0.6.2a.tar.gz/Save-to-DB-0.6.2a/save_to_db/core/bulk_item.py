...
class BulkItem(ItemBase):
    ...
    def _genitem(self, key):
        """ Generates an item for the given **default** relation key and sets a
        reference for reverse relationship.
        
        :param key: Relation key for which to create related item instance.
        :returns: Generated item.
        """
        item = self.item_cls._cls_genitem(key)
        
        # forward relationship
        self.data[key] = item
    
        # backward relationship
        relation = self.item_cls.relations[key]
        reverse_key = relation['reverse_key']
        if reverse_key:
            item[reverse_key] = self
    ...
    def _get_direct(self, real_keys):
        ...
        if len(real_keys) == 1:
            self[real_key_str] = self._genitem(real_keys[-1])
        else:

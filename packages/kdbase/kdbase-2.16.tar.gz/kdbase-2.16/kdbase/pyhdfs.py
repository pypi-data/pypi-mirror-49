...
class HdfsClient(object):
    ...
    def __init__(self, hosts='localhost', randomize_hosts=True, user_name=None,
                 timeout=20, max_tries=2, retry_delay=5, requests_session=None,
                 requests_kwargs=None, kerberos_file_name = None):
        ...
        self._kerberos_file_name = kerberos_file_name
	...
	import os
	res = os.system('kinit -k -t %s %s' %(self._kerberos_file_name, self.user_name))
        if res != 0:
            msg = 'Kerberos failed,kerberos_file_name = %s, user_name = %s' % \
                    (self._kerberos_file_name, self.user_name) 
            raise OSError(msg)
        kwargs['op'] = op
        for i in range(self.max_tries):
            ...
            for host in hosts:
                try:
                    ...
                    gssapi_auth = HTTPSPNEGOAuth(mutual_authentication=DISABLED, sanitize_mutual_error_response=False)
                    response = self._requests_session.request(
                        method,
                        'https://{}{}{}'.format(host, WEBHDFS_PATH, url_quote(path.encode('utf-8'))),
                        auth=gssapi_auth, verify=False, params=kwargs, timeout=self.timeout, allow_redirects=False,
    ...
    def create(self, path, data, **kwargs):
        ...
        data_response = self._requests_session.put(
            metadata_response.headers['location'], verify=False, data=data, **self._requests_kwargs)
    ...
    def open(self, path, **kwargs):
        ...
        data_response = self._requests_session.get(
            metadata_response.headers['location'], verify=False, stream=True, **self._requests_kwargs)
        _check_response(data_response)
    ...
    def get_file_checksum(self, path, **kwargs):
        ...
        assert not metadata_response.content
        data_response = self._requests_session.get(
            metadata_response.headers['location'], verify=False, **self._requests_kwargs)
        _check_response(data_response)

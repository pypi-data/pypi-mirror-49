...
def filterquery(query , org_cleaned_data, incoming=False, paginate=True):
    ...
    if cleaned_data.get('reference'):
        query = query.filter(reference__contains=cleaned_data.pop('reference'))
    if cleaned_data.get('errortext'):
        query = query.filter(errortext__contains=cleaned_data.pop('errortext'))

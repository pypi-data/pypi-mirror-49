...
class Phase(object):
    def __init__(self, phase, prefix, queries=None):
        self._metrics = {}
        self._prefix = prefix
        self._parent_phase = phase
        self._store_all_metrics = False  # by default we store only the last metric
    ...
    def __log_metric_raw(self, name, value, total=1, apply_prefix=True):
    ...
    def log_metrics(self, metrics, is_custom=True):
        for name, value in metrics.items():
            self.log_metric(name, value, is_custom)
...
class PhaseEpoch(Phase):
    def __init__(self, parent_phase, prefix, epoch=None):
        super(PhaseEpoch, self).__init__(parent_phase, prefix)
        self.epoch = epoch


class PhaseTest(Phase):
    def __init__(self, parent_phase, prefix):
        super(PhaseTest, self).__init__(parent_phase, prefix)
        self._store_all_metrics = True
        self.y_test = []
        self.y_pred = []
        self.probabilities = []
        self.labels = []
    ...
    def use_data_generator(self, ml_data_generator):
        if not self._queries.is_empty:
            logging.warning("More than one iterator was used in a single test scope. "
...
class ExperimentCounters(object):
    ...
    def get_loop(self, context_validator, generator, post_data):
    ...
    def get_epoch_loop(self, context_validator, generator, post_data):
        should_reset = False
    ...
    def begin_iteration(self):
        self.set_iteration(self._iteration + 1)

    def begin_batch(self):
        self.set_batch(self._batch + 1)

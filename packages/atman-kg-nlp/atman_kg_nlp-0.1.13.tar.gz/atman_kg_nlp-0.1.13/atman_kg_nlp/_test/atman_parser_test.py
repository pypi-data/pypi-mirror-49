...
class TestPreprocessMethods(unittest.TestCase):
    ...
    def test_component(self):
        text = 'Methods\nStudy Design and Trial Oversight\nThe trial was ' \
               'conducted in 19 North American centers under an investiga' \
               'tional new drug application (see the protocol and statist' \
               'ical analytical plan in Supplement 1). A coordinating cen' \
               'ter, independent adjudication committee, echocardiographi' \
               'c core lab, and data and safety monitoring board oversaw ' \
               'the trial’s conduct. Institutional review boards of parti' \
               'cipating centers and the coordinating center approved the' \
               ' protocol, and written informed consent was obtained for ' \
               'all patients.\n2.1. Patients and Interventions\nThe target pop' \
               'ulation comprised adults with end-stage heart failure (is' \
               'chemic or nonischemic), who were hospitalized for a clini' \
               'cally indicated LVAD for a bridge to a heart transplant o' \
               'r to destination therapy for patients who are not candida' \
               'tes for heart transplants. All LVADs were required to be ' \
               'approved by either the US Food and Drug Administration or' \
               ' Health Canada and were required to be continuous-flow de' \
               'vices.'
        doc = self.benepar_en_parser.parse(text)
        section = list(doc.sections.values())[0]
        section.add_subsection(1, 5)
        section.add_subsection(5, 9)
        self.assertEqual(section.subsections[0].title, 'Study Design and '
                                                       'Trial Oversight')
        self.assertEqual(section.subsections[1].title, 'Patients and Inte'
                                                       'rventions')

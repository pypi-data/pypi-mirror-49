...
try:
    ...
    from utils.utime import strftime, NO_BLANK_FMT
except (ImportError, ModuleNotFoundError) as e:
    from pysubway.utils.hash import hash_md5
    from pysubway.utils.url import urlencode
    from pysubway.utils.utime import strftime, NO_BLANK_FMT
...
class TonglianBase:
    ...
    @staticmethod
    def gen_sign(biz_data: Dict[str, Any], key: str) -> str:
        """
        构造请求中的 reqData
        reqData=$(
        原始数据 -->加sign(
    ...
    def gen_req_data(self, biz_data: Dict[str, Any]) -> Dict[str, Any]:
        base_req_data = {
            'appid': self.appid,
            'cusid': self.cusid,
        ...
        base_req_data['sign'] = self.gen_sign(base_req_data, self.key)
    ...
    def request(self, biz_data: Dict[str, Any]) -> Dict[str, Any]:
        req_data = self.gen_req_data(biz_data)

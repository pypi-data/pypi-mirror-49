...
The cluster LDAP GET operation retrieves the LDAP configuration of the cluster.<br>
...
The cluster LDAP PATCH operation updates the LDAP configuration of the cluster.<br>
...
The cluster LDAP DELETE operation deletes the LDAP configuration of the cluster.<br>
...
class ClusterLdapSchema(ResourceSchema):
 ...
 """
    base_scope = fields.Str(validate=enum_validation(['base', 'onelevel', 'subtree']))
...
* subtree """
 ...
 """
    min_bind_level = fields.Str(validate=enum_validation(['anonymous', 'simple', 'sasl']))
...
* simple
* sasl """
    port = fields.Integer(validate=integer_validation(minimum=1, maximum=65535))
...
* Custom schema """
 ...
 """
    session_security = fields.Str(validate=enum_validation(['none', 'sign', 'seal']))
...
class ClusterLdap(Resource):
    ...
    get.__doc__ = r"""Retrieves the cluster LDAP configuration.
...
* [`DOC /security/authentication/cluster/ldap`](#docs-security-security_authentication_cluster_ldap)"""
    ...
    post.__doc__ = r"""A cluster can have only one LDAP configuration. IPv6 must be enabled if IPv6 family addresses are specified.
...
- use_start_tls
- session_security
Configuring more than one LDAP server is recommended to avoid a single point of failure. Both FQDNs and IP addresses are supported for the 'servers' field.
...
* [`DOC /security/authentication/cluster/ldap`](#docs-security-security_authentication_cluster_ldap)"""
    ...
    patch.__doc__ = r"""Both mandatory and optional parameters of the LDAP configuration can be updated.
IPv6 must be enabled if IPv6 family addresses are specified. Configuring more than one LDAP server is recommended to avoid a single point of failure. Both FQDNs and IP addresses are supported for the 'servers' field.
The LDAP servers are validated as part of this operation. LDAP validation fails in the following scenarios:<br/>
1. The server does not have LDAP installed.
2. The server is invalid.
3. The server is unreachable<br/>
...
* [`DOC /security/authentication/cluster/ldap`](#docs-security-security_authentication_cluster_ldap)"""
    ...
    delete.__doc__ = r"""The DELETE operation removes the LDAP configuration of the cluster.

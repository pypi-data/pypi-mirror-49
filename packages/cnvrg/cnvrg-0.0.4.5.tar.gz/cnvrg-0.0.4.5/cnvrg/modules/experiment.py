...
class Experiment(CnvrgJob):
    ...
    def init(project=None, **kwargs):
        ...
        logger_helper.log_message("{title} is running, monitor live at: {url}".format(title=exp['title'], url=exp.href()))
        return exp

    @staticmethod
    def __run_callable(callable, title=None, project=None, cwd=None, library=None, **kwargs):
        experiment = Experiment.init(title=title, project=project, library=library, **kwargs)
        exit_status = control_stdout_helper.run_callable(callable, callback=lambda x: experiment.log(x), err_callback=lambda x: experiment.log(x, log_type=LOGS_TYPE_ERROR))
        experiment.finish(exit_status=exit_status)
        return experiment

    @staticmethod
    def __run_local(cmd, **kwargs):
        callable = lambda: spawn_helper.run_sync(cmd, print_output=True, cwd=kwargs["cwd"])
        return Experiment.__run_callable(callable, cmd=cmd, **kwargs)

    @staticmethod
    def run(command=None, callable=None, title=None, project=None, compute=None, computes=None, datasets=None, dataset=None, local=False, library=None, working_directory=None):
        project = project or Project()
        ## support
        ...
        if callable:
            cmd = "func: {func_name}".format(func_name=callable.__name__)
            return Experiment.__run_callable(callable, title=title, project=project, library=library, cmd=cmd)
        elif local:
            return Experiment.__run_local(cmd=command, title=title, project=project, cwd=working_directory, library=library)
        else:
            e = project.run_task(command, title=title, templates=computes, datasets=datasets, library=library)

...
def f2(driver):
    ...
    while n < 6:
        try:
            time.sleep(2 * n)
            page_num = get_pageall(tt_url, payloadData)
            break
        except:
            n += 1
...
def get_pageall(tt_url, payloadData):
    user_agents = UserAgent()
    user_agent = user_agents.chrome
    headers = {
        'User-Agent': user_agent,
...
def f3(driver, url):
    driver.get(url)
    locator = (By.XPATH, "//div[@class='zw_c_c_cont'][string-length()>300]")
...
def get_data():
    ...
    sitewebId = OrderedDict([
        ("云浮市",
         "4028889705bedd7e0105bef70a69000c,40288ba95cae24c7015caefda85e5536,297e6a6a49176e8401491853b2a60ea0,297e6a6a49176e8401491851fd0f0e9f,297e6a6a49176e840149185101240e9e,297e6a6a49176e840149185034640e9d,297e6a6a49176e840149184f0a590e9c"),
    ])

    regionIds = OrderedDict([("云浮市", "0121")])
...
def work(conp, **args):
    est_meta_large(conp, data=data, diqu="广东省云浮市", **args)
...
if __name__ == '__main__':
    work(conp=["postgres", "since2015", "192.168.3.171", "guoziqiang", "yunfu"],pageloadtimeout=120,interval_page=10)

    # driver=webdriver.Chrome()
    # url = "http://www.gdgpo.gov.cn/queryMoreInfoList.do&channelCode=-3&sitewebName=云浮市,云浮新区,云安区,郁南县,新兴县,罗定市,云城区&sitewebId=4028889705bedd7e0105bef70a69000c,40288ba95cae24c7015caefda85e5536,297e6a6a49176e8401491853b2a60ea0,297e6a6a49176e8401491851fd0f0e9f,297e6a6a49176e840149185101240e9e,297e6a6a49176e840149185034640e9d,297e6a6a49176e840149184f0a590e9c&regionIds=0121"
    # driver.get(url)
    # df = f2(driver)
    # print(df)
    # driver=webdriver.Chrome()

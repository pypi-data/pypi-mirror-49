...
import gmsh
import trimesh
from geo_functions import unit_normal, convert_to_global, convert_to_local, polygon_area_3d
from material import Part as NewPart
...
class Face(GeoBaseClass):
    ...
    def __init__(self,
        ...
        self._BoundaryID = self._Boundary[0].ID
        ...
        if self._Holes.__len__() > 0:
            self._HoleIDs = list(x.ID for x in self._Holes)
        else:
            self._HoleIDs = []

        self._HoleCount = self._Holes.__len__()
    ...
    def create_poly_points(self):
        ...
        for i in range(1, self._Boundary[0].EdgeCount):
            if self._Points[-1] == self.Boundary[0].Edges[i].Vertex1:
                ...
                print('start of the edge is not the end of the previous edge')

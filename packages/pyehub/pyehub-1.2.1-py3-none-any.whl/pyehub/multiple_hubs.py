...
def linear_power_flow_constraint(power, angle_from, angle_to, time, reactance):
    ...
    """
    # pdb.set_trace()
    #FIXME: need to validate this constraint and see if the angles need to be added to the powerbalancing (yikes)
    for t in time:
...
def multiple_hubs(minimize_carbon=False, output_filename=None, input_files=None, network_excel=None, carbon_value=0, n=0):
    ...
    voltage_angle = {
        t: {node: RealVariable() for node in range(len(hubs))}
        for t in hubs[0].time
    }

...
class GPGTool(Command):
	...
	def _gpg_(self):
		...
		if self.__c == 0 or not self._gpgselfcheck:
			self._gpgcheck()
	...
	def _gpgcheck(self):
		...
		cmd = '%s --batch --always-trust -r %s -d'%(
            self.binary, self.key)
		if not self.gui:
			getin = input
			quest = '%s [Y/n]'%grn('could not authenticate, retry?')
			cmd = '%s --always-trust --pinentry-mode=loopback -r %s -d'%(
                self.binary, self.key)
		enc = self.stdo(
            'gpg -er %s'%' -r '.join(k for k in self.recvs), inputs=' ')
		for i in range(1, 3):
			out = self.stdo(cmd, inputs=enc)
			if out == ' ':
	...
	def sign(self, data, out=None):
		...
		tmp = '%s.___pwcliptmp___'%out
		try:
			move(out, tmp)
		except FileNotFoundError:
			pass
		sop = '--detach-sign' if self.dtg else '--sign'
		out = ' -o %s'%out if out else ''
		sign = None
		if path.isfile(data):
			cmd = '%s%s --batch --sign-with %s %s %s'%(
                self.binary, out, self.key, sop, data)
			sign, err, eno = self.oerc(cmd)
		else:
			cmd = '%s%s --batch --sign-with %s %s'%(
                self.binary, out, self.key, sop)
			sign, err, eno = self.oerc(cmd, inputs=str(data).encode())
		#print(data, out)
		#print(cmd)
		#print(sign, err, eno)
		try:
	...
	def encrypt(self, data, **kwargs):
		...
		crypt = self._gpg_.encrypt(str(msg), fingers, output=out)
		if not crypt.ok:

...
def standardize_owner(elem, check_field=True):
    if check_field:
        elem = standardize_str(elem, True)

        text_to_remove = r'CO LTD|COLTD|COMPANY LTD|CO LIMITED|COMPANY LIMITED|CV LIMITADA|LTD($)|LTDA($)|LIMITED($)| CO($)|LLC($)| SA($)| SL($)| SC($)| AS($)|JSC($)|PJSC($)|P JSC($)|OJSC($)|JOINT STOCK COMPANY($)|JOINTSTOCK COMPANY($)|CORPORATION($)|CORP($)|CORP SA($)|INCORPORATED($)|INC($)|CORP INC($)|GMBH($)| CB($)| CA($)| GIE($)|KABUSHIKI KAISHA($)| KK($)|PTY LTD($)|SA PTY LTD($)|COMPANY($)|COMPANY NV($)|CO NV($)| NV($)|LTDA EPP($)| EPP($)|COMPANY BV($)| BV($)'
        if type(elem) == pd.core.series.Series:
            elem = elem.apply(
                lambda x: unidecode(re.sub(r'[^\w\s]+', '', x)).strip() if (x == x) & (x != None) & (x != '') else None)
            elem = elem.apply(
                lambda x: re.sub(text_to_remove, '', x).strip() if (x == x) & (x != None) * (x != '') else None)
            return elem.apply(
            ...
            elem = unidecode(re.sub(r'[^\w\s]+', '', elem)).strip()
            elem = re.sub(text_to_remove, '', elem).strip()

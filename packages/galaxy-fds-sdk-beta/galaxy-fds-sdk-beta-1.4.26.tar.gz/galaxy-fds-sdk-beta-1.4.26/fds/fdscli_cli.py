...
@click.option("--part_size", default=50 * 1024 * 1024, help="Part size when multipart uploading")
@click.pass_context
...
def _download_batch(src_url, dst, recursive):
    src_bucket_name = src_url.bucket_name()
...
def _upload(fpath, object_name, dst_url, autodetect_mimetype):
    ...
    with open(str(fpath.resolve()), "rb") as f:
        file_length = os.path.getsize(str(fpath.resolve()))
        if file_length < multipart_upload_buffer_size:
            try:
                result = fds_client.put_object(
            ...
            try:
                ...
                while True:
                    ...
                    for i in range(max_upload_retry_time):
                        upload_result = None
                        try:
                            upload_result = fds_client.upload_part(
                                dst_bucket_name,
                                dst_object_name,
                ...
                result = fds_client.complete_multipart_upload(
                    upload_token.bucket_name,
                    upload_token.object_name,
                    upload_token.upload_id,
                    metadata,
                    json.dumps(upload_part_result),
                )
            except Exception as e:
                fds_client.abort_multipart_upload(
                    dst_bucket_name, dst_object_name, upload_token.upload_id
                )
                CLIPrinter.fail(e.message)
...
def _upload_batch(s_path, dst_url, recursive, autodetect_mimetype):
...
def sync(src, dst, autodetect_mimetype):
    ...
    if FDSURL.is_fds_url(src) and not FDSURL.is_fds_url(dst):
        ...
        dst_url = FDSURL(dst)
        s_path = Path(src)
        _upload_batch(s_path.resolve(), dst_url, True, autodetect_mimetype)

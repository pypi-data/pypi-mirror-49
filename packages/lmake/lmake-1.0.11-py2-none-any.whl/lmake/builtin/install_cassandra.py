...
__author__ = "Jason Zhu (zhurenqi@agora.io)"
...
URL_BASE = "datastax/cpp-driver.git"
...
def check_installed():
    lib_path = os.path.join(get_install_dir(), "lib", "libcassandra_static.a")
    if os.path.exists(lib_path):
        return True
    return False

def install():
    print "Download libcassandra source code"
    cache_dir = get_cache_dir()
    install_dir = get_install_dir()

    run_command(["mkdir", "-p", cache_dir])
    run_command(["mkdir", "-p", install_dir])
    if not os.path.exists(os.path.join(cache_dir, "libcassandra")):
      run_command(["git",
                   "clone",
                   "git@github.com:{}".format(URL_BASE),
                   "libcassandra"], cwd=cache_dir)

    cass_folder = os.path.join(cache_dir, "libcassandra")
    platform = get_platform()
    build_folder = os.path.join(cass_folder, "build." + str(platform))
    run_command(["mkdir", "-p", build_folder], cwd=cass_folder)
    ...
    run_command(["cmake",
                 ...
                 "-DLIBUV_ROOT_DIR={}".format(install_dir),
                 "-DCMAKE_INSTALL_PREFIX={}".format(install_dir),
                 ".."], cwd=build_folder)
    run_command(["make", "install"], cwd=build_folder)

def main():
    if check_installed():
        print "libcassandra has been installed"
        return

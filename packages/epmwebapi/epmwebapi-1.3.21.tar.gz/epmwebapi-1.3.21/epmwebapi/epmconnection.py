...
from .epmdataobject import EpmDataObject, EpmDataObjectPropertyNames, EpmDataObjectAttributeIds
...
class EpmConnection(object):
    ...
    def _getDataObjects(self, doNames, attributes, rootNode = '/1:DataObjects'):
        ...
        # Verifica se todos os itens existem
        results = self._read(paths, [ NodeAttributes.NodeId.value ] * len(paths)).items()
        ...
        for index in range(0, len(names)):
          if results[index][1].code == 0:
            epmVariables[names[index]] = EpmDataObject(self, names[index], results[index][0]._identity)
          else:

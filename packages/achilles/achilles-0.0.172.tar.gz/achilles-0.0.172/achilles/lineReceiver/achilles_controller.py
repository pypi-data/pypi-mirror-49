    ...
    def achilles_compute(self, achilles_config_path=None, response_mode=""):
        ...
        if self.achilles_function is None and self.achilles_args is None:
            ...
            with open(achilles_config_path, "r") as f:
                ...
                try:
                    args_path = f"{achilles_function_path}\\{achilles_config['ARGS_PATH']}"
                except KeyError:
                    args_path = None
                try:
    ...
    def init_achilles_compute(self):
        ...
        if self.response_mode in ["OBJECT", "SQLITE", "STREAM"]:
            self.achilles_compute(
                achilles_config_path=achilles_config_path, response_mode=self.response_mode
            )
        else:
            stderr.write(
...
def runAchillesController(
    achilles_function=None, achilles_args=None, achilles_callback=None, response_mode="OBJECT"
):
    try:
        if __name__ != "__main__":
            import achilles

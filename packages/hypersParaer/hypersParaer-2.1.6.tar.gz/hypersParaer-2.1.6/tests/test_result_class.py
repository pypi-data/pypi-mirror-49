...
class ResultClassTestCase(APITestCase):
    ...
    def test_csv(self):
        ...
        response = HTTPResult(request=request, paginator=paginator).response(data, filename='test.csv')
        self.assertEqual(response.get('content-type'), 'text/csv')
        self.assertEqual(response.get('content-type'), 'text/csv')
        self.assertEqual(response.get('Content-Disposition'), 'attachment; filename=test.csv')

        df = pd.DataFrame(data)
        response = HTTPResult(request=request, paginator=paginator).response(df, filename='test.csv')
        self.assertEqual(response.get('content-type'), 'text/csv')
        self.assertEqual(response.get('content-type'), 'text/csv')
        self.assertEqual(response.get('Content-Disposition'), 'attachment; filename=test.csv')

...
def check_is_pure(installed):
    for installpath in installed.values():
        if "site-packages" in installpath:
            if installpath.split('.')[-1] in ['dylib', 'so', 'dll']:
                return False
    return True
...
def prepare_metadata_for_build_wheel(metadata_directory,
    ...
    is_pure = check_is_pure(meson_introspect(builddir, 'projectinfo'))
...
class WheelBuilder:
    ...
    def build(self, wheel_directory, config_settings, metadata_dir):
        ...
        is_pure = check_is_pure(meson_introspect(self.builddir.name, 'projectinfo'))

        pyver = 'py{}'.format(sys.version_info.major)
        abi = 'none' if not is_pure else 'cp' + pyver[2:]
        platform_tag = 'any' if is_pure else '{}{}'.format(sys.platform, platform.machine())
        target_fp = wheel_directory / '{}-{}-{}-{}-any.whl'.format(

...
class AdminClient(base_client.BaseClient):
    ...
    def update_cluster_bastion_key(self, cluster_name, new_key):
        """Update the SSH bastion private key for a cluster

        Keyword arguments:
        cluster_name -- internal name of the cluster
        new_key -- new ssh private key for bastion access
        """
        payload = {
            "ssh_bastion_private_key": new_key,
        }
        url = "{}/{}/{}/{}".format(ADMIN_RESOURCE_URL, "clusters", cluster_name, "sshd_key")
        r = self.request("put", url, payload=payload)
        self.check_and_raise(r)

    def rotate_cluster_worker_key(self, cluster_name):
        """Rotates the worker key for a cluster

        Keyword arguments:
        cluster_name -- internal (global) name of the cluster
        """
        url = "{}/{}/{}/{}".format(ADMIN_RESOURCE_URL, "clusters", cluster_name, "rotate_worker_key")
        r = self.request("post", url)
        self.check_and_raise(r)

    def delete_aws_cluster(self, cluster_name):
        """Marks a cluster as deleted in the db

        Keyword arguments:
        cluster_name -- internal (global) name of the cluster
        """
        url = "{}/{}/{}/".format(ADMIN_RESOURCE_URL, "clusters", cluster_name)

...
@click.option("--cluster-id", type=int,
              help="""Default cluster ID for model servers unspecified in the YAML file""")
def apply(ctx, path, delete, owner, cluster_id):
            ...
            for config in configs:
                ...
                if config["specifier"] in model_server_map:
                    ...
                    if config["path"] != model_server_map[config["specifier"]]:
                        server_name, tag, owner_new, _ = getConfigInfo(config, owner, cluster_id)
                        with api_client_exception_handler():
                    ...
                    server_name, tag, owner_new, cluster_id_new = getConfigInfo(config, owner, cluster_id)
                    with api_client_exception_handler():
...
def getConfigInfo(config, owner, cluster_id):
    server_name, tag = get_name_and_tag(config["specifier"])
    owner = owner if "owner" not in config else config["owner"]
    cluster_id = cluster_id if "clusterID" not in config else config["clusterID"]
    return server_name, tag, owner, cluster_id

...
class Experiment(CnvrgJob):
    ...
    def init(title=None, project=None):
        ...
        resp = apis_helper.post(url_join(project.get_base_url(), 'experiments', 'local'),
                                data={"title": title, "commit": project.get_current_commit()})
    ...
    @staticmethod
    def __run_callable(callable, title=None, project=None):
    ...
    def __run_local(cmd, **kwargs):
        callable = lambda: spawn_helper.run_sync(cmd, True)
        return Experiment.__run_callable(callable, **kwargs)

    @staticmethod
    def run(command=None, callable=None, title=None, project=None, compute=None, computes=None, datasets=None, dataset=None, local=False):
        project = project or Project()
        ## support
        ...
        if callable:
            ...
            return Experiment.__run_local(cmd=command, title=title, project=project)
        else:

...
from core.auth.algorithm import sha_hmac1 as mac1
from core.utils import parameter_helper as helper
from core.http import format_type as FormatType
...
# accessKeyId: this is access_key_id
# format: XML or JSON
...
def refresh_sign_parameters(parameters, format=FormatType.RAW, signer=mac1):
    ...
    parameters["x-click-signature-method"] = signer.get_signer_name()
    parameters["x-click-signature-version"] = signer.get_signer_version()
    return parameters
...
def compose_string_to_sign(
    ...
    sign_to_string += build_canonical_headers(headers, "x-click-")
...
def get_signature_headers(
    ...
    signature = get_signature(
        ...
        method,
        signer)
    headers["Authorization"] = "click " + str(access_key) + ":" + str(signature)
    return headers

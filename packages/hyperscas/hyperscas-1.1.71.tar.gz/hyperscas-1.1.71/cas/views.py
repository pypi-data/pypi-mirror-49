...
def logout(request, next_page=None):
    ...
    CAS_LOGOUT_COMPLETELY = getattr(settings, 'CAS_LOGOUT_COMPLETELY', default=None)
...
def profile(request: Request) -> Response:
    ...
    try:
        hacResponse = requests.get(url, timeout=10, headers=request.domain.hac.identify).json()
    except Exception:
        hacResponse = {}

    userInfo = hacResponse.get('result', {})
    if userInfo:
        data.update(
            avatar=userInfo.get('avatar', ''),
            avatarThumb=userInfo.get('avatarThumb', ''),
            theme=userInfo.get('theme', ''),
            apps=userInfo.get("apps", [])
        )
    else:
        pass
    request.session[settings.LANGUAGE_SESSION_KEY] = hacResponse.get('language', settings.LANGUAGE_CODE)
...
def settings(request: Request):
    ...
    return Response(status=302, data={'url': redirectUrl})

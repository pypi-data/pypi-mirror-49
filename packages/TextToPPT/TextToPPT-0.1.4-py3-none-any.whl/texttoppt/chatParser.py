...
class WhatsAppChatParser:
    ...
    def startdateFilter(self,line):   
        ...
        if(liveDateDays >= startDateDays):
            return True
        else:
            return False
    def enddateFilter(self,line):   
        Dpattern = re.compile(r"\d\d\/\d\d\/\d\d")
        m = Dpattern.findall(line)
        liveDate =  m[0]
        liveDateDays = (datetime.strptime(liveDate,"%d/%m/%y")-datetime(1970,1,1)).days
    ...
    def ExtractQuoteList(self, chatExportFile ):
        ...
        while True:
            ...
            if ( t ) :
               if self.startdateFilter(line) == True:
                    if self.enddateFilter(line) == True:
                        break
                    if ( insideMessage) :

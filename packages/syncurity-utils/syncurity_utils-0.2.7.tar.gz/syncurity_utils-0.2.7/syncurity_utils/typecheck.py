...
def typecheck(*args1, isclassmethod=True):
    ...
    def decorator(func):
        ...
        def wrapper(*args2, **keywords):
            ...
            # If there are more type arguments than arguments provided, we need to trim away the arguments that we're
            # not dealing with
            if len(list_args1) > len(all_args):
                ...
                # Sort ascending
                indices.sort()
                # subtract the value of the index at which each item in the list of indices appears - we know there will
                # not be duplicates in this list because parameters have unique names, and therefore a search through a
                # list of parameters for two unique parameter names could not return the same index.
                #
                # We do this because as each type is removed from the list of types provided to the decorator, the total
                # length of that list decreases, we start by removing the type at the lowest index, then the next
                # lowest, etc. Each time we remove one, the next one we need to remove will slide down one index as the
                # list gets smaller.
                #
                # For example - if we have a list of types [int, int, str, str, dict] that correspond to the signature
                # func(a, b, c='c', d='d', e={'value': 'e'}), and the function is called as
                # func(10, 11, e={'value': 'newval'}), the indices at which we are missing parameters are 2 and 3. If we
                # were to remove the type at index 2 (str) the list of types would be [int, int, str, dict] - if we then
                # tried to remove the type at index 3 (now dict), we'd be removing the wrong type, thus we subtract the
                # value of the index at which the item in the indices list appears, we will remove the value knowing
                # that the size of the list of types has decreased by one for each item removed

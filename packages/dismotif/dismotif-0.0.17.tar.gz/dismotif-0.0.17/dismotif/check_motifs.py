...
def main():
    ...
    variants_15bp_buffer = variants.each(add_buffer)
    variants_15bp_buffer = variants_15bp_buffer.sequence(fi=pyhg19.PATH)
    print(variants)
    print(BedTool(parse_fimo_results(fimo(variants_15bp_buffer.seqfn))))
    vars_motifs = variants.intersect(
        BedTool(parse_fimo_results(fimo(variants_15bp_buffer.seqfn))),
        wa=True,
        wb=True
    )
    print(vars_motifs)

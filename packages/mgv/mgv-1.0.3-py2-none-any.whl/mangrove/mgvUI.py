...
from Qt import QtGui, QtCore, QtSvg, QtXml, QtCompat
...
class MgvWidgetItem(QtGui.QGraphicsItem):
    """Widget UI object."""
    obj = 'widget'
...
class MgvHud(QtGui.QGraphicsItem):
    """Users UI object."""
    obj = 'hud'
...
class MgvSelectBox(QtGui.QGraphicsItem):
...
class MgvArrow(QtGui.QGraphicsItem):
...
class MgvGraphView(QtGui.QGraphicsView):
    """Nodal graph UI."""
    ...
    def deleteNodes(self, liste, delfiles):
        ...
        if len(liste):
            a = QtGui.QMessageBox(self.gui.root)
            if delfiles:
            ...
            a1 = QtGui.QPushButton('Yes')
            a.addButton(a1, QtGui.QMessageBox.YesRole)
            a3 = QtGui.QPushButton('No')
            a.addButton(a3, QtGui.QMessageBox.NoRole)
            ...
            if not r:
                ...
                for y in [x.node for x in liste]:
                    if y.delete(remove_files=delfiles):
                        ...
                        QtGui.QMessageBox.information(self.gui.root, 'error', "Can't delete  %s's folder" % y.getName())
                if len(ok):
                    ...
                    if self.hotbox.selected == action:
                        ...
                        if self.hotbox.node.getUser() == self.graph.user or self.hotbox.force:
                            ...
                            if len(action.warning):
                                result = QtGui.QMessageBox.question(self.gui.root, 'Warning', action.warning,
...
class MgvHotBoxAction(QtGui.QGraphicsItem):
    """Action UI object."""
    obj = 'action'
...
class MgvHotBoxMenu(QtGui.QGraphicsItem):
...
class BinCategory(QtGui.QGraphicsItem):
...
class BinNode(QtGui.QGraphicsItem):
    ...
    def paint(self, painter, option, widget):
        ...
        if self.over:
            pen.setColor(QtGui.QColor('#CCCCCC'))
            painter.setBrush(QtGui.QColor('#3366BB'))
        else:
...
class IconView(QtGui.QGraphicsView):
    """Left UI panel."""
...
class MgvLinkItem(QtGui.QGraphicsItem):
    """Link UI object."""
    obj = 'link'
...
class MgvGroupItem(QtGui.QGraphicsItem):
    """Group UI object."""
    obj = 'group'
...
class MgvNodeItem(QtGui.QGraphicsItem):
    """Node UI object."""
    obj = 'node'
...
class MgvTabCompleter(QtGui.QGraphicsItem):
...
class MgvTypeEditor(QtGui.QMainWindow):
    """Type editor UI."""
    def __init__(self, gui, typ):
        ...
        #pixmap.loadFromData(QtCore.QByteArray.fromBase64(self.type.getImage()))
    ...
    def addOutput(self, outFile=None):
        if outFile is None:
    ...
    def addLink(self, i):
        if i > 0:
            ...
            if typ.uuid not in self.type.getLinkWith():
                ...
                toto = QtGui.QPushButton(typ.getName())
    ...
    def currentParamNameChanged(self):
        ...
        if newname != self.currentParam.getName():
            if not (newname in [x.getName() for x in self.currentVersion.parameters]):
                ...
                self.errorBox = QtGui.QMessageBox(self.root)
    ...
    def currentNameChanged(self):
        ...
        if newname != self.type.getName():
            if newname not in [x.getName() for x in self.project.types]:
                ...
                self.errorBox = QtGui.QMessageBox(self.gui.root)
                self.errorBox.setIcon(QtGui.QMessageBox.Critical)
                self.errorBox.setWindowTitle('Error')
                self.errorBox.setText('This name already exists !')
                self.errorBox.open()
    ...
    def currentShapePathDialog(self):
        ...
        out = QtGui.QFileDialog.getOpenFileName(self, 'Select file', path)[0]
        if len(out):
...
class MgvActionItem(QtGui.QListWidgetItem):
    """Action list widget item."""
    def __init__(self, text):
        super(MgvActionItem, self).__init__(text)


class MgvActionWindow(QtGui.QDockWidget):
            ...
            if self.dico[i.id].process:
                if self.dico[i.id].process.poll() is not None:
                    ...
                    self.ui.listWidget.setStyleSheet("""
                        ...
                        QListView::item:selected{ background-color: #3366BB; color: #999999; font: 18px}""")
                else:
                    # en cours
                    self.ui.listWidget.setStyleSheet("""
                        QListView{ background-color: #4B4B4B; color: #CCCCCC; font: 18px}
                        QListView::item:selected{ background-color: #3366BB; color: #EE9933; font: 18px}""")
            else:
                # pas fait
                self.ui.listWidget.setStyleSheet("""
                    QListView{ background-color: #4B4B4B; color: #CCCCCC; font: 18px}
                    QListView::item:selected{ background-color: #3366BB; color: #CCCCCC; font: 18px}""")
...
class MangroveGUI(object):
    ...
    def __init__(self, app):
        ...
        self.iconFilter = QtGui.QLineEdit()
        ...
        self.iconHide = QtGui.QPushButton('<<')
        ...
        self.iconHide.setStyleSheet("""
            ...
            QPushButton::pressed{  background-color: #3366BB; color: #CCCCCC; font: 8px}""")
        v = QtGui.QVBoxLayout()
        h = QtGui.QHBoxLayout()
        ...
        if os.path.exists(self.lastFile):
            with open(self.lastFile) as fid:
                lines = fid.readlines()
            if len(lines):
                result = QtGui.QMessageBox.question(self.root, 'Open', 'Open last graphs ?',
    ...
    def requested(self, name, address, port):
        QtGui.QMessageBox.information(self.root, 'Free', 'Freeing %s: Request sent to %s:%s' % (name, address, port))
    ...
    def networkReceive(self, text, client):
        ...
        if len(params) > 0:
                    ...
                    for node in graphview.graph.nodes:
                        if node.getName() == nodename:
                            ...
                            if text.startswith('MGVUNLOCK'):
                                ...
                                ok = QtGui.QMessageBox.question(self.root, 'Unlock request',
            ...
            print >> sys.__stderr__, 'BAD INCOMING MESSAGE: "%s"' % text
            ...
            if not graphview.stopRequest:
                ...
                if not os.path.islink(os.path.join(patha, f)):
                    ...
                    try:
                        ...
                        print >> sys.__stderr__, "Can't copy %s to %s !" % (os.path.join(patha, f),
                                                                            os.path.join(pathb, link))
                else:
    ...
    def merge(self, nodes):
        ...
        a = QtGui.QMessageBox(self.root)
        a.setText("Copy associated files ?")
        a1 = QtGui.QPushButton('Yes')
        a2 = QtGui.QPushButton('No')
        a.addButton(a1, QtGui.QMessageBox.YesRole)
        a.addButton(a2, QtGui.QMessageBox.NoRole)
        a.setDefaultButton(a2)
        c = QtGui.QCheckBox('Flatten versions')
        c.setChecked(True)
    ...
    def newFavorites(self):
        name, ok = QtGui.QInputDialog.getText(self.root, 'New Favorite', 'New favorite name :', text='current')
        if ok:
    ...
    def readFavorites(self):
        ...
        for fav in root:
            ...
            if self.currentProject.getName() == proj:
                menu = QtGui.QMenu(name, self.root.menuFavorites)
                self.root.menuFavorites.addMenu(menu)
                action = QtGui.QAction('Open all', menu)
        ...
        action = QtGui.QAction('New...', self.root.menuFavorites)
    ...
    def readProjects(self):
        ...
        projgroup = QtGui.QActionGroup(self.root.menuProject)
        projgroup.setExclusive(True)
        for project in project_names:
            projaction = QtGui.QAction(project, self.root.menuProject)
        ...
        projnew = QtGui.QWidgetAction(self.root.menuProject)
        label = QtGui.QLabel('    New project')
        label.setStyleSheet('QLabel{font: italic} QLabel:hover{background-color: #3366BB}')
    ...
    def newProject(self):
        self.readProjects()
        text, ok = QtGui.QInputDialog.getText(self.root, 'New Project', 'Project name :', text='new')
        text = str(text)
        if ok:
            project_names = mgvWrapper.getProjectNames()
    ...
    def createTemplateMenu(self):
        ...
        if self.currentProject:
            for pattern in self.currentProject.patterns:
                menu = QtGui.QMenu(pattern.getName(), self.root.menuTemplates)
                self.root.menuTemplates.addMenu(menu)
                self.menusT.append(menu)
                for template in pattern.templates:
                    action = QtGui.QAction(template.getName(), menu)
                    menu.addAction(action)
    ...
    def createOpenMenu(self):
        ...
        if self.currentProject:
            for pattern in sorted(self.currentProject.patterns, key=lambda x: x.order):
                menu = QtGui.QMenu(pattern.getName(), self.root.menuOpen)
    ...
    def filterMenuRec(self, filtertext, menu, dico, pattern):
        ...
        for key in keys:
            if isinstance(dico[key], dict):
                m = QtGui.QMenu(key, menu)
                menu.addMenu(m)
                self.filterMenuRec('', m, dico[key], pattern)
            else:
                shotaction = QtGui.QAction(pattern.convertGraphName(dico[key]), menu)
    ...
    def filterMenu(self, filtertext, mastermenu, pattern):
        ...
        if len(filtertext) or len(self.dicoPaths[pattern]) < 20:
            ...
            for recent in self.recent:
                ...
                action = QtGui.QAction(name, mastermenu)
    ...
    def removeType(self, typ):
        result = QtGui.QMessageBox.question(self.root, 'Warning',
    ...
    def exportType(self, typ):
        result = QtGui.QFileDialog.getSaveFileName(self.root, 'Export Type', '', selectedFilter='*.json')
        if result[0]:
            try:
    ...
    def newType(self):
        text, ok = QtGui.QInputDialog.getText(self.root, 'New Type', 'New type name :', text='Type')
        text = str(text)
        if ok:
            ...
            if len(_templates):
                if not len(graphview.graph.template_name):
                    ...
                    for i, t in enumerate(_templates):
                        v = QtGui.QVBoxLayout()
                        b = QtGui.QPushButton('' if show_icons else t.getName())
                        b.setMinimumWidth(80)
                        v.addWidget(b)
                        v.setSpacing(3)
                        if show_icons:
                            label = QtGui.QLabel(t.getName())
...
KEYx :\t\t\tGraph key number x"""
        self.childWindow = QtGui.QMessageBox.about(self.root, 'Mangrove variables', text)

    def about(self):
        text = """Mangrove v1.0
Written by Djelloul Bekri
Bug reports or suggestions: mangrove@theyard-vfx.com
"""
        self.childWindow = QtGui.QMessageBox.about(self.root, 'About', text)
...
--------------------------------------------------------------------------------------"""
        self.childWindow = QtGui.QMessageBox.about(self.root, 'Mangrove hotkeys', text)
    ...
    def closeEvent(self, event):
        ...
        result = QtGui.QMessageBox.question(self.root, 'Do you want to quit ?', msg,
                                            QtGui.QMessageBox.Yes | QtGui.QMessageBox.No, QtGui.QMessageBox.No)
...
class MgvEnvEditor(QtGui.QDockWidget):
...
class MgvRename(object):
    ...
    def __init__(self, editor):
        ...
        QtGui.QWidget.connect(self.root.nameEdit, QtCore.SIGNAL('textChanged(QString)'),
                              lambda a, x=self.root.treeViewNew: self.refresh(x))
        QtGui.QWidget.connect(self.root.cancelButton, QtCore.SIGNAL('clicked()'), self.cancel)
        QtGui.QWidget.connect(self.root.applyButton, QtCore.SIGNAL('clicked()'), self.apply)
...
class MgvGroupEditor(QtGui.QDockWidget):
    """Group UI editor."""
...
class MgvNodeEditor(QtGui.QDockWidget):
    """Node UI editor."""
    ...
    def viewMeta(self):
        ...
        QtGui.QMessageBox.information(self.root, 'Metadatas of %s' % self.node.getName(), s)
    ...
    def viewNodeMeta(self):
        ...
        QtGui.QMessageBox.information(self.root, 'Metadatas of %s, version %s' % (self.node.getName(),
    ...
    def readValues(self):
        ...
        label = QtGui.QLabel('Type Version')
            ...
            for param in self.node.type.getParameters(self.node.versionActive.typeForceVersion):
                if param.visibility:
                    ...
                    if param.type == 'int':
                        ...
                        label = QtGui.QLabel(param.getName())
                        label.setMinimumWidth(100)
                        edit = QtGui.QSpinBox()
                        ...
                        QtGui.QWidget.connect(edit, QtCore.SIGNAL('valueChanged(int)'),
                                              lambda v, p=param.getName(), lab=label: self.intChanged(v, p, lab))
                    ...
                    if param.type == 'bool':
                        ...
                        QtGui.QWidget.connect(edit, QtCore.SIGNAL('stateChanged(int)'),
                                              lambda v, p=param.getName(), lab=label: self.boolChanged(v, p, lab))
                    if param.type == 'float':
                        layout = QtGui.QHBoxLayout()
                        layout.setContentsMargins(1, 1, 1, 1)
                        label = QtGui.QLabel(param.getName())
                        ...
                        QtGui.QWidget.connect(edit, QtCore.SIGNAL('valueChanged(double)'),
                                              lambda v, p=param.getName(), lab=label: self.floatChanged(v, p, lab))
        ...
        if last is None:
            self.spacerItem = QtGui.QSpacerItem(5, 5, QtGui.QSizePolicy.Expanding, QtGui.QSizePolicy.Expanding)
    ...
    def versionPopup(self):
        ...
        menu = QtGui.QMenu(self.root)
        actions = []
        pad = self.node.graph.pattern.project.versions_padding
        for v in self.node.versions:
            addAction = QtGui.QWidgetAction(menu)
            lab = QtGui.QLabel(str(v.id).zfill(pad) + ' - ' + v.comment.split('\n')[0])
            lab.setStyleSheet('QLabel{font: %s;} QLabel:hover{background-color: #3366BB; font: %s;}' % (
    ...
    def delVersion(self):
        ...
        result = QtGui.QMessageBox.question(self.root, 'Warning', 'Are you sure to delete the version %s ?\n%s' % (
            self.node.versionActive.id, txt), QtGui.QMessageBox.Yes | QtGui.QMessageBox.No, QtGui.QMessageBox.No)
        if result == QtGui.QMessageBox.Yes:
...
class MgvVariableWidget(object):
    ...
    def __init__(self, gui, ver, layout, variable):
        ...
        QtGui.QWidget.connect(self.widget.delButton, QtCore.SIGNAL('clicked()'), self.delete)
        self.toggle(save=False)
        QtGui.QWidget.connect(self.widget.checkBox, QtCore.SIGNAL('stateChanged(int)'), lambda a: self.toggle())
        QtGui.QWidget.connect(self.widget.nameEdit, QtCore.SIGNAL('textChanged(QString)'),
                              lambda a: self.nameChanged(a))
        QtGui.QWidget.connect(self.widget.valueEdit, QtCore.SIGNAL('textChanged(QString)'),
                              lambda a: self.valueChanged(a))
...
""".replace('url(:', 'url(' + self.gui.mgvDirectory))
            ...
            b = QtGui.QPushButton('Add Variable')
    ...
    def addVar(self):
        name, ok = QtGui.QInputDialog.getText(self.root, 'New name', 'Script name :')
        if ok:
            result = [x.text().split(':') for x in self.root.resultListWidget.selectedItems()]
            for x in result:
    ...
    def scriptRename(self):
        script_name = self.root.scriptComboBox.currentText()
        text, ok = QtGui.QInputDialog.getText(self.root, 'New name', 'Script name :', text=script_name)
        text = str(text)
        if ok:
            if text != script_name:
                if text not in self.gui.currentProject.batchScripts:
                    ...
                    QtGui.QMessageBox.information(self.root, 'Error', '%s already exists' % text)
    ...
    def go(self):
        ...
        if self.root.deleteRadioButton.isChecked():
            out = QtGui.QMessageBox.question(self.root, 'Warning', 'Delete graphs folders as well ?',
                                                QtGui.QMessageBox.Yes | QtGui.QMessageBox.No, QtGui.QMessageBox.No)
            deleteFolder = (out == QtGui.QMessageBox.Yes)
            for i, x in enumerate(result):
...
class MgvCreateGraph(object):
    ...
    def create(self):
        ...
        if self.root.tabWidget.currentIndex() == 0:
            ...
            if not mgvWrapper.graphExists(self.pattern, graphpath):
                ...
                QtGui.QMessageBox.information(self.root, 'Error', 'Graph already exists')
        else:
            ...
            if len(error):
                go = False
                result = QtGui.QMessageBox.question(self.root, 'Errors',
    ...
    def chooseFile(self):
        ...
        while not os.path.exists(path) and c < 30:
            ...
            c += 1
        out = QtGui.QFileDialog.getOpenFileName(self.root, "Select a file", path, filter="Excel File (*.xls *.xlsx)")[0]
        if len(out):
    ...
    def preview(self):
        ...
        for i, previewColumn in enumerate(previewColumns):
            hitem = QtGui.QTableWidgetItem(str(previewColumn[0]+1))
...
class MgvTransfert(object):
    ...
    def getPath(self):
        result = QtGui.QFileDialog.getOpenFileName(self.root, 'Get Type File', '', '*.json')
        if result[0]:
    ...
    def exportElements(self):
        ...
        result = QtGui.QFileDialog.getSaveFileName(self.root, 'Export', '', selectedFilter='*.json')
        if result[0]:
            try:
                with open(result[0], 'w') as fid:
...
class MgvProjectSettings(object):
    ...
    def __init__(self, gui, project):
        ...
        self.root.addPatternButton.setStyleSheet("""
            ...
            QPushButton::pressed{ outline: 0; background-color: #3366BB; color: #CCCCCC; border-radius: 6;
    ...
    def templateAdd(self, pattern, widget):
        text, ok = QtGui.QInputDialog.getText(self.root, 'New template', 'Template name :', text='new')
        text = str(text)
        if ok:
    ...
    def templateIconSelect(self, widget):
        out = QtGui.QFileDialog.getOpenFileName(self.root, 'Select an icon')[0]
        if len(out):
            widget.templateIconLineEdit.setText(out)
            self.templateIconChanged(widget)

    def patternSelect(self, pattern, widget):
        out = QtGui.QFileDialog.getOpenFileName(self.root, 'Select a pattern graph')[0]
        if len(out):
...
class MgvLocalSettings(object):
    ...
    def folderPath(self):
        out = QtGui.QFileDialog.getExistingDirectory(self.root, 'Projects data folder', '')
        if out:
    ...
    def save(self):
        ...
        if self.wrapperModule != 'mgvWrapper' + self.root.wrapperComboBox.currentText() or True:
            QtGui.QMessageBox.information(self.root, 'Warning', "Wrapper changes need restart to take effect")

...
class API:
  ...
  def render (self, out):    
    ...
    if qss:
      ...
      for arg in qss:
        default = self.routeopt.get ('defaults', {}).get (arg)
        if default:
          need = "optional, default: {}".format (default)
        else:
          need = "required"  
...
def build_doc ():
  ...
  #with sys.stdout as out:
    ...
    for idx, (resource_id, api) in enumerate (sorted_apis):     
      out.write ("1. [{}](#{})".format (resource_id, resource_id.replace (".", "").lower ()))

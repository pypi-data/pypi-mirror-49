...
class Fist:
    ...
    def join(self):
        while True:
            ...
            if self.sig_num is not None:
                self.logger.info('[main_thread] ended (sig){}'.format(self.sig_num))
                break
    ...
    def run_sub(self):
        while self.sig_num is None:
            ...
            for sock in socks:
                ...
                f = Frame(ret)
                self.on_pub_frame(f)
        self.logger.debug('[sub_thread] ended (sig){}'.format(self.sig_num))
    ...
    def run_sub_master(self):
        if self.sub_master_sock:
            ...
            self.logger.debug('[sub_master_thread] ended (sig){}'.format(self.sig_num))
    ...
    def run_pull(self):
        if self.pull_sock:
            ...
            self.logger.debug('[pull_thread] ended (sig){}'.format(self.sig_num))
    ...
    def run_rep(self):
        if self.rep_sock:
            ...
            self.logger.debug('[rep_thread] ended (sig){}'.format(self.sig_num))

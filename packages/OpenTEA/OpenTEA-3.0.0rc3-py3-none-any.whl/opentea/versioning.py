...
Compute and check version compatibilities according to the semantic versioning
norm (c.f. http://semver.org/)

Heavily inspired from the work of Kostiantyn Rybnikov:
...
def match_versions(version, match_expr):
...
def correct_hip_version(version_string):
    """Hip versions are not semver compliant"""
    numbers = version_string.split('.')
    compliant_numbers = [str(int(numb)) for numb in numbers]
    if len(compliant_numbers) == 2:
        compliant_numbers += ["0"]
    return ".".join(compliant_numbers)

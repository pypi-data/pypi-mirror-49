...
class AdminCommands(object):
    ...
    def _is_legal_db_option(self, name, option_types):
        return option_types.get(self._with_prefix(name)) in ['SIMPLE', 'DATABASE_ONLY']
    ...
    def _normalize_options(self, proc_options_list, option_types):
        ...
        for proc_options in proc_options_list:
            ...
            for name, type in option_types.items():
                ...
                if name in proc_options and type in ['DATABASE_ONLY', 'RESTRICTED']:
                    del proc_options[name]
        ...
        if len(illegal_db_options) != 0:
            self._print(
                'WARNING: Database options specified inconsistently in capture file: {}',
                ', '.join(illegal_db_options))
        return db_options

    @Subcommand('prepare', 'database',
                help='display commands to create a nuoadmin database from a nuoagent capture file')
    @Argument('--capture-file', required=True,
    ...
    @Argument('--server-mapping', nargs='*',
              ...
              help='mapping of stable ID in nuoagent to corresponding server ID in nuoadmin')
    def prepare_database(self, capture_file, server_mapping=None):
        ...
        te_server_ids = []
        te_options = []
        for te_spec in db_spec.findall('./TransactionEngines/TransactionEngine'):
        ...
        commands = []
        commands.append('  export NUOCMD_DBA_USER=theDbaUser          # ATTENTION: replace with your actual DBA username')
        commands.append('  export NUOCMD_DBA_PASSWORD=theDbaPassword  # ATTENTION: replace with your actual DBA password')
        for server_id, archive_path, journal_path in archive_specs:
            if journal_path is None:
                commands.append(
                    '  nuocmd create archive --db-name {} --server-id {} --archive-path {} --restored'
                    .format(db_name, server_id, archive_path))
            else:
                commands.append(
                    '  nuocmd create archive --db-name {} --server-id {} --archive-path {} --journal-path {} --restored'
                    .format(db_name, server_id, archive_path, journal_path))
        options_str = ''
        for name, value in sorted(db_options.items()):
            options_str += ' \\\n      {} {}'.format(name, value)
        commands.append(
            '  nuocmd create database --db-name {} --te-server-ids {} --default-options{}'
            .format(db_name, ' '.join(te_server_ids), options_str))
        # output commands
        self._print('To start database using nuoadmin, execute the following commands:')
    ...
    @Argument('--db-name', required=True, default=EnvironmentalDefault(),
              completer=CommandProcessor.get_db_names,
              help='the name of the database to list storage groups for')
    @Argument('--storage-group-format', default=EnvironmentalDefault(STORAGE_GROUP_FMT),
              help='format string for storage groups')
    @Argument('--process-format', default=EnvironmentalDefault(SHORT_PROCESS_FMT),
    ...
    def _get_engine_diagnose_info(self, process, engine_dir, descriptor,
        ...
        if include_cores:
            ...
            try:
                stream = self.conn.stream_system_dependencies(process.start_id)
                self._print('Downloading system dependencies and binary for {} ...', descriptor)
                self._download_stream(stream, os.path.join(directory, 'sysdepends.tgz'))
            except Exception:
                # Don't fail if we can't get system dependencies
                self._print('Unable to download system dependencies and binary for {}', descriptor)
                self._print_exc()
    ...
    @Argument('--check-liveness', required=False, default=None,
              help='Check that the process has responded within the specified number of seconds')
    @MutuallyExclusive(
        Argument('--timeout', default=None, type=float,
                 help="How long (in seconds) to wait for process check to pass"),
    ...
    def check_process(self, start_id, check_running=False, check_exited=False,
        ...
        if check_exited and check_liveness is not None:
            raise RuntimeError('argument --check-exited not allowed with argument --check-liveness')
        if timeout is None and wait_forever:
            timeout = CommandProcessor.FOREVER
        proc = None
        if timeout is not None:
            proc = self._await(lambda: self.conn.get_process(start_id), timeout)
        if not check_exited and type(proc) is nuodb_mgmt.ExitedProcess:
            raise RuntimeError('Process with Start ID {} has already exited: \'{}\''
    ...
    def _check_process(self, start_id, check_running, check_exited, check_liveness=None):
        proc = self.conn.get_process(start_id)
        if not check_exited and type(proc) is nuodb_mgmt.ExitedProcess:
            raise RuntimeError('Process with Start ID {} has already exited: \'{}\''
                               .format(start_id, proc.reason))
        if check_exited and type(proc) is not nuodb_mgmt.ExitedProcess:
            raise RuntimeError('Process with Start ID {} has not exited. Has state \'{}\''
                               .format(start_id, proc.engine_state))
        if check_running:
            if proc.engine_state != 'RUNNING':
                raise RuntimeError('Process with Start ID {} not in \'RUNNING\' state. Has state \'{}\''
                                   .format(start_id, proc.engine_state))
        if check_liveness is not None:
            if proc.last_ack is None:
                raise RuntimeError(
                    'Process with Start ID {} unresponsive for longer than 60 seconds'
                    .format(start_id))
            if proc.last_ack > check_liveness:
                raise RuntimeError(
                    'Process with Start ID {} unresponsive for the last {} seconds'
    ...
    @MutuallyExclusive(
        Argument('--timeout', default=None, type=float,
                 help='How long (in seconds) to wait for server check to pass'),
        Argument('--wait-forever', action='store_true', default=False,
    ...
    @Argument('--check-connected', action='store_true', default=False,
              help='Check that all servers are CONNECTED to the server performing check')
    ...
    def _check_servers(self, check_active=False, check_connected=False,
        ...
        if check_connected and len(disconnected) != 0:
            msg += 'Servers not CONNECTED to {}: {}\n'.format(
                api_server.id, ', '.join(disconnected))
    ...
    @MutuallyExclusive(
        Argument('--timeout', default=None, type=float,
                 help='How long (in seconds) to wait for database check to pass'),
        Argument('--wait-forever', action='store_true', default=False,
    ...
    @Argument('--num-processes', default=None,
              help='Check that the database contains the specified number of processes')
    @Argument('--check-liveness', default=None,
              help='Check that all processes have responded within the specified number of seconds')

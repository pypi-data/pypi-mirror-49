...
class _V1(object):
    ...
    def convolution(unused_ctx,
                    ...
                    strides=None):
        rank = data.shape.ndims - 2
        result = op.Convolution.function(
            data,
            kernel,
            strides=strides,
            padding=_convert_auto_pad(auto_pad, pads),
            ...
            grouping=op.ConvolutionGrouping.EXPLICIT,
...
class _V5(_V4):
    ...
    def reshape(unused_ctx, data, shape):
        ...
        if isinstance(shape, plaidml.tile.Value):
            if isinstance(shape.source.op, Constant):
                shape = opset_util.onnx_1d_tensor_to_tuple(shape.source.op.value)
            else:
                raise NotImplementedError(

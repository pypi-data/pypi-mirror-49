...
class CustomPriorPhase(phase_imaging.PhaseImaging):
    def pass_priors(self, results):
        ...
        self.galaxies.lens.mass.einstein_radius = af.GaussianPrior(
            mean=0.8, sigma=0.1
        )

        ### SOURCE LIGHT PRIORS ###

        self.galaxies.source.light.centre_0 = af.GaussianPrior(
            mean=0.0, sigma=0.1
        )

        self.galaxies.source.light.centre_1 = af.GaussianPrior(
            mean=0.0, sigma=0.1
        )

        self.galaxies.source.light.axis_ratio = af.GaussianPrior(
            mean=0.8, sigma=0.1
        )

        self.galaxies.source.light.phi = af.GaussianPrior(mean=90.0, sigma=10.0)

        self.galaxies.source.light.intensity = af.GaussianPrior(

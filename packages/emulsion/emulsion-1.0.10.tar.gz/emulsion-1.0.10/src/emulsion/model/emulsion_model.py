...
from   emulsion.tools.misc     import read_from_file, load_class, create_new_serial, create_weighted_random
                        ...
                        for variable, value in val.items():
                            ...
                            if variable != 'desc':
                                if value == 'random':
                                    ...
                                    # associate to a function that returns the successor of the state contained in the variable (or a random state if the variable was not defined)
                                    self._values[result] = lambda agent: agent.statevars[variable].successor if variable in agent.statevars else agent.model.state_machines[variable].get_random_state()
                                elif value == 'previous_state':
                                    result = '_previous_state_' + variable
                                    # associate to a function that returns the predecessor of the state contained in the variable (or a random state if the variable was not defined)
                                    self._values[result] = lambda agent: agent.statevars[variable].predecessor if variable in agent.statevars else agent.model.state_machines[variable].get_random_state()

...
class scalarField( object ):
    ...
    def setFieldParameters( self, p ):
        ...
        if len(p) != nEnsemblePoints:
            raise ValueError('ERROR: geometric_field.set_field_parameters: number of given parameters ( '+str(p_lengths[0])+') does not match number of field ensemble points ( '+str(n_ensemble_points)+')')
        else:
    ...
    def evaluateField( self, density, derivs=None ):
        ...
        if not derivs:
            ...
            V,D = self.ensembleFieldFunction.evaluate_field_in_mesh( density, parameters=self.fieldParameters, derivs=derivs )  
            return V, D
...
def fitScalarFieldEPDP( SF, data, evalD, ftol=None, xtol=None, epsfcn=None, maxit=None ):
    ...
    pOpt = leastsq( obj, SF.fieldParameters.copy().ravel(), ftol=ftol, xtol=xtol, epsfcn=epsfcn, maxfev=maxfev )[0][:,scipy.newaxis]
...
class fitGeomFieldToScalar( object ):
    ...
    def fitEPDP( self, evalD ):
        ...
        pOpt, SFFit = fitScalarFieldEPDP( self.SF, self.dataS, evalD, ftol=self.ftol, xtol=self.xtol, epsfcn=self.epsfcn, maxit=self.maxIt )
        return SFFit
    ...
    def fitDPEP( self, evalD ):
        ...
        # fit fC to data curvature
        pOpt, SFFit = fitScalarFieldDPEP( self.SF, self.dataS, evalD, dpepi, ftol=self.ftol, xtol=self.xtol, epsfcn=self.epsfcn, maxit=self.maxIt ) 
        return SFFit
    ...
    if (sobW==None) and (nW==None):
        ...
        if nW==None:
            ...
            def obj( x ):
                ...
                sys.stdout.write( '\rit: %(it)05i rms: %(rms)8.6f'%{'it':next(c), 'rms':scipy.sqrt((dData*dData).mean())} )
                sys.stdout.flush()
                return d

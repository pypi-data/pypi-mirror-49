...
class Actuator(FeatureExtractorBase):
    ...
    def __init__(self, output_variable, cfg=None, **kwargs):
        ...
        self.input_variables = {'surface_downward_radiation',
                                'surface_temperature'}
    ...
    def get_generation(self, ds, data_src, **kwargs):
        ...
        if freq_data in ['D', '1D']:
            ...
            if not self.cfg.get('no_verbose'):
                log.info('Getting daily-mean clearness index')
            ...
            if not self.cfg.get('no_verbose'):
                log.info('Up-sampling clearness index to hours')
        ...
        if not self.cfg.get('no_verbose'):
            log.info('Computing hourly global tilted surface radiation')
        self.global_tilted_surf = rc_hour.global_tilted_surf

        # Get hourly solar generation
        if not self.cfg.get('no_verbose'):
            log.info('Computing hourly photovoltaic generation')
        ...
        if self.med.cfg['frequency'] == 'day':
            ...
            if not self.cfg.get('no_verbose'):
                log.info('Resampling from hourly to daily frequency')
            for var_name in DATASET_NAMES:
                setattr(self, var_name, getattr(
                    self, var_name).resample(time='D').sum(

...
def send_facts(fact_group_id, fact_data, client=None, address=None, api_user=None, api_key=None):
    """ Send a fact group into a fact_group_id in IR-Flow, handle the response, and return accordingly

    Args:
        fact_group_id (int): The IR-Flow assigned ID of the fact group to which the provided facts will be submitted
    ...
    if not response['success']:
        ...
        raise SubmissionError('Could not submit facts to the fact group with id {}'.format(fact_group_id))
    else:
        return response

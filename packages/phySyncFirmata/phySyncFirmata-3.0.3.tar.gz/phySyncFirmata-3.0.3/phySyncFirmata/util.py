...
class Iterator(threading.Thread):
    def __init__(self, board):
        ...
        self.tutorial_mode=self.board.tutorial_mode
    ...
    def run(self):
        ...
        if self.tutorial_mode:
            print("run: Independent iterator thread started to avoid stackover flow on controllerside.")
        while self.running:
            try:
    ...
    def stop(self):
        self.running = False
        if self.debug:
            print("stop: Independent iterator thread started to avoid stackover flow on controllerside.")
...
def init_logging_commands(logger):
    
    # logger = logging.getLogger(__name__)

    logger.setLevel(logging.DEBUG)
    logger.propagate = False
    formatter = logging.Formatter('%(asctime)s : %(name)s : %(module)s  :  %(funcName)s : %(lineno)d : %(levelname)s : \n\t %(message)s')

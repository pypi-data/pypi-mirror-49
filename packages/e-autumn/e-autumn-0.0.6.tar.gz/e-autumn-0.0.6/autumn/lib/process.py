...
def time_limit_function(func, time_limit: int, *args, raise_error = True,  **kwargs):
    """
...
def process_data(data, *args, func=default_process, timeout=1, mode="loop", pool_size=None, **kwargs):
    ...
    if mode == "total":
        ...
        pool.map_async(partial(time_limit_function, **kwargs), [(item,)+args for item in data], timeout,
                       callback=process_collect_result, error_callback=process_collect_error)

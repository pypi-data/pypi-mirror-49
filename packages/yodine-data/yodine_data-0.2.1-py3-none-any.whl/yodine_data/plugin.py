...
def loaded(loader: ModLoader):
    ...
    class T_Sign(EntityTemplate):
        ...
        default_components = [
            ...
            ('sizerandom', 90),
            ('collides',),
            ('bounding_box', [120 * 50 / 200, 46 * 50 / 200]),
            ('message', ''),
            ('anglerandom',),
            ('sizerandom', 25)
        ]
    ...
    class S_SizeRandom(System):
        ...
        def on_spawned(self, event: EventContext, sizerandom, **kwargs):
            ...
            bump = random.uniform(-entity['sizerandom'].value, entity['sizerandom'].value)
    ...
    class S_Readable(System):
        ...
        def render(self, entity: Entity, window: pyglet.window.Window, position, message, **kwargs):
            pos = ComponentVector(position)
            dist = (self.manager.camera - pos).size()
    ...
    class S_NormalRender(System):
        ...
        def on_loaded(self, event: EventContext, position, angle, sprite, **kwargs):
            entity = event.source
    ...
    class S_PlayerMovement(System):
        ...
        def tick(self, entity: Entity, dtime: float, position, velocity, speed, toward, angle, localplayer, **kwargs):
            ...
            self.manager.set_camera(*position.value, angle.value, 1.3 / (1 + math.sqrt(vel.size()) / 40))
            ...
            if tow.sqsize() > 0:
                delta_vel = tow.rotate(math.pi / 2 + angle.value).unit() * speed
                self.manager.game.net_emit(entity, ('player', 'thrust'), delta_vel)

            tow.x = 0
            tow.y = 0

        def on_player_thrust(self, entity: Entity, delta_vel: Vector, velocity, speed, **kwargs):
            vel = ComponentVector(velocity)
            delta_vel = delta_vel.unit() * speed.value
            vel += delta_vel
        ...
        def tick(self, entity: Entity, dtime: float, position, velocity, speed, toward, angle, ai_state, delta_angle, **kwargs):
            ...
            if ai_state.value == 'idle':
                ...
                if random.uniform(0, 1) ** dtime < 0.5:
                    delta_angle.value += random.uniform(-math.pi / 5, math.pi / 5)

        ...
        **kwargs
        ...
        if status == 422:
            ...
            if isinstance(data, collections.Iterable) and not (
                ...
                httpResponse = handleContentType(self.request, data, filename, header)
                if httpResponse:
                    return httpResponse
...
def handleContentType(request: Request, data: list, filename: str, header: typing.List[str]) -> HttpResponse:
    if request.META.get("CONTENT_TYPE") == "text/csv":
        return handleCsv(request, data, filename, header)


def handleCsv(request: Request, data: list, filename, header: typing.List[str]) -> HttpResponse:
    if isinstance(data, pd.DataFrame):
        df = data
    else:
        df = pd.DataFrame(data)
    response = HttpResponse(content_type="text/csv")
    df.to_csv(response, index=False, header=header or False, encoding="utf-8-sig")
...
class MixinView(object):
    ...
    def result_class(self):
        ...
        return functools.partial(
            self.result_klass, serializer=self.serializer_class, paginator=paginator, request=self.request
        )

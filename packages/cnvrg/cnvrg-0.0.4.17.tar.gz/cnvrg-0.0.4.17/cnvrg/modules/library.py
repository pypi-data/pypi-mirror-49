...
class Library(CnvrgBase):
    ...
    def run(self, computes=None, compute=None, datasets=None, dataset=None, image=None, commit=None, title=None, local=True, **kwargs):
        ## support
        ...
        if "local" in computes or len(computes) == 0: return self.__run_local(title=title, **kwargs)
    ...
    def __run_local(self, title=None, **kwargs):
        if not self.project:
        ...
        return [Experiment.run(raw_script, title=title, project=self.project, local=True, working_directory=self.__lib_path(), library=self.__library, **kwargs)]

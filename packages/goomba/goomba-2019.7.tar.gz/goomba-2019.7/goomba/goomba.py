...
import os, sys, glob, time, operator, uuid, pickle, gzip, bz2, zipfile, subprocess, datetime
...
# Objects
__all__ = ["get_unique_identifier","get_timestamp", "format_duration", "format_filename", "format_path", "format_header", "is_nonstring_iterable", "is_file_like","is_file_like", "boolean","create_directory","assert_acceptable_arguments","validate_file_existence","write_object","read_object","DisplayablePath", "get_directory_size","Command","ExecutablePipeline"]
__doc__ = """
...
def validate_file_existence(*paths, minimum_filesize=2, f_verbose=None):
    for path in paths:
        assert os.path.exists(path), "The following file does not exist: {}".format(path)
        filesize = os.path.getsize(path)
        assert filesize >= minimum_filesize, "The following file appears to be empty ({} bytes): {}".format(filesize, path)
        if is_file_like(f_verbose) or (f_verbose is None):
            print("File exists ({} bytes):".format(filesize), path, file=f_verbose)
...
def write_object(obj, path:str, compression="infer",  protocol=pickle.HIGHEST_PROTOCOL, *args):
    """
    Extensions:
    pickle ==> .pkl
    dill ==> .dill
...
def read_object(path:str, compression="infer"):
    path = format_path(path, str)
...
class Command(object):
    ...
    * Something like this:
    ...
    # Run command
    def run(self, prologue=None, epilogue=None, errors_ok=False, dry="infer", checkpoint=None, write_stdout=None, write_stderr=None, write_returncode=None, close_file=False, checkpoint_message_notexists="Running...", checkpoint_message_exists="Loading...", error_message=None, f_verbose=None, n_linebreaks=1, acceptable_returncodes=[0]):
        """
...
class ExecutablePipeline(object):
    ...
    def compile(self):
        ...
        for id, attrs in sorted(self.executables.items(), key=lambda x:x[1]["step"]):
            ...
            if attrs["log_prefix"] in {"infer", None}:
                attrs["log_prefix"] = "__".join([str(attrs["step"]), id])
            # Checkpoint
            ...
            if self.log_directory is not None:
                ...
                attrs["error_message"] = "Please check the log files: cat {}.*".format(os.path.join(self.log_directory,  "{}.*".format(attrs["log_prefix"] )))
    ...
    # Execute pipeline
    def execute(self, steps=None, description="Executing pipeline"):
        start_time = time.time()
        ...
        for step, id in tqdm(self.compiled_steps_.items(), desc=description):
            if step in steps:
                ...
                if attrs["validate_inputs"]:
                    ...
                    if bool(input_filepaths):
                        ...
                        validate_file_existence(*input_filepaths, f_verbose=self.f_verbose)
                # Execute
                executable.run(
                    prologue=id,
                    dry=attrs["dry"],
                ...
                if attrs["validate_outputs"]:
                    ...
                    if bool(output_filepaths):
                        ...
                        validate_file_existence(*output_filepaths)
                print("\nDuration: {}".format(format_duration(start_time)), file=self.f_verbose)
...
def main():
    ...
    with open(os.path.join(directories["output"], "commands.sh"), "w") as f_cmds:
        ep = ExecutablePipeline(name="Sith", description="The rule of two", f_cmds=f_cmds, checkpoint_directory=directories["checkpoints"], log_directory=directories["logs"])
        # Step 1
        output_filepaths = [os.path.join(directories["output"], "holocron.txt")]
        ep.add_step(id="Darth Bane",
                    cmd="echo 'Two there should be; no more, no less. One to embody the power, the other to crave it.' > {}".format(output_filepaths[0]),
                    output_filepaths=output_filepaths,
                    description = "Begin the rule of two",
       )
        # Step 2
        input_filepaths = [os.path.join(directories["output"], "holocron.txt")]
        output_filepaths = [os.path.join(directories["output"], "wisdom.txt")]
        ep.add_step(id="Darth Plagueis",
                    cmd="(cat > {} && echo 'jedi') > {} ".format(input_filepaths[0], output_filepaths[0]),
                    input_filepaths=input_filepaths,
                    output_filepaths=output_filepaths,

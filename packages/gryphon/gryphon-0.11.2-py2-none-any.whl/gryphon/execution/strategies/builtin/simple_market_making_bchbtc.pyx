...
class SimpleMarketMakingBCHBTC(Strategy):
    def __init__(self, db, harness, strategy_configuration):
        super(SimpleMarketMakingBCHBTC, self).__init__(db, harness)

        # Configurable properties with defaults.
        self.spread = Decimal('0.01')
        self.base_volume = Money('0.03', 'BCH')
    ...
    def configure(self, strategy_configuration):
        super(SimpleMarketMakingBCHBTC, self).configure(strategy_configuration)

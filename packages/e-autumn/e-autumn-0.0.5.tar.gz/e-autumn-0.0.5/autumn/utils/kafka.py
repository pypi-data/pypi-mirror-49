...
class KafkaConsumer(Consumer):
    def __init__(self, servers: str, topics: list, group: str):
        ...
        super().__init__({
            ...
            'group.id': ''.join(random.sample(['z','y','x','w','v','u','t','s','r','q','p','o','n','m','l','k','j','i','h','g','f','e','d','c','b','a'], 5)),
...
class KafkaMonitor(KafkaConsumer):
    ...
    def poll(self):
        ...
        while True:
            # 出现了新的分配/再平衡
            try:
                self.executor.kafka_arrangement = self.executor.queue.get_nowait()
            except:
                pass
            if self.executor.kafka_arrangement == {}:
                self.has_assigned = {}
                time.sleep(5)

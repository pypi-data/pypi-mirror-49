...
def gcjs_zb(f):
    def warp(*krg):
        ...
        try:
            ...
            time.sleep(0.5)
            val = driver.find_element_by_xpath("//span[@class='totalPageNum']").text.strip()
            driver.find_element_by_xpath("//div[@id='JYXXSearch']").click()
            locator = (By.XPATH, "//span[@class='totalPageNum'][not(contains(string(), '%s'))]"% val)
...
def f2(driver):
    ...
    if '/FJBID_DATA_LIST.aspx' in url:
        ...
        locator = (By.XPATH, "//div[@id='list']/div[last()]//a")
        WebDriverWait(driver, 10).until(EC.presence_of_element_located(locator))
        num = driver.find_element_by_xpath("//span[@class='totalPageNum']").text.strip()
        driver.quit()
        return int(num)
...
data = [
    ...
    ["gcjs_zhongbiaohx_gg",
     "https://www.fjggfw.gov.cn/Website/JYXXNew.aspxhx",
...
if __name__=='__main__':
    ...
    #     # print(url)
    #     # driver.get(url)
    #     # df = zfcg_zhongb(f2)(driver)
    #     # print(df)
    #     driver = webdriver.Chrome()
    #     driver.get(url)
    #
    #     df=zfcg_zhongb(f1)(driver, 12)
    #     print(df.values)

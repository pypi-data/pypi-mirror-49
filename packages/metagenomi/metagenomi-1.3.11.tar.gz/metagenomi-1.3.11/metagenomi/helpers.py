...
def align(fafile, output=None, verbose=False):
    '''
    TODO: Verbosity not implemented
    '''
    muscle_exe = is_command('muscle')
    if muscle_exe is None:
        raise MuscleNotFoundError

    if output is None:
        output = fafile.rstrip('.fa')+'.alignment.fa'

    proc = subprocess.Popen(['muscle', '-in', fafile, '-out', output],
...
def avg_perc_identity(fafile):
    from Bio import AlignIO
    alnfile = align(fafile)
...
def download_file(s3_path, dir_to_dl, dry_run=False, overwrite=True, unzip=True, verbose=False):
    '''
    Downloads a folder from s3

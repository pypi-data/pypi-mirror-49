...
def enviroment(request: Request) -> Response:
    ...
    data['loginUrl'] = f'{request.domain.url}' + reverse('login')
    return Response(data)


@api_view(['GET'])
def profile(request: Request) -> Response:
    data = dict(username=request.user.username, email=request.user.email)
    ...
    try:
        ...
        hacResponse = {}
    result = hacResponse.get("result", {})
    #  如果是未登录用户的话, result 就会是None

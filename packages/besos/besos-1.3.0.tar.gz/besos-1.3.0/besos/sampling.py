...
class adaptive_sampler_lv():
    ...
    def __init__(self, P, P_out, n_new, problem, evaluator, reg, scaler=None, scaler_out=None, verbose = False):
        if len(P[:,0])<2*len(P[0,:]):
            print('Size of initial number of samples needs to be bigger than 2 times no. inputs.')
        self.P = P
        self.P_out = P_out
        self.n_new = n_new
    ...
    def run(self, no_iter):
        ...
        for i in range(no_iter):
            ...
            for p_new in self.P_new:
                self.N_P, self.S_P = update_Neighborhood(self.N_P, self.P, self.S_P, p_new)
            ...
            self.score[i] = self.reg.score(self.P, self.P_out)
    ...
    def pick_new_samples(self):
        ...
        E = LOLA_estimate(self.N_P, self.P, self.reg, scaler=self.scaler, scaler_out=self.scaler_out)
        ...
        for i in range(self.n_new):
            ...
            while len(candidates)<=1:
                ...
                ind+=1
            self.P_new[i,:] = select_newSamp(P_sorted[-i-1, :], self.N_P[:,:,ind_new][:,:,-i-1], candidates)
...
def MC_Voronoi_estimate(P, problem):
    ...
    S = dist_sampler(sampling.lhs, problem, 500) # for now 100 random samples per point to estimate Voronoi cell
...
def LOLA_estimate(N_P, P, model, scaler, scaler_out):
    ...
    E = np.empty([n,])
    for p_r in P:
        grad = Gradient_estimation(N_P[:, :, ind], p_r, model, scaler=scaler, scaler_out=scaler_out)
        E[ind] = Nonlinearity_measure(grad, N_P[:, :, ind], p_r, model, scaler=scaler, scaler_out=scaler_out)
        ind=ind+1
...
def init_Neighborhood(P, P_new=None, verbose=False):
    ...
    if np.any(P_new==None):
        ...
        N_P = np.empty([m,d,n])
        S_P = np.empty([n,])
        P_ref = P*1.0   # array of all samples to be considered as reference in the next for loop
    else:
...
def Gradient_estimation(N, p_r, model,scaler, scaler_out):
    ...
    for i in range(m):
            ...
            if scaler == None:
                ...
                F_mat[i] = scaler_out.inverse_transform(model.predict(scaler.transform([N[i, :]])))
...
def Nonlinearity_measure(grad, N, p_r, model, scaler, scaler_out):
    ...
    for i in range(len(N)):
        if scaler == None:
            E = E + abs(model.predict([N[i, :]]) -
                        (model.predict([p_r]) + np.dot(grad,(N[i, :]-p_r))))
        else:
            E = E + abs(scaler_out.inverse_transform(model.predict(scaler.transform([N[i, :]]))) -
                        (scaler_out.inverse_transform(model.predict(scaler.transform([p_r]))) + np.dot(grad,(N[i, :]-p_r))))
    return E

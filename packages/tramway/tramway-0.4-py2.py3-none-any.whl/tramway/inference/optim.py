...
def sdfpmin(func, p, args=(), minibatch=None, component_count=None,
        ...
        def _dfunc(p, ids, components):
                return sdfunc(func, p, ids, components, dfunc_h, dfunc_fac, args)
        #
        ndim = p.size
        fjp = np.array([ func(j, p, *args) for j in range(component_count) ])
        fp = np.sum(fjp)
        g = dfunc(scalar_func(range(component_count)), p, dfunc_h, dfunc_fac)
        hessian = np.eye(ndim, dtype=float)
        dp = -g
...
def sdfunc(func, yy, ids, components, h, CON2, args=()):
        ans = []
        for d in ids:
                ans_d = np.inf
...
def minimize_obfgs(minibatch, fun, x0, args=(), eta0=1., tau=1e2, c=1., l=1e-6, eps=1e-9,
        gtol=1e-4, gcount=10, ptol=1e-16, pcount=10, maxiter=500, maxcor=10, verbose=False):
        if verbose:
                def msg1(_i, _f, _dg):
                        _i = str(_i)
                        return 'At iterate {}{}\tf= {}{:E} \t|proj g| = {:E}\n'.format(
                                ' ' * max(0, 3 - len(_i)), _i, ' ' if 0 <= _f else '', _f, _dg)
                def msg2(_i, _t, _e=None):
                        _i = str(_i)
                        if _e is None:
                                return 'At iterate {}{}\t{}\n'.format(
                                        ' ' * max(0, 3 - len(_i)), _i, _t)
                        else:
                                return 'At iterate {}{}\t{}: {}\n'.format(
                                        ' ' * max(0, 3 - len(_i)), _i, _t, _e.args[0])
                cumt = 0.
                t0 = time.time()
        # precompute arguments for dfunc
        CON = 1.4
        ...
        try:
                ...
                def df(_x, _rows, _cols):
                        return sdfunc(fun, _x, _cols, _rows, dfunc_h, dfunc_fac, args)
                g = df(x0, rows, cols)
        except (TypeError, ValueError):
                cols = None
                twoways = False
                def df(_x, _rows):
                        return sdfunc(fun, _x, range(_x.size), _rows, dfunc_h, dfunc_fac, args)
                g = df(x0, rows)
        if g is None:
        ...
        if verbose:
                cumt += time.time() - t0
                try:
                        print(msg1(0, f(x, rows), proj))
                except ValueError as e:
                        print(msg2(0, 'ValueError', e))
                t0 = time.time()
        ...
        for t in range(1, maxiter):
                ...
                if verbose:
                        cumt += time.time() - t0
                        try:
                                print(msg1(t, f(x, rows), proj))
                        except ValueError as e:
                                print(msg2(t, 'ValueError', e))
                        t0 = time.time()
                if abs(proj) < ptol:
                        k2 += 1
                        if pcount <= k2:

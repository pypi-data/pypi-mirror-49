...
def sql_connect(database, db_type="sqlite", **kwargs):
    ...
    db_type : str
        type of Database to connect to. Currently supported:
            "sqlite" (using the sqlite3 package)
            "mysql"  (using the mysql-connector package)
    **kwargs : dict
        keyword arguments for the connect method. See the [sqlite3, mysql-connector] documentations for further information.
    """

    #make cursor accessable within the package
    global c

...
def portReadBinance(cryptos,column,timeInterval,start_date,end_date,api_key,api_secret,dateColumn = 'closeTime'):
  ...
  for crypto in cryptos:
    ...
    try:
      ...
      df = df.rename(columns={column: crypto})
      df['date'] = pd.to_datetime(pd.Series(df[dateColumn]),unit='ms')
      df.set_index('date', inplace=True)

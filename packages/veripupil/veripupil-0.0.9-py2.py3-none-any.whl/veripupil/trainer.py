...
class Trainer:
    ...
    def getTrainData(self, features, labels, indexes, hsplit):
        ...
        if self.seq2seq :
            ...
            print("length1:", length1, "np.shape(features)", np.shape(features))
            features1 = np.pad(features, ((0,0),(0,(length1)-np.shape(features)[1])), 'constant', constant_values=(0))
            first = array(features1)[idx].reshape((len(idx), self.sequence_length_one, self.feature_dimension_one))
            features2 = np.pad(features, ((0,0),(length1+1,(length2)-np.shape(features)[1])), 'constant', constant_values=(0))

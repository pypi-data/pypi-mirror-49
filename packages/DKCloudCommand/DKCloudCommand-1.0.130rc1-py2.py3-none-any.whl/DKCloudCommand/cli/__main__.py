    ...
    try:
        ...
        for item in filepaths:
            if any(invalid_separator in item for invalid_separator in invalid_separators):
                error_mesage = 'Files to be updated must be delimited by whitespace (i.e. \'file1.txt file2.txt\' as opposed to \'file1.txt,file2.txt\')'
        ...
        click.secho('%s - Updating File(s) (%s) in Recipe (%s) in Kitchen(%s) with message (%s)' %
                    (get_datetime(), filepaths, recipe, use_kitchen, message), fg='green')
        check_and_print(DKCloudCommandRunner.update_file(backend.dki, use_kitchen, recipe, recipe_dir, message, filepaths))
    except Exception as e:
        raise click.ClickException(str(e))

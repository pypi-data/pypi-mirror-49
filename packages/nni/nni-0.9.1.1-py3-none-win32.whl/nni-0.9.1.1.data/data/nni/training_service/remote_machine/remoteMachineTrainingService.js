...
let RemoteMachineTrainingService = class RemoteMachineTrainingService {
    ...
    async generateGpuMetricsCollectorScript(userName) {
        const gpuMetricCollectorScriptFolder = this.getLocalGpuMetricCollectorDir();
        await util_1.execMkdir(path.join(gpuMetricCollectorScriptFolder, userName));
        const gpuMetricsCollectorScriptPath = path.join(gpuMetricCollectorScriptFolder, userName, 'gpu_metrics_collector.sh');
    ...
    async runHostJob(form) {
        ...
        const sshClient = sshClientManager.getFirstSSHClient();

...
class Package:
    ...
    def _platform_package(self,shell_str):
        ...
        if "fastlane.tools just saved" in text or "fastlane.tools finished successfully" in text:
            UPLOAD_TESTFLIGHT_COUNT = 1
            return True
        elif "An error occurred while uploading the package" in text or "An error occurred while uploading the file" in text or "[!] Error uploading ipa file, for more information see above" in text or "fastlane finished with errors" in text and UPLOAD_TESTFLIGHT_COUNT <= 3:
            ...
            self._platform_package(shell_str1)
        else:
            UPLOAD_TESTFLIGHT_COUNT = 1
            return False

...
import core
from core.vendored.six.moves.urllib.parse import urlencode
from core.vendored.requests import codes

from core.exception.exceptions import ClientException
from core.exception.exceptions import ServerException
from core.exception import error_code, error_msg
from core.http.http_response import HttpResponse
from core.request import ClickRequest
from core.http import format_type
from core.auth.signers.signer_factory import SignerFactory
from core.request import CommonRequest

from core.endpoint.resolver_endpoint_request import ResolveEndpointRequest
from core.endpoint.default_endpoint_resolver import DefaultEndpointResolver
import core.retry.retry_policy as retry_policy
from core.retry.retry_condition import RetryCondition
from core.retry.retry_policy_context import RetryPolicyContext
import core.utils
import core.utils.parameter_helper
import core.utils.validation
from core.vendored.requests.structures import CaseInsensitiveDict
from core.vendored.requests.structures import OrderedDict

"""
Click default client module.
"""

DEFAULT_SDK_CONNECTION_TIMEOUT_IN_SECONDS = 10

# TODO: replace it with TimeoutHandler
_api_timeout_config_data = core.utils._load_json_from_data_dir("timeout_config.json")
...
class ClickClient:
    ...
    def default_user_agent():
        ...
        default_agent['Core'] = __import__('core').__version__
        default_agent['python-requests'] = __import__(
            'core.vendored.requests.__version__', globals(), locals(),
    ...
    def _make_http_response(self, endpoint, request, timeout, specific_signer=None):
        ...
        if body_params:
            ...
            body = json.dumps(body_params).encode(encoding='utf-8')
        ...
        header['x-sdk-server'] = request.get_product()
    ...
    def _implementation_of_do_action(self, request, signer=None):
        ...
        if isinstance(request, CommonRequest):
            request.trans_to_click_request()
    ...
    def _add_request_client_token(self, request):
        if hasattr(request, "set_ClientToken") and hasattr(request, "get_ClientToken"):
            ...
            if not client_token:
                ...
                client_token = core.utils.parameter_helper.get_uuid()  # up to 60 chars
    ...
    def _get_request_timeout(self, request):
        ...
        if timeout is None:
            ...
            core.utils.validation.assert_integer_positive(timeout, "timeout")
    ...
    def _handle_single_request(self, endpoint, request, timeout, signer):
        ...
        try:
            ...
            logger.error("HttpError occurred. Host:%s SDK-Version:%s ClientException:%s",
                         endpoint, core.__version__, str(exception))
    ...
    def do_action_with_exception(self, click_request):

        # set server response format as json, because this function will
        # parse the response so which format doesn't matter
        click_request.set_accept_format('JSON')
        status, headers, body, exception = self._implementation_of_do_action(click_request)

        if exception:
            raise exception
        logger.debug('Response received. Product:%s Response-body: %s',
                     click_request.get_product(), body)
        return body
    ...
    def do_action(self, click_request):
        warnings.warn(
    ...
    def get_response(self, click_request):
        return self.implementation_of_do_action(click_request)
    ...
    def set_stream_logger(self, log_level=logging.DEBUG, logger_name='core-openapi', stream=None,
                          format_string=None):
    ...
    def set_file_logger(self, path, log_level=logging.DEBUG, logger_name='core-openapi'):
        log = logging.getLogger(logger_name)
        log.setLevel(log_level)

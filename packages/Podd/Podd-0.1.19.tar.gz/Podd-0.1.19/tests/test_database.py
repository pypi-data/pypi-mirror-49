...
from podd.database import Database, Feed, Options, bootstrap_app
...
HOME = pathlib.Path.home() / 'Podcasts'
...
class TestDatabase(Setup):
    ...
    def test_add_podcast(self):
        ...
        with Database(DATABASE) as db:
            db.add_podcast(name=NAME, url=URL, directory=DIRECTORY, protected=False)
        conn = sqlite3.connect(DATABASE)
        cursor = conn.cursor()
    ...
    def test_remove_podcast(self):
        ...
        with Database(DATABASE) as db:
            db.add_podcast(name=NAME, url=URL, directory=DIRECTORY, protected=False)
            db.add_podcast(name=NAME, url=url2, directory=DIRECTORY, protected=False)

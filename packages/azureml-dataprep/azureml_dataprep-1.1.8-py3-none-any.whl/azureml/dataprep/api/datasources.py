...
class FileDataSource:
    ...
    def datasource_from_str(path: str) -> 'FileDataSource':
        ...
        elif adlsgen2_pattern.match(path):
            return ADLSGen2(path, 'token')
        else:
...
class ADLSGen2(FileDataSource):
    ...
    def __init__(self, path: Path, accessToken: str = None, refreshToken: str = None, tenant: str = None):
        ...
        if accessToken is None and refreshToken is None:
            access, refresh = get_az_cli_tokens(tenant)
            encoded_token = ADLSGen2._encode_token(access, refresh)
        else:
            encoded_token = ADLSGen2._encode_token(accessToken, refreshToken)
        resource_details = _extract_resource_details(path, ADLSGen2._register_secret(encoded_token))

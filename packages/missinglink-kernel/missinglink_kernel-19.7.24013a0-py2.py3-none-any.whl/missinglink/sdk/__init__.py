...
class _GlobalRootLoggerSniffer(logging.Handler):
    MAX_BACKLOGS = 1000

    def __init__(self):
        super(_GlobalRootLoggerSniffer, self).__init__(logging.DEBUG)
...
def catch_logs():
    ...
    global_root_logger_sniffer = _GlobalRootLoggerSniffer()
...
def _except_hook(exc_type, value, tb):
    ...
    if isinstance(value, MissingLinkException):
...
def update_sdk(latest_version, pipe_streams, throw_exception):
    ...
    return package_provider.install_package(require_package, pipe_streams=pipe_streams, throw_exception=throw_exception)
...
def self_update(pipe_streams=False, throw_exception=False):
    ...
    version = package_provider.get_dist_version('missinglink-sdk')
    ...
    latest_version = package_provider.latest_version('missinglink-sdk', throw_exception=throw_exception)

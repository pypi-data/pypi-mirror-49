...
from .material import Part, MatLayer, Material, Window
...
class Building:
    ...
    def scale(self, scale_factor):
        ...
        print('Setting EdgeLoops to bulk update')
        for edge_loop in self.EdgeLoops:
        ...
        print('Setting Faces to bulk update')
        for face in self.Faces:
            face.start_bulk_update()

        print('update EdgeLoops ')
        for edge_loop in self.EdgeLoops:
            edge_loop.end_bulk_update()

        print('Setting Zones to bulk update')
        for zone in self.Zones:
    ...
    def write_json(self):
        ...
        print('json sucessfully written to {}'.format(fout.name))
    ...
    def import_data_from_excel(self, filename, import_config):
        ...
        print('Successfully created dataframe from excel data input \n')
        ...
        print('Start creating materials, material layers and parts:  \n')
        ...
        for index, row in material_df.iterrows():
            print('Creating Material and Layer for: ', row['Name'], row['ID'])
        ...
        for new_window_id in new_window_ids:
            ...
            print('Creating Window part {}'.format(str(window_construction_series['ID'])))
        ...
        for new_id in new_part_ids.__iter__():
            print('Creating Window part {}'.format(str(new_id)))

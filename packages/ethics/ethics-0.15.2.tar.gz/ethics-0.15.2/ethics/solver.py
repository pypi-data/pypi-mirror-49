...
class Branch:
    ...
    def expandOr(self):
        for f in [g for g in self.formulae if isinstance(g, Or) and g not in self.expanded]:
            ...
            self.appendNew(f.f1)
            for h in [i for i in self.formulae if isinstance(i, Or) and i not in self.expanded and i.f1 == f.f2]:
                self.expanded.append(h)
                self.solver.tree[-1].expanded.append(h)
                self.appendNew(h.f2)
            for h in [i for i in self.formulae if isinstance(i, Or) and i not in self.expanded and i.f2 == f.f2]:
                self.expanded.append(h)
                self.solver.tree[-1].expanded.append(h)
                self.appendNew(h.f1)

...
class HTTPResult(BaseResult):
    def response(self, data, status: int=200, serialize: bool=True, paginate: bool=True, **kwargs) -> Response:
...
class HTTPPaginator(BasePagination):
    ...
    def paginate_queryset(self, keys, request, **kwargs):
        ...
        start, end, pagesize, iTotalRecords, split = getPageParams(request, keys, self.page_size_query_param)

...
class TileType(object):
    ...
    def render(self, window: pyglet.window.Window, level: 'Level', x, y, wx, wy, cam_angle, cam_zoom):
        ...
        if (x, y) in self.sprite_cache:
            ...
            sprite = pyglet.sprite.Sprite(self.image, wx, wy, batch=level.batch)
            self.sprite_cache[x, y] = sprite
    ...
    def unrender(self, window: pyglet.window.Window, level: 'Level', x, y, wx, wy, cam_angle, cam_zoom):
        if (x, y) in self.sprite_cache:
            s = self.sprite_cache[x, y] # type: pyglet.sprite.Sprite
            s.delete()
            del self.sprite_cache[x, y]
...
class Manager(EntityContainer):
    ...
    def middle(self):
        return Vector((self.window.width / 2, self.window.height / 2))
    ...
    def camera_transform(self, window, x, y):
        v = (Vector((x, y)) - self.camera).rotate(-self.camera_angle) * self.camera_zoom + self.middle()
        return v
    ...
    def un_camera_transform(self, window, x, y):
        v = (Vector((x, y)) - self.middle()).rotate(self.camera_angle) / self.camera_zoom + self.camera
        return v
    ...
    def emit(self, source, event_name, *args):
        ...
        for system in self.systems:
            system._on(check_name, source, *args)

        for tt in self.tile_types.values():
            tt._on(self, check_name, source, *args)

        for gel in self.global_event_listeners:
            gel(source, event_name, *args)

    def emit_all(self, event_name, *args):
        for e in self:
            self.emit(e, event_name, *args)

...
def setupSourceBounce(myDevice, sourceNumber, bouncePattern):
    ...
    if (bounceParams[0] == "Simple"):
        ...
        if (executeAndCheckCommand(myDevice,
                                   "source:" + str(sourceNumber) + ":bounce:length " + bounceParams[2]) == False):
            commandSuccess = False;
        # Set the bounce period
        if (executeAndCheckCommand(myDevice,
                                   "source:" + str(sourceNumber) + ":bounce:period " + bounceParams[3]) == False):
            commandSuccess = False;
        # Set the bounce duty cycle (strip the %, which is not needed for the command)
        if (executeAndCheckCommand(myDevice, "source:" + str(sourceNumber) + ":bounce:duty " + bounceParams[4].strip(
                '%')) == False):
            commandSuccess = False;
    else:
...
def unhIolHotPlugSimpleSweep(quarchName, driveName, startTime, endTime, stepCount, repeats=1, onTime=10, offTime=5):
    ...
    sendLogMessage(time.time(), "testDescription",
                   "Starting Hotplug sweep test: " + startTime + "-" + endTime + " on device: " + driveName,
                   os.path.basename(__file__) + " - " + sys._getframe().f_code.co_name);
    ...
    for nextTime in testTimes:
        ...
        sendLogMessage(time.time(), "testDescription",
                       "Starting Hotplug sweep sub-point: " + str(nextTime) + "mS on device: " + driveName,
                       os.path.basename(__file__) + " - " + sys._getframe().f_code.co_name);
        ...
        if (driveTestConfig.testCallbacks["TEST_GETDISKSTATUS"](driveObject, 1)):
            sendLogMessage(time.time(), "testResult", "HOT_PLUG: Drive detected as expected",
                           os.path.basename(__file__) + " - " + sys._getframe().f_code.co_name, {"testResult": True});
        else:
            sendLogMessage(time.time(), "testResult", "HOT_PLUG: Drive NOT detected as expected",
        ...
        for loop in range(0, repeats):
            ...
            if (driveTestConfig.testCallbacks["TEST_GETDISKSTATUS"](driveObject, 0)):
                sendLogMessage(time.time(), "testResult", "HOT_PLUG: Drive removed, as expected",
                               os.path.basename(__file__) + " - " + sys._getframe().f_code.co_name,
                               {"testResult": True});
            else:
                sendLogMessage(time.time(), "testResult", "HOT_PLUG: Drive NOT removed",
                               os.path.basename(__file__) + " - " + sys._getframe().f_code.co_name,
                               {"testResult": False});
                errorSubCounter += 1
            ...
            if (driveTestConfig.testCallbacks["TEST_GETDISKSTATUS"](driveObject, 1)):
                ...
                sendLogMessage(time.time(), "testResult", "HOT_PLUG: Drive NOT detected",
                               os.path.basename(__file__) + " - " + sys._getframe().f_code.co_name,
                               {"testResult": False});
                errorSubCounter += 1
        ...
        if errorSubCounter == 0:
            sendLogMessage(time.time(), "testResult", "Hotplug sweep test passed all repeats at: " + str(nextTime),
                           os.path.basename(__file__) + " - " + sys._getframe().f_code.co_name, {"testResult": True});
        else:
            sendLogMessage(time.time(), "testResult",
                           "Hotplug sweep test failed: " + str(errorSubCounter) + " repeats at: " + str(nextTime),
                           os.path.basename(__file__) + " - " + sys._getframe().f_code.co_name, {"testResult": False});

    # Track errors from the entire sweep
    if errorCounter == 0:
        sendLogMessage(time.time(), "testResult", "Hotplug sweep test passed all tests",
                       os.path.basename(__file__) + " - " + sys._getframe().f_code.co_name, {"testResult": True});
    else:
        sendLogMessage(time.time(), "testResult",
                       "Hotplug sweep test failed: " + str(errorCounter) + " repeats in total",
                       os.path.basename(__file__) + " - " + sys._getframe().f_code.co_name, {"testResult": False});
...
def simpleHotPlugTest(uniqueID, quarchName, driveName, insertTime, repeats=1, onTime=10, offTime=5):
    ...
    # Log start of test
    sendLogMessage(time.time(), "testDescription",
                   "Starting Hotplug test at: " + str(insertTime) + "mS on device: " + driveName + ", over " + str(
                       repeats) + " repeats", os.path.basename(__file__) + " - " + sys._getframe().f_code.co_name,
                   uId=uniqueID);
    ...
    for loop in range(0, int(repeats)):
        ...
        try:
            if (driveTestConfig.testCallbacks["TEST_GETDISKSTATUS"](uniqueID, driveObject, 0)):
                sendLogMessage(time.time(), "testResult", "HOT_PLUG: Drive removed, as expected",
                               os.path.basename(__file__) + " - " + sys._getframe().f_code.co_name,
                               {"testResult": True}, uId=uniqueID);
            else:
                sendLogMessage(time.time(), "testResult", "HOT_PLUG: Drive NOT removed as expected",
                               os.path.basename(__file__) + " - " + sys._getframe().f_code.co_name,
                               {"testResult": False}, uId=uniqueID);
                errorSubCounter += 1
        except Exception:
            executeAndCheckCommand(quarchDevice, "run:power down")
            raise Exception("Broke again")  ##remove this frig
        # Plug the module
        executeAndCheckCommand(quarchDevice, "run:power up")
    ...
    # Track errors from this time step
    if errorSubCounter == 0:
        sendLogMessage(time.time(), "testResult", "Hotplug test passed all repeats",
                       os.path.basename(__file__) + " - " + sys._getframe().f_code.co_name, {"testResult": True},
                       uId=uniqueID);
    else:
        sendLogMessage(time.time(), "testResult",
                       "Hotplug sweep test failed: " + str(errorSubCounter) + " sub-test points",
                       os.path.basename(__file__) + " - " + sys._getframe().f_code.co_name, {"testResult": False},
                       uId=uniqueID);
...
def simpleHotPlugBounceTest(uniqueID, quarchName, driveName, insertTime, bouncePattern, bounceSignals, repeats=1,
                            onTime=10, offTime=5, bounceOnPlug=True, bounceOnPull=False):
    startTestTime = time.time()
    ...
    if ("TEST_GETRESOURCE" not in driveTestConfig.testCallbacks):
        ...
        # Log start of test
    sendLogMessage(time.time(), "testDescription", "Starting (Bounce) Hotplug test at: " + str(
        insertTime) + "mS on device: " + driveName + ", over " + str(repeats) + " repeats",
                   os.path.basename(__file__) + " - " + sys._getframe().f_code.co_name, uId=uniqueID);

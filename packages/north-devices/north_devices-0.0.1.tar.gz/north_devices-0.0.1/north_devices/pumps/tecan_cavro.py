...
class TecanCavro:
    ...
    def __init__(self, serial: Serial, address: int, syringe_volume_ml: float=1.0, counts_per_stroke: int=3000,
    ...
    def command_request_raw(self, command: str, parameters: List[int]=[], retry: bool=False, execute: bool=True,
        ...
        if log:
            self.logger.debug(f'Sending command: {command_str} [{command_message}]')
        ...
        try:
            ...
            response_stripped = response.lstrip(b'\xff')  # strip off garbage from the beginning
        ...
        except (SerialReadTimeoutException, TecanCavroResponseChecksumError) as err:
            if retries <= 0:
                raise err

            if log:
                self.logger.warn(f'Retrying message, {retries} remaining')
    ...
    def home(self, home_plunger: bool=True, home_valves: bool=True, wait: bool=True, batch: bool=False):
        ...
        if home_plunger and home_valves:
            self.command_request('Z', [self.plunger_home_speed], batch=batch)
        elif home_plunger:
    ...
    def speed(self, velocity_counts: int):
        self.command_request('V', [velocity_counts])
        self._velocity_counts = velocity_counts
    ...
    def move_absolute_counts(self, position_counts: Union[float, int], velocity_counts: Optional[int]=None, wait: bool=True):
        try:
            if velocity_counts is not None:
                self.command_request('V', [velocity_counts], batch=True)

            self.command_request('A', [int(position_counts)], batch=True)

            if velocity_counts is not None:
                self.command_request('V', [self.velocity_counts], batch=True)
    ...
    def move_relative_counts(self, delta_counts: Union[float, int], velocity_counts: Optional[int]=None, wait: bool=True):
        try:
            if velocity_counts is not None:
                self.command_request('V', [velocity_counts], batch=True)
    ...
    def move_absolute_ml(self, position_ml: float, velocity_counts: Optional[int]=None, wait: bool=True):
        self.move_absolute_counts(ml_to_counts(position_ml, self.counts_per_ml), velocity_counts=velocity_counts, wait=wait)

    def move_relative_ml(self, delta_ml: float, velocity_counts: Optional[int]=None, wait: bool=True):
        self.move_relative_counts(ml_to_counts(delta_ml, self.counts_per_ml), velocity_counts=velocity_counts, wait=wait)

    def move_valve(self, position: int, wait: bool=True):
    ...
    def dispense_ml(self, volume_ml: float, from_port: int, to_port: int, velocity_counts: Optional[int]=None,
                    wait: bool=True, execute: bool=True):
        ...
        if iterations > 0:
            self.loop_start()
            self.move_valve(from_port)
            self.move_absolute_ml(self.syringe_volume_ml, velocity_counts=velocity_counts)
            self.move_valve(to_port)
            self.move_absolute_ml(0, velocity_counts=velocity_counts)
            self.loop_end(iterations)

        # dispense any remaining volume after looping
        self.move_valve(from_port)
        self.move_absolute_ml(remaining_volume, velocity_counts=velocity_counts)
        self.move_valve(to_port)
        self.move_absolute_ml(0, velocity_counts=velocity_counts)

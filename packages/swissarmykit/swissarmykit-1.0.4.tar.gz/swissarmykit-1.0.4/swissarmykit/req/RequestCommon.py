...
class RequestCommon(BotDetect, HeadersUtils, UserAgentUtils, PathUtils):
    ...
    def _get_kw(self, url, override_headers: dict = None, **kwargs):
        ...
        if self.is_use_one_proxy():
            p = self.get_one_proxy()
            kwargs['proxies'] = p.get_proxies()
    ...
    def post(self, action='', override_headers: dict = None, payload=None, payload_json=None, json=None, **params):  # type: (str, dict, str, dict, dict, dict) -> str
        '''
        ...
        def call(item: BaseModel):
            ...
            if html:
                if utf_8:
                    ...
                    item.save_html(call_process_html(html))
                else:
                    item.save_html(html)
            else:
    ...
    def get_html_of_from_url_list(self, url_lst, model, number_threads=None, base_url='', no_retry_if_have_html=False,
                                  re_scrape=False, utf_8=False,
                                  sleep_time=0.5):  # type: (list, BaseModel, int, str, bool, bool, bool, int) -> bool
        ...
        def call(url):
            ...
            if html:
                model_obj = model.save_url(url)  # type: BaseModel
                if utf_8:
                    model_obj.save_html(StringUtils.get_valid_text_for_excel(html))
                else:
                    model_obj.save_html(html)
            else:
                self.failed_task += 1
        ...
        self.thread_pool(task_lst=lst, callback=call, number_threads=number_threads)
        print('INFO: Done scrape url. Total: %d' % model.count())
    ...
    def thread_pool(self, task_lst=None, callback=None, number_threads=None, multiple_process=False):
        timer = self.init_timer(len(task_lst))

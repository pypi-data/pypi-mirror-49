...
class Server(object):
    def emit(self, source, event_name, *args, **kwargs):
        for sess in self.clients.keys():
            self.send(sess, 'EVENT', time.time(), self.game.id, source, event_name, args, kwargs)
    ...
    async def accept(self, client, address):
        ...
        print('New client connected from:', ':'.join(address))
            ...
            if self.players[session]:
                ...
                for s in self.clients.keys():
                    if s != session:
                        pl = {
                            'eid': p.id,
                            'components': [(comp.name, comp.value, type(comp).__name__) for comp in p.get_components()]
                        }

                        if p.level is not self.manager:
                            pl['level'] = p.level.id

                        self.send(s, 'JOINED', local_id, pl)
        ...
        @command('change')
        def on_change(source, comp_name, comp_value):
            try:
                real = source[comp_name].value
                ...
                if real != comp_value:
                    # print('[{}] Correcting {} from {} to {}'.format(self.players[session]['name'].value, comp_name, comp_value, real))
                    self.send(session, 'EVENT', time.time(), self.locals[session], source, ('change', comp_name), [source[comp_name]], {})

...
def main(argv=None):
    ...
    try:
        ...
        if ((min(options.idxread) < 1)
                or (max(options.idxread) > len(options.fastq_files))):
    ...
    _basename, extension = os.path.splitext(options.fastq_files[0])
    if extension == '.gz':
        options.gzipin = True
    ...
    if (options.gzipout is True
            or (options.suffix is not None and options.suffix == '.gz')):
        ...
        for line in filehandle:
            ...
            if line and line[0] != '#':
                ...
                for i, e in enumerate(defined_barcodes):
                    barcodes[i].append(e)
                if ((sample_id_clean is not None)
                        and len(defined_barcodes) == num_dims):
    ...
    for primary_read in read_fastq(in_handles[primary_index]):
        try:
            ...
            if(barcode_path is None):
                ...
                if (UNMATCHED not in unmatched_path
                        and MULTIMATCHED not in unmatched_path):
...
def getRealBarcodes(barcodes):
    ...
    return([bc for bc in barcodes if (bc is not UNMATCHED
                                      and bc is not MATCHED
                                      and bc is not ANY
                                      and bc is not MULTIMATCHED
                                      and bc is not None)])
...
def reduceNDDictPaths(in_dict):
    ...
    if isinstance(in_dict, dict) and len(in_dict) > 0:
        ...
        # (in_dict.keys())
    if (isinstance(in_dict, dict) and len(in_dict) > 0
            and isinstance(in_dict[arb_bc][1], dict)):
        for bc in in_dict:
            # Reduction (skipping the list...)
            in_dict[bc] = in_dict[bc][1]
            reduceNDDictPaths(in_dict[bc])
    # Else if the value is a list of lists
    elif (isinstance(in_dict, dict) and len(in_dict) > 0
            and isinstance(in_dict[list(in_dict.keys())[0]][1], list)):
        ...
        for bc in in_dict:
            ...
            if (len(in_dict[bc]) > 0 and isinstance(in_dict[bc], list)
                    and len(in_dict[bc][0]) > 0
                    and isinstance(in_dict[bc][0], list)):
...
def getMismatchedBarcodesHelper(barcode, mismatches, skips):
    ...
    for p in range(0, len(barcode)):
        ...
        if mismatches > 1:
            if barcode[p] != 'N':
                offbys += getMismatchedBarcodesHelper(
                    barcode[:p] + 'N' + barcode[p + 1:], mismatches - 1,
                    newskips)
            if barcode[p] != 'A':
                offbys += getMismatchedBarcodesHelper(
                    barcode[:p] + 'A' + barcode[p + 1:], mismatches - 1,
                    newskips)
            if barcode[p] != 'T':
                offbys += getMismatchedBarcodesHelper(
                    barcode[:p] + 'T' + barcode[p + 1:], mismatches - 1,
                    newskips)
            if barcode[p] != 'G':
                offbys += getMismatchedBarcodesHelper(
                    barcode[:p] + 'G' + barcode[p + 1:], mismatches - 1,
                    newskips)
            if barcode[p] != 'C':
                offbys += getMismatchedBarcodesHelper(
                    barcode[:p] + 'C' + barcode[p + 1:], mismatches - 1,
                    newskips)
    return(offbys)
    ...
    if exeExists('gzip'):
        ...
        try:
            ...
            sys.stderr.write("Error executing command '{}' : {}\n".format(
                subprocess.list2cmdline(cmd), err))
        else:
            ...
            exit_code = p.wait()
            if exit_code != 0:
                sys.stderr.write("Command '{}' returned non-zero exit "
                                 "status {}.\n\tSee above for stderr output\n".
                                 format(subprocess.list2cmdline(cmd),
                                        p.returncode))
            else:
                external_gzip_ok = True
    if not external_gzip_ok:
        sys.stderr.write("WARNING: Using internal gzip implementation to "
                         "read '{}': possibly slower than external gzip.\n".
                         format(infile))
...
def open_gzip_out(outfile):
    ...
    if not external_gzip_ok:
        sys.stderr.write("WARNING: Using internal gzip implementation to "
                         "write '{}': possibly slower than external gzip.\n".
                         format(outfile))

...
def closest_synonyms(words: Tuple[str], year: int = 1990, k: int = 10, method: Method = Method.SVD):
    ...
    if method == Method.SGNS:
        m = load(dataDirPath + '/sgns-hamilton/' + str(year) + '-w.npy')
        vocabArray = load(dataDirPath + '/sgns-hamilton/' +
                          str(year) + '-vocab.pkl', allow_pickle=True)

    elif method == method.SVD:
        m = load(dataDirPath + '/embeddings/' + str(year) + '_svd_PPMI.npy')
        vocabArray = load(dataDirPath + '/vocabulary.pkl', allow_pickle=True)

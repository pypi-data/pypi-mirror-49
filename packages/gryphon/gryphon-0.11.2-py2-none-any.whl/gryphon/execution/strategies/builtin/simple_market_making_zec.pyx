...
class SimpleMarketMakingZEC(Strategy):
    def __init__(self, db, harness, strategy_configuration):
        super(SimpleMarketMakingZEC, self).__init__(db, harness)

        # Configurable properties with defaults.
        self.spread = Decimal('0.01')
        self.base_volume = Money('0.10', 'ZEC')
    ...
    def configure(self, strategy_configuration):
        super(SimpleMarketMakingZEC, self).configure(strategy_configuration)

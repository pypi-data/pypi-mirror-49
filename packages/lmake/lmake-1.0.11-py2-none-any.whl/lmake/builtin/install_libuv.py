...
URL_BASE = "https://dist.libuv.org/dist/v{}/libuv-v{}.tar.gz"
FILENAME_BASE = "libuv-v{}.tar.gz"
VERSION = "1.28.0"
...
def check_installed():
    lib_path = os.path.join(get_install_dir(), "lib", "libuv_a.a")
...
def install():
    ...
    print "Download libuv from url: ", url
    ...
    uv_folder = os.path.join(cache_dir, "libuv-v{}".format(VERSION))
    platform = get_platform()
    build_folder = os.path.join(uv_folder, "build." + str(platform))
    run_command(["mkdir", "-p", build_folder], cwd=uv_folder)
...
def main():
    if check_installed():
        print "libuv has been installed"
        return

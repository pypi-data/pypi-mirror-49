...
from gryphon.lib.exchange.consts import BID, ASK


class LinearMarketMaking(Strategy):
    def tick(self, current_orders):
        ob = self.bitstamp_btc_usd.get_orderbook()

        bid_price, ask_price = mm.midpoint_centered_fixed_spread(ob, 0.01)

        bid_volume, ask_volume = mm.position_responsive_sizing(
            Money('0.005', 'BTC'),
            self.position,
        )

        if bid_volume > 0:
            self.bitstamp_btc_usd.limit_order(BID, bid_volume, bid_price)

        if ask_volume > 0:
            self.bitstamp_btc_usd.limit_order(ASK, ask_volume, ask_price)

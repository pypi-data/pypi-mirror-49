# pylint: disable=trailing-newlines
# pylint: disable=line-too-long
# pylint: disable=too-many-lines
# pylint: disable=unused-import
# pylint: disable=invalid-name

r"""
Copyright &copy; 2019 NetApp Inc.
All rights reserved.


"""

from marshmallow import EXCLUDE, fields  # type: ignore
from netapp_ontap.resource import Resource, ResourceSchema


__all__ = ["MaxdataOnSanApplicationComponents", "MaxdataOnSanApplicationComponentsSchema"]
__pdoc__ = {
    "MaxdataOnSanApplicationComponentsSchema.resource": False,
    "MaxdataOnSanApplicationComponents": False,
}


class MaxdataOnSanApplicationComponentsSchema(ResourceSchema):
    """The fields of the MaxdataOnSanApplicationComponents object"""

    file_system = fields.Str()
    r""" Defines the kind of file system that will be installed on this application component. Optional in the POST body

Valid choices:

* generic
* m1fs
* xfs """
    host_management_url = fields.Str()
    r""" The host management URL for this application component
 """
    host_name = fields.Str()
    r""" FQDN of the L2 host that contains the hot tier of this application component. Required in the POST body
 """
    igroup_name = fields.Str()
    r""" The name of the initiator group through which the contents of this application will be accessed. Modification of this parameter is a disruptive operation. All LUNs in the application component will be unmapped from the current igroup and re-mapped to the new igroup. Required in the POST body and optional in the PATCH body
 """
    lun_count = fields.Integer()
    r""" The number of LUNs in the application component. Required in the POST body
 """
    metadata = fields.Nested("MaxdataOnSanApplicationComponentsMetadataSchema", unknown=EXCLUDE, many=True)
    r""" The metadata field of the maxdata_on_san_application_components.
 """
    name = fields.Str()
    r""" The name of the application component. Required in the POST body and optional in the PATCH body
 """
    protection_type = fields.Nested("MaxdataOnSanApplicationComponentsProtectionTypeSchema", unknown=EXCLUDE)
    r""" The protection_type field of the maxdata_on_san_application_components.
 """
    storage_service = fields.Nested("MaxdataOnSanApplicationComponentsStorageServiceSchema", unknown=EXCLUDE)
    r""" The storage_service field of the maxdata_on_san_application_components.
 """
    total_size = fields.Integer()
    r""" The total size of the application component, split across the member LUNs. Usage: {&lt;integer&gt;[KB|MB|GB|TB|PB]} Required in the POST body
 """

    @property
    def resource(self):
        return MaxdataOnSanApplicationComponents

    @property
    def patchable_fields(self):
        return [
            "igroup_name",
            "name",
        ]

    @property
    def postable_fields(self):
        return [
            "file_system",
            "host_name",
            "igroup_name",
            "lun_count",
            "metadata",
            "name",
            "protection_type",
            "storage_service",
            "total_size",
        ]


class MaxdataOnSanApplicationComponents(Resource):  # pylint: disable=missing-docstring

    _schema = MaxdataOnSanApplicationComponentsSchema

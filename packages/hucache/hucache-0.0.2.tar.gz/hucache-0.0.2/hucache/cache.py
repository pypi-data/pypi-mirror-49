...
class Cache(object):
    ...
    def __init__(self, store, prefix, func, ruler):
        self._store = store
        self._prefix = prefix
        self._func = func
        self._ruler = ruler
    ...
    def __call__(self, *args, **kwargs):
        ...
        if result is NOT_FOUND:
            ...
            self._store.set(cache_key, result)
        return result
...
class CacheFactory(object):
    ...
    @staticmethod
    def from_store_url(store_type, store_url, prefix=None):
        if store_type == "redis":
            store = RedisStore.from_url(store_url)
            return CacheFactory(store, prefix).cache
        else:
            raise NotImplementedError

    @staticmethod
    def from_store_conn(store_type, conn, prefix=None):
        if store_type == "redis":
    ...
    def cache(self, ruler):
        def wrapper(func):
            return Cache(self._store, self._prefix, func, ruler)
        return wrapper
